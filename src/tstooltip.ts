...
class Tooltip {
    ...
    constructor() {
        this.defaults = {
            ...
            class           : '',       // add class for tsg-tooltip-body
    ...
    show(name?: string | HTMLElement | TooltipOptions, extraOptions?: TooltipOptions): AttachReturn | { overlay: TooltipOverlay } | undefined { // any: name is polymorphic (string|HTMLElement|options)
        ...
        if (options.html === '' || options.html == null) {
            ...
            query(overlay.box)
                .find('.tsg-overlay-body')
                .attr('style', (options.style || '') + '; ' + overlayStyles)
                .removeClass(null) // removes all classes
                .addClass('tsg-overlay-body ' + options.class + (options.draggable ? ' tsg-draggable' : ''))
                .html(options.html)
            this.resize(overlay.name)
        } else {
            // event before
        ...
        // bind events
        TsUtils.bindEvents(query(overlay.box).find('.tsg-eaction'), this as unknown as Record<string, unknown>) // any: bindEvents accepts event handler object; Tooltip is valid
        ...
        query(overlay.box)
            .off('mousedown.tsg-bringfront')
            .on('mousedown.tsg-bringfront', () => {
    ...
    resize(name?: string): { moved: boolean; resize: boolean } | { multiple: boolean } | void {
        ...
        qBox.then((q: Query): Query => {
            ...
            .find('.tsg-overlay-body')
            .removeClass('tsg-arrow-right tsg-arrow-left tsg-arrow-top tsg-arrow-bottom')
    ...
    startDrag(event: MouseEvent & { target: EventTarget & { _lastBoundingRect?: DOMRect } }): void {
        ...
        const el = query(event.target).closest('.tsg-overlay')
        ...
        query(document)
            .off('.tsg-drag')
            .on('selectstart.tsg-drag, dragstart.tsg-drag', e => e.preventDefault())
            .find('body')
            .addClass('tsg-overlay-dragging')
        query('html')
            .off('.TsColor')
        ...
        function mouseUp(_event: any) { // any: query event
            ...
            query(document).off('selectstart.tsg-drag')
            query(document).off('dragstart.tsg-drag')
            query(document.body).removeClass('tsg-overlay-dragging')
            if (initial['moved']) {
        ...
        function mouseMove(event: any) { // any: query event
            ...
            if (!initial._removed) {
                ...
                initial.el.find(':scope > .tsg-overlay-body')
                    .removeClass('tsg-arrow-right tsg-arrow-left tsg-arrow-top tsg-arrow-bottom')
            }
        }
    }
...
class ColorTooltip extends Tooltip {
    ...
    override attach(anchor: any, text?: any): AttachReturn | undefined { // any: params are overloaded per call site
        ...
        overlay.on('show:after.attach', (_event: any) => { // any: TsBase event
            if (ret.overlay?.box) {
                const actions = query(ret.overlay.box).find('.tsg-eaction')
    ...
    select(color: any, name: any) { // any: color is string; name can be string or Event-like object
        ...
        if (typeof name != 'string') {
            ...
            name = query(target).closest('.tsg-overlay').attr('name') as string // any: attr returns string|undefined; name is always present here
        }
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        ...
        query(overlay.box).find('.tsg-color.tsg-selected').removeClass('tsg-selected')
        if (target) {
            query(target).addClass('tsg-selected')
        }
        // event after
        edata.finish()
    }
    ...
    tabClick(index: any, name: any) { // any: index is number or string; name can be string or Event-like
        if (typeof name != 'string') {
            name = query(name.target).closest('.tsg-overlay').attr('name')
        }
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const overlay: any = this.get(name) // any: get() returns union; runtime is TooltipOverlay
        const tab = query(overlay.box).find(`.tsg-color-tab:nth-child(${index})`)
        query(overlay.box).find('.tsg-color-tab').removeClass('tsg-selected')
        query(tab).addClass('tsg-selected')
        query(overlay.box)
            .find('.tsg-tab-content')
            .hide()
            .closest('.tsg-colors')
            .find('.tab-'+ index)
            .show()
    ...
    override initControls(overlay: TooltipOverlay): void {
        ...
        function setColor(color: any, fullUpdate?: boolean, initial?: string) {
            ...
            if (initial != null) {
                ...
                query(overlay.box).find('.tsg-color.tsg-selected')
                    .removeClass('tsg-selected')
                query(overlay.box).find(`.tsg-colors [name="${color}"], .tsg-colors [name="${initial}"]`) // color conversion might be slightly off
                    .addClass('tsg-selected')
    ...
    async pickAndSelect(name: string, event: any) {
        ...
        if (typeof color == 'string' && color.substr(0, 1) == '#' && [7, 9].includes(color.length)) {
            ...
            const cnt = query(event.target).closest('.tsg-colors-custom')
            cnt.html(this.getCustomColorsHTML(name))
            TsUtils.bindEvents(cnt.find('.tsg-eaction'), this as unknown as Record<string, unknown>)
...
class MenuTooltip extends Tooltip {
    ...
    override attach(anchor: any, text?: any): AttachReturn | undefined { // any: params are overloaded per call site
        ...
        overlay.click = () => {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            query(overlay.box).find('.tsg-selected').each((el: any) => { el.click() }) // any: query.each gives Node; runtime is HTMLElement
        }
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    override initControls(overlay: any): void {
        ...
        query(overlay.box).find('.tsg-menu:not(.tsg-sub-menu)')
            .off('.TsMenu')
            // any: callback parameter — caller signature varies; TsTooltip overlay options merge from multiple user sources at runtime
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            ...
            // any: callback parameter — caller signature varies; TsTooltip overlay options merge from multiple user sources at runtime
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            .on(`${mdown}.TsMenu`, { delegate: '.tsg-menu-item' }, (event: any) => {
                const dt = event.delegate.dataset
                const parents = query(event.delegate).closest('.tsg-menu').data('parents')
            ...
            // any: callback parameter — caller signature varies; TsTooltip overlay options merge from multiple user sources at runtime
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            .on(`${mclick}.TsMenu`, { delegate: '.tsg-menu-item' }, (event: any) => {
                const dt = event.delegate.dataset
                const parents = query(event.delegate).closest('.tsg-menu').data('parents')
            ...
            .on('mouseEnter.TsMenu', (event: any) => {
                ...
                // any: parameter typed any — runtime dispatch by call site; TsTooltip overlay options merge from multiple user sources at runtime
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                const _menu: any = query(event.target).closest('.tsg-menu').get(0)
    ...
    refreshIndex(name: string, instant?: boolean) {
        ...
        const view = query(overlay.box).find('.tsg-overlay-body').get(0) as HTMLElement
        const search = query(overlay.box).find('.tsg-menu-search, .tsg-menu-top').get(0) as HTMLElement
        query(overlay.box).find('.tsg-menu-item.tsg-selected')
            .removeClass('tsg-selected')
        const el = query(overlay.box).find(`.tsg-menu-item[index="${overlay.selected}"]`)
            .addClass('tsg-selected')
    ...
    showTooltip(name: string, options?: any) {
        ...
        const anchor = options?.anchor ?? query(overlay.box).find(`.tsg-menu-item[index="${overlay.selected}"]`).get(0)
        // any: cast-to-any for dynamic dispatch; TsTooltip overlay options merge from multiple user sources at runtime
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    refreshSearch(name: string) {
        ...
        TsTooltip.hide(overlay.name + '-tooltip')
        query(overlay.box).find('.tsg-no-items').hide()
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        query(overlay.box).find('.tsg-menu-item, .tsg-menu-divider').each((el: any) => { // any: query.each gives Node; runtime is HTMLElement
        ...
        // hide empty menus
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        query(overlay.box).find('.tsg-sub-menu').each((sub: any) => { // any: query.each gives Node; runtime is HTMLElement
            const hasItems = (query(sub).find('.tsg-menu-item').get() as HTMLElement[]).some(el => {
        ...
        if (overlay.tmp['searchCount'] == 0 || (overlay.options?.items?.length ?? 0) == 0) {
            if (query(overlay.box).find('.tsg-no-items').length == 0) {
                query(overlay.box).find('.tsg-menu:not(.tsg-sub-menu)').append(`
                    <div class="tsg-no-items">
                        ${TsUtils.lang(overlay.options.msgNoItems as string)}
                    </div>`)
            }
            query(overlay.box).find('.tsg-no-items').show()
        }
    }
    ...
    applyFilter(name: string, items: any, search: any, debounce?: any): Promise<any> {
        ...
        if (items == null && options.url && remote.hasMore && remote.search !== search) {
            ...
            query(overlay.box).find('.tsg-no-items').html(msg)
    ...
    menuDown(overlay: any, event: any, index: any, parents: any) {
        const options = overlay.options
        let items   = options.items
        const icon    = query(event.delegate).find('.tsg-icon')
        const menu    = query(event.target).closest('.tsg-menu:not(.tsg-sub-menu)')
        ...
        if ((options.type === 'check' || options.type === 'radio') && item.group !== false
                    ...
                    && !query(event.target).closest('.tsg-menu-item').hasClass('has-sub-menu')) {
        ...
        if (!query(event.target).hasClass('menu-remove') && !query(event.target).hasClass('menu-help')) {
            ...
            if (!query(event.delegate).hasClass('has-sub-menu')) {
                query(event.delegate).addClass('tsg-selected')
            }
        }
    }
    ...
    menuClick(overlay: any, event: any, index: any, parents: any) {
        const options  = overlay.options
        let items    = options.items
        const $item    = query(event.delegate).closest('.tsg-menu-item')
    ...
    keyUp(overlay: any, event: any) {
        ...
        switch (event.keyCode) {
            ...
            case 13: { // enter
                ...
                event.delegate = query(overlay.box).find('.tsg-selected').get(0)
            ...
            case 37: { // left
                ...
                if (Array.isArray(item?.items) && item.items.length > 0 && item.expanded) {
                    event.delegate = query(overlay.box).find(`.tsg-menu-item[index="${index}"]`).get(0)
                    overlay.selected = index
                    this.menuClick(overlay, event, parseInt(String(index)), parents)
                }
                filter = false
...
class DateTooltip extends Tooltip {
    ...
    override initControls(overlay: any) { // any: overlay shape varies at runtime
        ...
        const moveMonth = (inc: number) => {
            ...
            Object.assign(overlay.tmp, cal)
            query(overlay.box).find('.tsg-overlay-body').html(cal.html)
            this.initControls(overlay)
        }
        // any: parameter typed any — runtime dispatch by call site; TsTooltip overlay options merge from multiple user sources at runtime
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const checkJump = (event: any, dblclick?: boolean) => {
            query(event.target).parent().find('.tsg-jump-month, .tsg-jump-year')
                .removeClass('tsg-selected')
            query(event.target).addClass('tsg-selected')
            const dt = new Date()
        ...
        query(overlay.box)
            ...
            .on('click.calendar', event => {
                ...
                if (overlay.tmp.jump) {
                    ...
                    query(overlay.box).find('.tsg-overlay-body .tsg-cal-days')
        ...
        // now button
        query(overlay.box).find('.tsg-cal-now')
            .off('.calendar')
        ...
        query(overlay.box)
            ...
            .on('click.calendar', { delegate: '.tsg-day.tsg-date' }, event => {
                if (options.type == 'datetime') {
                    ...
                    query(overlay.box).find('.tsg-overlay-body').html(this.getHourHTML(overlay.options).html)
                    this.initControls(overlay)
                } else {
            ...
            // any: callback parameter — caller signature varies; TsTooltip overlay options merge from multiple user sources at runtime
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            .on('click.calendar', { delegate: '.tsg-time.hour' }, (event: any) => {
                ...
                if (options.noMinutes) {
                    ...
                    query(overlay.box).find('.tsg-overlay-body').html(html)
                    this.initControls(overlay)
                }
            })
            // click on minute
            // any: callback parameter — caller signature varies; TsTooltip overlay options merge from multiple user sources at runtime
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            .on('click.calendar', { delegate: '.tsg-time.min' }, (event: any) => {
        ...
        // After any innerHTML refresh, re-attach tsg-eaction handlers (startDrag on title, stop on arrows, etc.)
        TsUtils.bindEvents(query(overlay.box).find('.tsg-eaction'), this as unknown as Record<string, unknown>)
    }
    ...
    getMonthHTML(options: any, month?: any, year?: any) {
        ...
        for (let ci = 0; ci < 42; ci++) {
            ...
            html += `<div class="tsg-day ${this.inRange(tmp_dt, options, true)
                            ? 'tsg-date ' + (tmp_dt_fmt == selected_dsp ? 'tsg-selected' : '')
                            : 'tsg-blocked'
                        } ${className.join(' ')}"
