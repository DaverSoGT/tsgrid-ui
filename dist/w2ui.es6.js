  ...
  message(where, options) {
    ...
    const closeComplete = (options2) => {
      ...
      if (query(where.box).find(".w2ui-message").length <= 1) {
        ...
        query(where.box).find(`#w2ui-message-${where.owner?.name}-${options2["msgIndex"] - 1}`).css("z-index", "1500");
      }
      if (focus) {
    ...
    msgOpts["on"]("open", (event2) => {
      w2utils.bindEvents(query(msgOpts["box"]).find(".w2ui-eaction"), msgOpts);
    ...
    if (msgBase.html === "" && msgBase.body === "" && msgBase.buttons === "") {
      ...
      msgBase.box = query(where.box).find(`#w2ui-message-${where.owner?.name}-${msgBase.msgIndex}`)[0];
      ...
      openTimer = setTimeout(() => {
        query(where.box).find(`#w2ui-message-${where.owner?.name}-${msgBase.msgIndex}`).removeClass("animating").css({ "transition": "0s" });
        edata?.["finish"]?.();
      }, 300);
    }
    ...
    msgBase.close = () => {
      ...
      query(msgBase.box).addClass("w2ui-closing animating").css({
        "transition": "0.15s",
  ...
  prompt(where, options) {
    ...
    if (prom) {
      ...
      prom.action((event2) => {
        ...
        const d = event2["detail"];
        (d?.["self"])["input"] = query(d?.["box"]).find("#w2prompt").get(0);
        query(d?.["box"]).find("#w2prompt").on("keydown", (evt) => {
  ...
  notify(text, options) {
    return new Promise((resolve) => {
      ...
      if (textStr) {
        ...
        query(opts["where"]).find("#w2ui-notify").find(".w2ui-notify-close").on("click", (_event) => {
          query(opts["where"]).find("#w2ui-notify").remove();
          resolve();
        });
        if (opts["actions"]) {
          query(opts["where"]).find("#w2ui-notify .w2ui-notify-link").on("click", (event2) => {
  ...
  bindEvents(selector, subject) {
    ...
    query(normalizedSelector).each((el) => {
      ...
      Object.keys(actions).forEach((name) => {
        ...
        query(el).off(name + ".w2utils-bind").on(name + ".w2utils-bind", function(event2) {
          switch (method) {
            case "alert":
              alert(params[0]);
              break;
...
var w2toolbar = class extends w2base {
  ...
  remove(...args) {
    ...
    args.forEach((item) => {
      ...
      effected++;
      query5(this.box).find("#tb_" + this.name + "_item_" + w2utils.escapeId(it.id)).remove();
  ...
  setCount(id, count, className, style) {
    const btn = query5(this.box).find(`#tb_${this.name}_item_${w2utils.escapeId(id)} .w2ui-tb-count > span`);
    if (btn.length > 0) {
      ...
      if (["menu", "menu-radio", "menu-check", "drop", "color", "text-color"].includes(it.type)) {
        ...
        if (it.checked) {
          ...
          setTimeout(() => {
            ...
            const el = query5(this.box).find("#tb_" + this.name + "_item_" + w2utils.escapeId(it.id));
  ...
  scroll(direction, line, instant) {
    return new Promise((resolve, _reject) => {
      const scrollBox = query5(this.box).find(`.w2ui-tb-line:nth-child(${line}) .w2ui-scroll-wrapper`);
  ...
  render(box) {
    ...
    query5(this.box).attr("name", this.name).addClass("w2ui-reset w2ui-toolbar").html(html);
    if (query5(this.box).length > 0) {
      query5(this.box)[0].style.cssText += this["style"];
    }
    w2utils.bindEvents(query5(this.box).find(".w2ui-tb-line .w2ui-eaction"), this);
  ...
  refresh(id) {
    ...
    this.tooltipHide(id);
    if (it.type == "spacer") {
      query5(this.box).find(`.w2ui-tb-line:nth-child(${it.line ?? 1})`).find(".w2ui-tb-right").css("width", "auto");
    }
    if (btn.length === 0) {
      const next = parseInt(this.get(id, true)) + 1;
      let $next = query5(this.box).find(`#tb_${this.name}_item_${w2utils.escapeId(this.items[next] ? this.items[next].id : "--")}`);
      if ($next.length == 0) {
        $next = query5(this.box).find(`.w2ui-tb-line:nth-child(${it.line}`).find(".w2ui-tb-right").before(html);
      } else {
        $next.after(html);
      }
      w2utils.bindEvents(query5(this.box).find(`${selector}, ${selector} .w2ui-eaction`), this);
    } else {
  ...
  resize() {
    ...
    if (edata.isCancelled === true) return;
    query5(this.box).find(".w2ui-tb-line").each((el) => {
      const box = query5(el);
  ...
  destroy() {
    ...
    if (edata.isCancelled === true) return;
    if (query5(this.box).find(".w2ui-scroll-wrapper").length > 0) {
      this.unmount();
    }
  ...
  change(id, value, dynamic) {
    const it = this.get(id);
    const input = query5(this.box).find("#tb_" + this.name + "_item_" + w2utils.escapeId(id)).find("input.w2ui-toolbar-input");
  ...
  tooltipShow(id) {
    ...
    const el = query5(this.box).find("#tb_" + this.name + "_item_" + w2utils.escapeId(id)).get(0);
    const item = this.get(id);
...
var w2sidebar = class extends w2base {
  ...
  setCount(id, count, options = {}) {
    ...
    const btn = query6(this.box).find(`#node_${w2utils.escapeId(id)} .w2ui-node-badge`);
    if (btn.length > 0) {
  ...
  select(id) {
    ...
    const $el = query6(this.box).find("#node_" + w2utils.escapeId(id));
  ...
  unselect(id) {
    ...
    query6(this.box).find("#node_" + w2utils.escapeId(id)).removeClass("w2ui-selected").find(".w2ui-icon").removeClass("w2ui-icon-selected");
  ...
  collapse(id) {
    ...
    if (edata.isCancelled === true) return;
    query6(this.box).find("#node_" + w2utils.escapeId(id) + "_sub").hide();
    query6(this.box).find("#node_" + w2utils.escapeId(id) + " .w2ui-expanded").removeClass("w2ui-expanded").addClass("w2ui-collapsed");
    nd.expanded = false;
    edata.finish();
    this.refresh(id);
    return true;
  ...
  expand(id) {
    ...
    if (edata.isCancelled === true) return;
    query6(this.box).find("#node_" + w2utils.escapeId(id) + "_sub").show();
    query6(this.box).find("#node_" + w2utils.escapeId(id) + " .w2ui-collapsed").removeClass("w2ui-collapsed").addClass("w2ui-expanded");
    nd.expanded = true;
    edata.finish();
    this.refresh(id);
    return true;
  ...
  click(id, event2) {
    ...
    const newNode = query6(this.box).find("#node_" + w2utils.escapeId(id));
  ...
  flatMenu(el, items) {
    const self = this;
    const $el = query6(el).find(".w2ui-node-data");
    w2menu.show({
  ...
  focus(event2) {
    ...
    this.hasFocus = true;
    query6(this.box).find(".w2ui-sidebar-body").addClass("w2ui-focus");
    setTimeout(() => {
  ...
  blur(event2) {
    ...
    if (edata.isCancelled === true) return false;
    this.hasFocus = false;
    query6(this.box).find(".w2ui-sidebar-body").removeClass("w2ui-focus");
    edata.finish();
  }
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  next(node, noSubs) {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  inView(id) {
    const item = query6(this.box).find("#node_" + w2utils.escapeId(id)).get(0);
    if (!item) {
      return false;
    }
    const div = query6(this.box).find(".w2ui-sidebar-body").get(0);
    if (!div) return false;
  ...
  mouseDown(id, event2) {
    ...
    if (this.reorder) {
      ...
      const mv = this.last.move;
      const body = query6(this.box).find(".w2ui-sidebar-body");
      if (!mv.ghost) {
        const node = query6(this.box).find(`#node_${w2utils.escapeId(id)}`);
        mv.offsetY = event2.offsetY;
        mv.target = id;
        const nodeEl = node.get(0);
        mv.pos = { top: nodeEl.offsetTop - 1, left: nodeEl.offsetLeft };
        const clone = query6(node.find(".w2ui-node-data").get(0).cloneNode(true));
        mv.node = node;
        ...
        mv.resetReorder = () => {
          ...
          query6(document).off(`.w2ui-${this.name}-reorder`);
        };
      }
      query6(document).on(`mousemove.w2ui-${this.name}-reorder`, _mouseMove).on(`mouseup.w2ui-${this.name}-reorder`, _mouseStop);
    }
    function _mouseMove(event3) {
      ...
      const over = query6(event3.target).closest(".w2ui-node, .w2ui-node-group");
      const id2 = over.attr("data-id");
      if (query6(event3.target).hasClass("w2ui-sidebar-body") && event3.layerY > 5 && !mv.append) {
        ...
        const el = query6(self.box).find(`#node_${w2utils.escapeId(id2)}`);
        el.before(mv.node);
  ...
  render(box) {
    ...
    if (!this.box) return;
    query6(this.box).attr("name", this.name).addClass("w2ui-reset w2ui-sidebar").html(`<div>
    ...
    if (this["topHTML"] !== "" || flatHTML !== "") {
      query6(this.box).find(".w2ui-sidebar-top").html(this["topHTML"] + flatHTML);
      query6(this.box).find(".w2ui-sidebar-body").css("top", query6(this.box).find(".w2ui-sidebar-top").get(0)?.clientHeight + "px");
      query6(this.box).find(".w2ui-flat").off("click").on("click", (_event) => {
        this.goFlat();
      });
    }
    if (this["bottomHTML"] !== "") {
      query6(this.box).find(".w2ui-sidebar-bottom").html(this["bottomHTML"]);
      query6(this.box).find(".w2ui-sidebar-body").css("bottom", query6(this.box).find(".w2ui-sidebar-bottom").get(0)?.clientHeight + "px");
    }
    this.last.observeResize = new ResizeObserver(() => {
      this.resize();
    });
  ...
  update(id, options = {}) {
    ...
    let level;
    if (nd) {
      const $el = query6(this.box).find("#node_" + w2utils.escapeId(nd.id));
      if (nd.group) {
        if (options.text) {
  ...
  refresh(id, options = {}) {
    ...
    const body = query6(this.box).find(":scope > div > .w2ui-sidebar-body").get(0);
    ...
    if (this.flatButton == true) {
      query6(this.box).find(".w2ui-sidebar-top .w2ui-flat").show().removeClass("w2ui-flat-left w2ui-flat-right").addClass(` w2ui-flat-${this.flat ? "right" : "left"}`);
    } else {
      query6(this.box).find(".w2ui-sidebar-top .w2ui-flat").hide();
    }
    const boxEl2 = query6(this.box).get(0);
    query6(this.box).find(":scope > div").removeClass("w2ui-sidebar-flat").addClass(this.flat ? "w2ui-sidebar-flat" : "").css({
    ...
    const cnt = node == this ? query6(this.box).find(":scope > div > .w2ui-sidebar-body") : query6(body).find(nodeSubId);
    cnt.html("");
    ...
    if (!options.recursive) {
      const els = query6(this.box).find(`${nodeId}, ${nodeId} .w2ui-eaction, ${nodeSubId} .w2ui-eaction`);
  ...
  destroy() {
    const edata = this.trigger("destroy", { target: this.name });
    if (edata.isCancelled === true) return;
    if (query6(this.box).find(".w2ui-sidebar-body").length > 0) {
      this.unmount();
    }
    delete w2ui[this.name];
    edata.finish();
...
var w2tabs = class extends w2base {
  ...
  remove(...ids) {
    ...
    ids.forEach((it) => {
      ...
      query7(this.box).find(`#tabs_${this.name}_tab_${w2utils.escapeId(tab.id)}`).remove();
    });
    this.resize();
    return effected;
  }
  ...
  dragMove(event2) {
    ...
    const $el = query7(this.box).find("#tabs_" + this.name + "_tab_" + w2utils.escapeId(tab.id));
    if (info.divX > 0 && next) {
      const $nextEl = query7(this.box).find("#tabs_" + this.name + "_tab_" + w2utils.escapeId(next.id));
    ...
    if (info.divX < 0 && prev) {
      const $prevEl = query7(this.box).find("#tabs_" + this.name + "_tab_" + w2utils.escapeId(prev.id));
      let width1 = $el.get(0).clientWidth;
  ...
  tooltipShow(id) {
    const tab = this.get(id);
    const el = query7(this.box).find("#tabs_" + this.name + "_tab_" + w2utils.escapeId(id)).get(0);
  ...
  refresh(id) {
    const time = Date.now();
    if (this.flow == "up") {
      query7(this.box).addClass("w2ui-tabs-up");
    } else {
      query7(this.box).removeClass("w2ui-tabs-up");
    }
  ...
  render(box) {
    ...
    query7(this.box).attr("name", this.name).addClass("w2ui-reset w2ui-tabs").html(html);
    if (query7(this.box).length > 0) {
      query7(this.box)[0].style.cssText += this.style;
    }
    w2utils.bindEvents(query7(this.box).find(".w2ui-eaction"), this);
    this.last.observeResize = new ResizeObserver(() => {
      this.resize();
    });
    this.last.observeResize.observe(this.box);
  ...
  initReorder(id, event2) {
    ...
    const self = this;
    const $tab = query7(this.box).find("#tabs_" + this.name + "_tab_" + w2utils.escapeId(id));
    ...
    query7(document).off(".w2uiTabReorder").on("mousemove.w2uiTabReorder", function(event3) {
      ...
      if (!self.last.reordering) {
        ...
        $tab.css("opacity", 0);
        query7(self.box).find(".w2ui-scroll-wrapper").append($ghost.get(0));
        query7(self.box).find(".w2ui-tab-close").hide();
      }
      ...
      query7(self.box).find(".w2ui-tab-close").show();
      $ghost.remove();
  ...
  scroll(direction, instant) {
    return new Promise((resolve, _reject) => {
      const scrollBox = query7(this.box).find(".w2ui-scroll-wrapper");
      const scrollBoxEl = scrollBox.get(0);
      const scrollLeft = scrollBoxEl.scrollLeft;
  ...
  scrollIntoView(id, instant) {
    return new Promise((resolve, _reject) => {
      ...
      const tab = this.get(id);
      if (tab == null) return;
      const tabEl = query7(this.box).find("#tabs_" + this.name + "_tab_" + w2utils.escapeId(id)).get(0);
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  click(id, event2) {
    if (event2 && query7(event2.target).hasClass("w2ui-tab-close")) {
      return;
    }
    const tab = this.get(id);
    ...
    if (edata.isCancelled === true) return;
    query7(this.box).find("#tabs_" + this.name + "_tab_" + w2utils.escapeId(this.active)).removeClass("active");
    this.active = tab.id;
    query7(this.box).find("#tabs_" + this.name + "_tab_" + w2utils.escapeId(this.active)).addClass("active");
  ...
  animateInsert(id, tab) {
    return new Promise((resolve, _reject) => {
      let $before = query7(this.box).find("#tabs_" + this.name + "_tab_" + w2utils.escapeId(id));
...
var w2layout = class extends w2base {
    ...
    if (p.html === "") {
      ...
      if (!p.hidden) {
        if (transition != null && transition !== "") {
          ...
          switch (transition) {
            case "slide-left":
              style1 = "left: -" + w2utils.getSize(query8(this.box), "width") + "px";
              style2 = "left: 0px";
              break;
            case "slide-right":
              style1 = "left: " + w2utils.getSize(query8(this.box), "width") + "px";
              ...
              break;
            case "slide-down":
              style1 = "top: -" + w2utils.getSize(query8(this.box), "height") + "px";
  ...
  sizeTo(panel, size, instant) {
    ...
    query8(this.box).find(":scope > div > .w2ui-panel").css("transition", instant !== true ? ".2s" : "0s");
    setTimeout(() => {
      this.set(panel, { size });
    }, 1);
    setTimeout(() => {
      query8(this.box).find(":scope > div > .w2ui-panel").css("transition", "0s");
      this.resize();
    }, 300);
    return true;
  }
  show(panel, immediate) {
    ...
    if (immediate === true) {
      ...
      query8(this.box).find(":scope > div > .w2ui-panel").css("transition", ".2s");
      setTimeout(() => {
        this.resize();
      }, 1);
      setTimeout(() => {
  ...
  render(box) {
    ...
    if (!this.box) return false;
    query8(this.box).attr("name", this.name).addClass("w2ui-layout").html("<div></div>");
    ...
    function resizeStart(type, evnt) {
      ...
      w2panels.forEach((panel) => {
        const $tmp = query8(self.el(panel)).find(".w2ui-lock");
        if ($tmp.length > 0) {
          $tmp.data("locked", "yes");
        } else {
    ...
    function mouseUp(evnt) {
      ...
      if (self.last["diff_x"] !== 0 || self.last["resize"].diff_y !== 0) {
        ...
        const width = w2utils.getSize(query8(self.box), "width");
        const height = w2utils.getSize(query8(self.box), "height");
  ...
  refresh(panel) {
    ...
    if (typeof panel == "string") {
      ...
      if (typeof p.html == "object" && typeof p.html.render === "function") {
        ...
        setTimeout(() => {
          if (query8(self.box).find(pname + '> [data-role="panel-content"]').length > 0) {
            const $content = query8(self.box).find(pname + '> [data-role="panel-content"]').removeClass(null).removeAttr("name").addClass("w2ui-panel-content");
      ...
      tmp = query8(self.box).find(pname + "> .w2ui-panel-title");
      if (p.title) {
        ;
  ...
  resize() {
    ...
    const width = w2utils.getSize(query8(this.box), "width");
    const height = w2utils.getSize(query8(this.box), "height");
    const self = this;
  ...
  resizeBoxes(panel) {
    ...
    panels.forEach((pname, ind) => {
      ...
      if (pan) {
        if (pan.title) {
          const el = query8(this.box).find(tmp2 + ".w2ui-panel-title").css({ top: topHeight + "px", display: "block" });
...
var w2field = class extends w2base {
  ...
  init() {
    ...
    switch (this.type) {
      ...
      case "combo": {
        ...
        if (this.type === "list") {
          query9(this.el).addClass("w2ui-select");
      ...
      case "file": {
        ...
        this.selected = options.selected;
        if (query9(this.el).attr("placeholder") == null) {
          query9(this.el).attr("placeholder", w2utils.lang("Attach files by dragging and dropping or Click to Select"));
        }
        break;
      }
      default: {
    ...
    if (this.type == "list") {
      ...
      if (focus.val() === "") {
        ...
        if (this.selected?.id != null) {
          ...
          if (text != null) {
            ;
            query9(this.el).val(w2utils.lang(text)).data({
              selected: text,
              selectedIndex: ind[0]
            });
          }
        ...
        setTimeout(() => {
          ...
          if (options.icon) {
            ...
            query9(this.helpers.search).find(".w2ui-icon-search").addClass("show-search");
          } else {
            focus.css("margin-left", "0px");
            query9(this.helpers.search).find(".w2ui-icon-search").removeClass("show-search");
          }
        }, 1);
      }
    ...
    if (["enum", "file"].includes(this.type) && div) {
      ...
      if (html !== "") {
        ...
        const style = w2utils.stripSpaces(`
                ...
                `);
        div.prepend(`<div class="w2ui-enum-placeholder" style="${style}">${query9(this.el).attr("placeholder")}</div>`);
      }
      div.off(".w2item").on("scroll.w2item", (event2) => {
        ...
        let edata;
        if (query9(mouseEvent.target).hasClass("w2ui-list-remove")) {
          ...
          const input = query9(mouseEvent.target).closest(".w2ui-list").find("input.file-input").get(0);
          if (input) {
  ...
  resize() {
    ...
    if (["enum", "file"].includes(this.type) && div) {
      ...
      let cntHeight = query9(div).find(":scope div.w2ui-multi-items").get(0).clientHeight + 5;
  ...
  reset() {
    ...
    ;
    query9(this.el).val(this.clean(query9(this.el).val())).removeClass("w2field w2ui-input").removeData("selected selectedIndex").off(".w2field");
  ...
  keyDown(event2, extra) {
    ...
    if (this.type === "time") {
      ...
      if (cancel) {
        event2.preventDefault();
        query9(this.el).val(w2date2.min2str(time)).trigger("input").trigger("change");
        this.moveCaret2end();
      }
    }
  ...
  */
  ...
  addPrefix() {
    ...
    query9(this.el).before(`<div class="w2ui-field-helper">${this.options.prefix}</div>`);
  ...
  addSuffix() {
    ...
    if (this.options.suffix !== "") {
      ...
      query9(this.el).after(`<div class="w2ui-field-helper">${this.options.suffix}</div>`);
  ...
  addSearch() {
    ...
    query9(helper).find("input").off(".w2ui-helper").on("focus.w2ui-helper", (event2) => {
    ...
    query9(helper).off(".w2ui-helper").on("click.w2ui-helper", (_event) => {
...
var w2grid = class extends w2base {
  ...
  add(record, first) {
    ...
    if (!url) {
      ...
      if ((this.last.vscroll.recIndStart ?? 0) <= indEnd && (this.last.vscroll.recIndEnd ?? 0) >= indStart) {
        ...
        query10(this.box).find("#grid_" + this.name + "_footer .w2ui-footer-right .w2ui-total").html(w2utils.formatNumber(this.total));
      }
    } else {
      this.refresh();
    }
  ...
  refreshRanges() {
    ...
    for (let i = 0; i < this.ranges.length; i++) {
      ...
      let td1 = query10(this.box).find("#grid_" + this.name + "_rec_" + w2utils.escapeId(first.recid) + ' td[col="' + first.column + '"]');
      let td2 = query10(this.box).find("#grid_" + this.name + "_rec_" + w2utils.escapeId(last.recid) + ' td[col="' + last.column + '"]');
      let td1f = query10(this.box).find("#grid_" + this.name + "_frec_" + w2utils.escapeId(first.recid) + ' td[col="' + first.column + '"]');
      let td2f = query10(this.box).find("#grid_" + this.name + "_frec_" + w2utils.escapeId(last.recid) + ' td[col="' + last.column + '"]');
      let _lastColumn = last.column;
      if (first.column < this.last.vscroll.colIndStart && last.column > this.last.vscroll.colIndStart) {
        td1 = query10(this.box).find("#grid_" + this.name + "_rec_" + w2utils.escapeId(first.recid) + ' td[col="start"]');
      }
      if (first.column < this.last.vscroll.colIndEnd && last.column > this.last.vscroll.colIndEnd) {
        td2 = query10(this.box).find("#grid_" + this.name + "_rec_" + w2utils.escapeId(last.recid) + ' td[col="end"]');
        _lastColumn = "end";
      }
      ...
      if (td1f.length > 0 || td2f.length > 0) {
        ...
        if (td2f.length === 0) {
          td2f = query10(this.box).find("#grid_" + this.name + "_frec_" + w2utils.escapeId(last.recid) + " td:last-child");
      ...
      if (td1.length > 0 || td2.length > 0) {
        ...
        if (td1.length === 0) {
          td1 = query10(this.box).find("#grid_" + this.name + "_rec_" + w2utils.escapeId(first.recid) + " td:first-child");
    ...
    query10(this.box).find(".w2ui-selection-resizer").off(".resizer").on("mousedown.resizer", mouseStart).on("dblclick.resizer", (event2) => {
    ...
    function mouseStart(event2) {
      ...
      query10("body").off(".w2ui-" + self.name).on("mousemove.w2ui-" + self.name, mouseMove).on("mouseup.w2ui-" + self.name, mouseStop);
      event2.preventDefault();
    }
    function mouseMove(event2) {
      const mv = self.last.move;
    ...
    function mouseStop(_event) {
      ...
      query10("body").off(".w2ui-" + self.name);
    ...
    if (this.selectType == "row") {
      for (let a = 0; a < args.length; a++) {
        ...
        if (this.searchData.length !== 0 || index + 1 >= (this.last.vscroll.recIndStart ?? 0) && index + 1 <= (this.last.vscroll.recIndEnd ?? 0)) {
          recEl1 = query10(this.box).find("#grid_" + this.name + "_frec_" + w2utils.escapeId(recid));
          recEl2 = query10(this.box).find("#grid_" + this.name + "_rec_" + w2utils.escapeId(recid));
        }
        if (this.selectType == "row") {
      ...
      for (const recid in new_sel) {
        ...
        if (index + 1 >= (this.last.vscroll.recIndStart ?? 0) && index + 1 <= (this.last.vscroll.recIndEnd ?? 0)) {
          recEl1 = query10(this.box).find("#grid_" + this.name + "_rec_" + w2utils.escapeId(recid));
          recEl2 = query10(this.box).find("#grid_" + this.name + "_frec_" + w2utils.escapeId(recid));
        }
      ...
      for (let c = 0; c < col_sel.length; c++) {
        query10(this.box).find("#grid_" + this.name + "_column_" + col_sel[c] + " .w2ui-col-header").addClass("w2ui-col-selected");
      }
    }
  ...
  unselect(...unselectArgs) {
    ...
    for (let a = 0; a < args.length; a++) {
      ...
      const recEl1 = query10(this.box).find("#grid_" + this.name + "_frec_" + w2utils.escapeId(recid));
      const recEl2 = query10(this.box).find("#grid_" + this.name + "_rec_" + w2utils.escapeId(recid));
      if (this.selectType == "row") {
        if (sel.indexes.indexOf(index) == -1) continue;
        ...
        query10(this.box).find(`#grid_${this.name}_rec_${w2utils.escapeId(recid)} > td[col="${col}"]`).removeClass("w2ui-selected w2ui-inactive");
        query10(this.box).find(`#grid_${this.name}_frec_${w2utils.escapeId(recid)} > td[col="${col}"]`).removeClass("w2ui-selected w2ui-inactive");
        ...
        if (!isColSelected) {
          query10(this.box).find(`.w2ui-grid-columns td[col="${col}"] .w2ui-col-header, .w2ui-grid-fcolumns td[col="${col}"] .w2ui-col-header`).removeClass("w2ui-col-selected");
        }
        if (!isRowSelected) {
          query10(this.box).find("#grid_" + this.name + "_frec_" + w2utils.escapeId(recid)).find(".w2ui-col-number").removeClass("w2ui-row-selected");
        }
        unselected++;
        if (s.length === 0) {
  ...
  selectAll() {
    ...
    if (this.selectType == "row") {
      query10(this.box).find(".w2ui-grid-records tr:not(.w2ui-empty-record)").addClass("w2ui-selected").find(".w2ui-col-number").addClass("w2ui-row-selected");
      query10(this.box).find(".w2ui-grid-frecords tr:not(.w2ui-empty-record)").addClass("w2ui-selected").find(".w2ui-col-number").addClass("w2ui-row-selected");
      query10(this.box).find("input.w2ui-grid-select-check").prop("checked", true);
    } else {
      query10(this.box).find(".w2ui-grid-columns td .w2ui-col-header, .w2ui-grid-fcolumns td .w2ui-col-header").addClass("w2ui-col-selected");
      query10(this.box).find(".w2ui-grid-records tr .w2ui-col-number").addClass("w2ui-row-selected");
      query10(this.box).find(".w2ui-grid-records tr:not(.w2ui-empty-record)").find(".w2ui-grid-data:not(.w2ui-col-select)").addClass("w2ui-selected");
      query10(this.box).find(".w2ui-grid-frecords tr .w2ui-col-number").addClass("w2ui-row-selected");
      query10(this.box).find(".w2ui-grid-frecords tr:not(.w2ui-empty-record)").find(".w2ui-grid-data:not(.w2ui-col-select)").addClass("w2ui-selected");
      query10(this.box).find("input.w2ui-grid-select-check").prop("checked", true);
    }
  ...
  selectNone(skipEvent) {
    ...
    const sel = this.last.selection;
    if (this.selectType == "row") {
      query10(this.box).find(".w2ui-grid-records tr.w2ui-selected").removeClass("w2ui-selected w2ui-inactive").find(".w2ui-col-number").removeClass("w2ui-row-selected");
      query10(this.box).find(".w2ui-grid-frecords tr.w2ui-selected").removeClass("w2ui-selected w2ui-inactive").find(".w2ui-col-number").removeClass("w2ui-row-selected");
      query10(this.box).find("input.w2ui-grid-select-check").prop("checked", false);
    } else {
      query10(this.box).find(".w2ui-grid-columns td .w2ui-col-header, .w2ui-grid-fcolumns td .w2ui-col-header").removeClass("w2ui-col-selected");
      query10(this.box).find(".w2ui-grid-records tr .w2ui-col-number").removeClass("w2ui-row-selected");
      query10(this.box).find(".w2ui-grid-frecords tr .w2ui-col-number").removeClass("w2ui-row-selected");
      query10(this.box).find(".w2ui-grid-data.w2ui-selected").removeClass("w2ui-selected w2ui-inactive");
      query10(this.box).find("input.w2ui-grid-select-check").prop("checked", false);
    }
    sel.indexes = [];
    sel.columns = {};
  ...
  searchOpen(options = {}) {
    ...
    const $btn = query10(this.toolbar.box).find(".w2ui-grid-search-input .w2ui-search-drop");
      ...
      overlay.data("gridName", this.name).off(".grid-search").on("click.grid-search", (event2) => {
        overlay.find("input, select").each((el) => {
          ...
          if (names) names.forEach((name) => {
            w2tooltip3.hide(name);
          });
        });
        console.log(event2.target);
        if (!query10(event2.target).hasClass("w2ui-saved-searches")) {
          w2tooltip3.hide(this.name + "-search-suggest");
        }
      });
  ...
  searchClose() {
    w2tooltip3.hide(this.name + "-search-overlay");
  }
  ...
  searchFieldTooltip(ind, sd_ind, el) {
    ...
    w2tooltip3.hide(this.name + "-search-props");
    w2tooltip3.show({
  ...
  searchSave() {
    ...
    this.message({
      ...
      query10(event2.detail.box).find("#grid-search-save").on("click", () => {
        const input = query10(event2.detail.box).find(".w2ui-message .search-name");
      ...
      query10(event2.detail.box).find("input, button").off(".message").on("keydown.message", (evt) => {
        const val = String(query10(event2.detail.box).find(".w2ui-message-body input").val()).trim();
        ...
        const $save = query10(event2.detail.box).closest(".w2ui-message").find("#grid-search-save");
  ...
  searchShowFields(forceHide) {
    if (forceHide === true) {
      w2tooltip3.hide(this.name + "-search-fields");
      return;
    }
    const items = [];
    ...
    w2menu3.show({
      ...
      anchor: query10(this.box).find("#grid_" + this.name + "_search_name").parent().find(".w2ui-search-down").get(0),
      items,
      align: "none",
      hideOn: ["doc-click", "select"]
    }).select((event2) => {
  ...
  editField(recid, column, value, event2) {
    ...
    if (this.last.inEditMode === true) {
      if (event2 && event2.keyCode == 13) {
        ...
        const input2 = query10(this.box).find("div.w2ui-edit-box .w2ui-input");
        if (input2.length > 0) {
    ...
    const tr = query10(this.box).find("#grid_" + this.name + prefix + "rec_" + w2utils.escapeId(recid));
    let div = tr.find('[col="' + column + '"] > div');
    this.last._edit["tr"] = tr;
    this.last._edit["div"] = div;
    query10(this.box).find("div.w2ui-edit-box").remove();
    if (this.selectType != "row") {
      query10(this.box).find("#grid_" + this.name + prefix + "selection").attr("id", "grid_" + this.name + "_editable").removeClass("w2ui-selection").addClass("w2ui-edit-box").prepend('<div style="position: absolute; top: 0px; bottom: 0px; left: 0px; right: 0px;"></div>').find(".w2ui-selection-resizer").remove();
    ...
    switch (edit.type) {
      ...
      default: {
        ...
        const doHide = (event3) => {
          ...
          if (name && w2tooltip3.get(name[0])?.selected != null) {
            selected = true;
          }
    ...
    query10(input).off(".w2ui-editable").on("blur.w2ui-editable", (event3) => {
  ...
  click(recid, event2) {
    ...
    if (rec?.w2ui?.selectable === false && (rec?.w2ui?.children?.length ?? 0) > 0) {
      if (!query10(event2.target).hasClass("w2ui-show-children")) {
        this.toggle(recid);
        return;
      }
    }
    ...
    if (event2.shiftKey && sel.length > 0 && this.multiSelect) {
      ...
      let fselect = false;
      if (query10(event2.target).closest("td").hasClass("w2ui-col-select")) fselect = true;
  ...
  columnContextMenu(field, event2) {
    ...
    w2menu3.show({
      ...
      query10("#w2overlay-context-menu .w2ui-grid-skip").off(".w2ui-grid").on("click.w2ui-grid", (evt) => {
        evt.stopPropagation();
      }).on("keypress", (evt) => {
        if (evt.keyCode == 13) {
  ...
  columnAutoSize(colIndex) {
    ...
    const el = query10(`#grid_${this.name}_column_${colIndex} .w2ui-col-header`)[0];
    ...
    query10(this.box).find(`.w2ui-grid-records td[col="${colIndex}"] > div`, this.box).each((el2) => {
      const htmlEl = el2;
  ...
  focus(event2) {
    ...
    if (edata.isCancelled === true) return false;
    this.hasFocus = true;
    query10(this.box).removeClass("w2ui-inactive").find(".w2ui-inactive").removeClass("w2ui-inactive");
    setTimeout(() => {
  ...
  blur(event2) {
    ...
    if (edata.isCancelled === true) return false;
    this.hasFocus = false;
    query10(this.box).addClass("w2ui-inactive").find(".w2ui-selected").addClass("w2ui-inactive");
    query10(this.box).find(".w2ui-selection").addClass("w2ui-inactive");
    edata.finish();
  }
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  keydown(event2) {
    ...
    if (edata.isCancelled === true) return;
    if (query10(this.box).find(".w2ui-message").length > 0) {
    ...
    const recEL = query10(obj.box).find(`#grid_${obj.name}_rec_${ind >= 0 ? w2utils.escapeId(obj.records[ind].recid) : "none"}`);
  ...
  expand(recid, noRefresh) {
    ...
    if (Array.isArray(children)) {
      ...
      query10(this.box).find("#grid_" + this.name + "_rec_" + id).attr("expanded", "yes").addClass("w2ui-expanded");
      query10(this.box).find("#grid_" + this.name + "_frec_" + id).attr("expanded", "yes").addClass("w2ui-expanded");
  ...
  collapse(recid, noRefresh) {
    ...
    if (Array.isArray(children)) {
      ...
      if (edata.isCancelled === true) return false;
      query10(this.box).find("#grid_" + this.name + "_rec_" + id).removeAttr("expanded").removeClass("w2ui-expanded");
      query10(this.box).find("#grid_" + this.name + "_frec_" + id).removeAttr("expanded").removeClass("w2ui-expanded");
      query10(this.box).find("#grid_" + this.name + "_cell_" + this.get(recid, true) + "_expand div").html("+");
  ...
  refreshCell(recid, field) {
    ...
    const cell = query10(this.box).find(`${isSummary ? ".w2ui-grid-summary " : ""}#grid_${this.name}_data_${index}_${col_ind}`);
  ...
  refreshRow(recid, ind = null) {
    let tr1 = query10(this.box).find("#grid_" + this.name + "_frec_" + w2utils.escapeId(recid));
    let tr2 = query10(this.box).find("#grid_" + this.name + "_rec_" + w2utils.escapeId(recid));
    if (tr1.length > 0) {
      ...
      if (typeof st == "string") {
        tr1 = query10(this.box).find("#grid_" + this.name + "_frec_" + w2utils.escapeId(recid));
        tr2 = query10(this.box).find("#grid_" + this.name + "_rec_" + w2utils.escapeId(recid));
  ...
  refresh() {
    ...
    if (edata.isCancelled === true) return;
    if (this.show.header) {
      query10(this.box).find(`#grid_${this.name}_header`).html(w2utils.lang(this.header) + "&#160;").show();
    } else {
    ...
    if (this.markSearch) {
      setTimeout(() => {
        ...
        if (search.length > 0) {
          search.forEach((item) => {
            const el = query10(this.box).find('td[col="' + item.col + '"]:not(.w2ui-head)');
  ...
  refreshSearch() {
    if (this.multiSearch && this.searchData.length > 0) {
      if (query10(this.box).find(".w2ui-grid-searches").length == 0) {
        query10(this.box).find(".w2ui-grid-toolbar").css("height", this.last.toolbar_height + 35 + "px").append(`<div id="grid_${this.name}_searches" class="w2ui-grid-searches"></div>`);
      }
      let searches = `
      ...
      query10(this.box).find(`#grid_${this.name}_search_logic`).html(w2utils.lang(this.last.logic == "AND" ? "All" : "Any"));
    } else {
      query10(this.box).find(".w2ui-grid-toolbar").css("height", this.last.toolbar_height + "px").find(".w2ui-grid-searches").remove();
    }
    if (this["searchSelected"]) {
      query10(this.box).find(`#grid_${this.name}_search_all`).val(" ").prop("readOnly", true);
      query10(this.box).find(`#grid_${this.name}_search_name`).show().find(".name-text").html(this["searchSelected"].text);
    ...
    w2utils.bindEvents(query10(this.box).find(`#grid_${this.name}_searches .w2ui-action, #grid_${this.name}_searches button`), this);
  }
  refreshBody() {
    this.updateExpanded();
    this.scroll();
    ...
    if (this.selectType == "row") {
      records.on("mouseover mouseout", { delegate: "tr" }, (event2) => {
        ...
        query10(this.box).find(`#grid_${this.name}_frec_${w2utils.escapeId(recid)}`).toggleClass("w2ui-record-hover", event2.type == "mouseover");
      });
      frecords.on("mouseover mouseout", { delegate: "tr" }, (event2) => {
        const ind = query10(event2.delegate).attr("index");
        const recid = this.records[ind]?.recid;
        query10(this.box).find(`#grid_${this.name}_rec_${w2utils.escapeId(recid)}`).toggleClass("w2ui-record-hover", event2.type == "mouseover");
      });
    }
    if (w2utils.isMobile) {
    ...
    query10(this.box).find(".w2ui-grid-body").off(".body-global").on("click.body-global dblclick.body-global contextmenu.body-global", { delegate: "td.w2ui-head" }, (event2) => {
      ...
      if (["mouseenter", "mouseover"].includes(col?.["info"]?.showOn?.toLowerCase()) && event2.type == "mouseover") {
        this.showBubble(parseInt(tr.attr("index")), parseInt(td.attr("col")), isSummary).then(() => {
          query10(event2.delegate).off(".tooltip").on("mouseleave.tooltip", () => {
            w2tooltip3.hide(this.name + "-bubble");
          });
        });
      ...
      query10(event2.delegate).off(".tooltip").on("mouseleave.tooltip", (_evt) => {
        ...
        evt.stopPropagation();
        w2tooltip3.update(this.name + "-bubble", w2utils.lang("Copied"));
    ...
    if (this.records.length === 0 && this.msgEmpty) {
      query10(this.box).find(`#grid_${this.name}_body`).append(`<div id="grid_${this.name}_empty_msg" class="w2ui-grid-empty-msg"><div>${w2utils.lang(this.msgEmpty)}</div></div>`);
  ...
  render(box) {
    ...
    if (this.selectType != "row") query10(this.box).addClass("w2ui-ss");
    ...
    function mouseStart(event2) {
      ...
      if (obj.last.userSelect == "text") {
        obj.last.userSelect = "";
        query10(obj.box).find(".w2ui-grid-body").css("user-select", "none");
      }
      if (obj.selectType == "row" && (query10(event2.target).parents().hasClass("w2ui-head") || query10(event2.target).hasClass("w2ui-head"))) return;
      if (obj.last.move && obj.last.move.type == "expand") return;
      if (event2.altKey) {
        query10(obj.box).find(".w2ui-grid-body").css("user-select", "text");
        obj.selectNone();
        ...
        if (obj.last.move) {
          ...
          if (query10(target).hasClass("w2ui-grid-footer") || query10(target).parents("div.w2ui-grid-footer").length > 0) {
      ...
      if (obj.reorderRows == true) {
        ...
        if (query10(el).hasClass("w2ui-col-number") || query10(el).hasClass("w2ui-col-order")) {
          ...
          const eColor = query10(obj.box).find(".w2ui-even.w2ui-empty-record").css("background-color");
          const oColor = query10(obj.box).find(".w2ui-odd.w2ui-empty-record").css("background-color");
          query10(obj.box).find(".w2ui-even td").filter(":not(.w2ui-col-number)").css("background-color", eColor);
          query10(obj.box).find(".w2ui-odd td").filter(":not(.w2ui-col-number)").css("background-color", oColor);
          const mv = obj.last.move;
          const recs = query10(obj.box).find(".w2ui-grid-records");
          if (!mv["ghost"]) {
      ...
      query10(document).on("mousemove.w2ui-" + obj.name, mouseMove).on("mouseup.w2ui-" + obj.name, mouseStop);
      event2.stopPropagation();
    }
    function mouseMove(event2) {
      ...
      if (recid == null) {
        ...
        if (isNaN(col)) {
          ...
          query10(obj.box).find(".w2ui-grid-columns .w2ui-col-header, .w2ui-grid-fcolumns .w2ui-col-header").removeClass("w2ui-col-selected");
          query10(obj.box).find(".w2ui-col-number").removeClass("w2ui-row-selected");
          delete mv.colRange;
        } else {
    ...
    function mouseStop(event2) {
      ...
      if (query10(event2.target).parents().hasClass(".w2ui-head") || query10(event2.target).hasClass(".w2ui-head")) return;
      obj.removeRange("selection-preview");
      if (mv && ["select", "select-column"].includes(mv.type)) {
        ...
        if (obj.reorderRows == true && obj.last.move.reorder) {
          if (mv.to != null) {
            ...
            obj.sortData = [];
            query10(obj.box).find(`#grid_${obj.name}_columns .w2ui-col-header`).removeClass("w2ui-col-sorted");
            resetRowReorder();
            obj.selectNone(true);
            obj.select(mv.from);
            edata2.finish();
      ...
      delete obj.last.move;
      query10(document).off(".w2ui-" + obj.name);
    }
  ...
  initColumnDrag(_box) {
    ...
    const hasInvalidClass = (target, lastColumn) => {
      ...
      for (let i = 0; i < iClass.length; i++) {
        if (query10(target).closest(".w2ui-head").hasClass(iClass[i])) {
          return true;
        }
      }
      return false;
    ...
    function dragColStart(event2) {
      ...
      if (!query10(event2.target).parents().hasClass("w2ui-head") || hasInvalidClass(event2.target)) return;
      ...
      const origColumn = dragData.columnHead = query10(event2.target).closest(".w2ui-head");
      ...
      if (edata.isCancelled === true) return false;
      const columns = dragData["columns"] = query10(self.box).find(".w2ui-head:not(.w2ui-head-last)");
    ...
    function dragColEnd(event2) {
      ...
      let target;
      const finish = () => {
        const ghosts = query10(self.box).find(".w2ui-grid-ghost");
        query10(self.box).find(".w2ui-intersection-marker").hide();
    ...
    function markIntersection(event2) {
      ...
      if (dragData.targetPos != newPos) {
        const rect1 = query10(self.box).find(".w2ui-grid-body").get(0).getBoundingClientRect();
        const rect2 = query10(event2.target).closest("td").get(0).getBoundingClientRect();
        query10(self.box).find(".w2ui-intersection-marker").show().css({
    ...
    if (this.show.toolbarSearch) {
      ...
      this.toolbar.items.push({
        ...
        onRefresh: async (event2) => {
          await event2.complete;
          const input = query10(this.box).find(`#grid_${this.name}_search_all`);
          w2utils.bindEvents(query10(this.box).find(`#grid_${this.name}_search_all, .w2ui-action`), this);
      ...
      switch (event2.detail.item.id) {
        ...
        case "w2ui-column-on-off":
          if (event2.detail.subItem) {
            ...
            setTimeout(() => {
              query10(`#w2overlay-${this.name}_toolbar-drop .w2ui-grid-skip`).off(".w2ui-grid").on("click.w2ui-grid", (evt) => {
                evt.stopPropagation();
              }).on("keypress", (evt) => {
                if (evt.keyCode == 13) {
                  this.skip(evt.target.value);
  ...
  initResize() {
    const obj = this;
    query10(this.box).find(".w2ui-resizer").off(".grid-col-resize").on("click.grid-col-resize", function(event2) {
      event2.stopPropagation();
      event2.preventDefault();
  ...
  resizeRecords() {
    const obj = this;
    query10(this.box).find(".w2ui-empty-record").remove();
    const box = query10(this.box);
    const grid = query10(this.box).find(":scope > div.w2ui-grid-box");
    const header = query10(this.box).find(`#grid_${this.name}_header`);
    const toolbar = query10(this.box).find(`#grid_${this.name}_toolbar`);
    const summary = query10(this.box).find(`#grid_${this.name}_summary`);
    const fsummary = query10(this.box).find(`#grid_${this.name}_fsummary`);
    ...
    if (body[0]?.clientHeight - (columns[0]?.clientHeight ?? 0) < (query10(records).find(":scope > table")[0]?.clientHeight ?? 0) + (bodyOverflowX ? w2utils.scrollBarSize() : 0)) {
      bodyOverflowY = true;
    }
    if (!this.fixedBody) {
      const bodyHeight = w2utils.getSize(columns, "height") + w2utils.getSize(query10(this.box).find("#grid_" + this.name + "_records table"), "height") + (bodyOverflowX ? w2utils.scrollBarSize() : 0);
    ...
    columns.find(":scope > table > tbody > tr:nth-child(1) td").add(fcolumns.find(":scope > table > tbody > tr:nth-child(1) td")).each((el) => {
      if (query10(el).hasClass("w2ui-col-number")) {
      ...
      if (query10(el).hasClass("w2ui-head-last")) {
        if ((obj.last.vscroll.colIndEnd ?? 0) + 1 < obj.columns.length) {
          ...
          query10(el).css("width", w2utils.scrollBarSize() + (width_diff > 0 && percent === 0 ? width_diff : 0) + "px");
        }
      }
    });
    ...
    records.find(":scope > table > tbody > tr:nth-child(1) td").add(frecords.find(":scope > table > tbody > tr:nth-child(1) td")).each((el) => {
      ...
      if (query10(el).hasClass("w2ui-grid-data-last") && query10(el).parents(".w2ui-grid-frecords").length === 0) {
        if ((obj.last.vscroll.colIndEnd ?? 0) + 1 < obj.columns.length) {
          let width = 0;
          for (let i = (obj.last.vscroll.colIndEnd ?? 0) + 1; i < obj.columns.length; i++) {
            if (!obj.columns[i] || obj.columns[i].frozen || obj.columns[i].hidden) continue;
...
`;
  ...
  columnTooltipHide(_ind, _event) {
    w2tooltip3.hide(this.name + "-column-tooltip");
  }
  getRecordsHTML() {
    let buffered = this.records.length;
    const url = this.url?.get ?? this.url;
  ...
  scroll(event2) {
    ...
    query10(this.box).find("#grid_" + this.name + "_footer .w2ui-footer-right").html(
    ...
    if (e + 10 > buffered && this.last.vscroll.pull_more !== true && (buffered < this.total - this.offset || this.total == -1 && this.last.fetch.hasMore)) {
      ...
      more.show().eq(1).off(".load-more").on("click.load-more", function() {
        query10(this).find("td").html('<div><div style="width: 20px; height: 20px;" class="w2ui-spinner"></div></div>');
    ...
    function markSearch() {
      ...
      obj.last.marker_timer = setTimeout(() => {
        ...
        if (search.length > 0) {
          search.forEach((item) => {
            const el = query10(obj.box).find('td[col="' + item.col + '"]:not(.w2ui-head)');
            w2utils.marker(el, item.search);
          });
        }
      }, 50);
  ...
  showBubble(ind, col_ind, summary) {
    ...
    let html = "";
    const rec = this.records[ind];
    const el = query10(this.box).find(`${summary ? ".w2ui-grid-summary" : ""} #grid_${this.name}_data_${ind}_${col_ind} .w2ui-info`);
    if (this.last.bubbleEl) {
      w2tooltip3.hide(this.name + "-bubble");
    }
    ...
    return w2tooltip3.show(w2utils.extend({
      name: this.name + "-bubble",
      html,
      anchor: el.get(0),
      position: "top|bottom",
  ...
  status(msg) {
    if (msg != null) {
      query10(this.box).find(`#grid_${this.name}_footer`).find(".w2ui-footer-left").html(msg);
    } else {
      let msgLeft = "";
      const sel = this.getSelection();
      if (sel.length > 0) {
      ...
      query10(this.box).find("#grid_" + this.name + "_footer .w2ui-footer-left").html(msgLeft);
    }
  }
  lock(msg, showSpinner) {
  ...
  unlock(speed) {
    setTimeout(() => {
      if (query10(this.box).find(".w2ui-message").hasClass("w2ui-closing")) return;
      w2utils.unlock(this.box, speed);
    }, 25);
  }
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
var w2form = class extends w2base {
  ...
  getFieldValue(name) {
    ...
    if (["radio"].includes(field.type)) {
      const selected2 = query11(el).closest(".w2ui-field-group").find("input:checked").get(0);
      if (selected2) {
    ...
    if (["check", "checks"].includes(field.type)) {
      current = [];
      const selected2 = query11(el).closest(".w2ui-field-group").find("input:checked");
    ...
    if (["map", "array"].includes(field.type)) {
      ...
      field.$el.parent().find(".w2ui-map-field").each((div, _ind) => {
        const key = query11(div).find(".w2ui-map.key").val();
        const value = query11(div).find(".w2ui-map.value").val();
  ...
  setFieldValue(name, value) {
    ...
    switch (field.type) {
      ...
      case "radio": {
        ...
        const inputs = query11(el).closest(".w2ui-field-group").find("input");
      ...
      case "checks": {
        ...
        const inputs = query11(el).closest("div.w2ui-field-group").find("input");
        const items = field.options.items;
        items.forEach((it, ind) => {
          const input = inputs.filter(`[data-index="${ind}"]`);
      ...
      case "array": {
        ...
        const container = query11(field.el).parent().find(".w2ui-map-container");
  ...
  updateEmptyGroups() {
    query11(this.box).find(".w2ui-group").each((node) => {
      const group = node;
      if (isHidden(query11(group).find(".w2ui-field"))) {
        query11(group).hide();
      } else {
        query11(group).show();
      }
  ...
  toggleGroup(groupName, show) {
    const el = query11(this.box).find('.w2ui-group-title[data-group="' + w2utils.base64encode(groupName) + '"]');
  ...
  showErrors() {
    ...
    query11(errors[0].field.$el).parents(".w2ui-field").get(0).scrollIntoView({ block: "nearest", inline: "nearest" });
    errors.forEach((error) => {
      ...
      w2tooltip4.show(w2utils.extend({
        anchor,
    ...
    query11(errors[0].field.$el).parents(".w2ui-page").off(".hideErrors").on("scroll.hideErrors", (_evt) => {
  ...
  hideErrors() {
    ...
    this.fields.forEach((field) => {
      w2tooltip4.hide(`${this.name}-${field.field}-error`);
    });
  }
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  getChanges() {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  getAction(action) {
    const ret = query11(this.box).find('.w2ui-buttons button[name="' + action + '"]');
    if (ret.length === 0) {
  ...
  resize() {
    ...
    if (this.box != null) {
      ...
      const header = query11(this.box).find(":scope > div .w2ui-form-header");
      const toolbar = query11(this.box).find(":scope > div .w2ui-form-toolbar");
      const tabs = query11(this.box).find(":scope > div .w2ui-form-tabs");
      const page = query11(this.box).find(":scope > div .w2ui-page");
      const dpage = query11(this.box).find(":scope > div .w2ui-page.page-" + this.page + " > div");
      const buttons = query11(this.box).find(":scope > div .w2ui-buttons");
  ...
  refresh(...args) {
    ...
    if (args.length > 0) {
      ...
      query11(this.box).find("input, textarea, select").each((node) => {
        ...
        const field = this.get(name);
        if (field) {
          const div = query11(el).closest(".w2ui-page");
          if (div.length > 0) {
            for (let i = 0; i < 100; i++) {
      ...
      query11(this.box).find(".w2ui-page").hide();
      query11(this.box).find(".w2ui-page.page-" + this.page).show();
      query11(this.box).find(".w2ui-form-header").html(w2utils.lang(this.header));
    ...
    for (let f = 0; f < fields.length; f++) {
      ...
      field.$el.off(".w2form").on("change.w2form", function(event2) {
        ...
        if (["enum", "file"].includes(field.type)) {
          ...
          query11(helper).removeClass("w2ui-error");
        }
      ...
      if (field.hidden) {
        query11(tmp).closest(".w2ui-field").hide();
      } else {
        query11(tmp).closest(".w2ui-field").show();
      }
    }
    ...
    for (let f = 0; f < fields.length; f++) {
      ...
      if (field.type == "switch") {
        ...
        field.$el.off(".form-input").on("focus.form-input", (event2) => {
          ...
          query11(field.toolbar.box).addClass("w2ui-tb-focus");
        }).on("blur.form-input", (event2) => {
          query11(event2.target).removeProp("_index");
          query11(`#${field.name}-tb .w2ui-tb-button`).removeClass("over");
          query11(field.toolbar.box).removeClass("w2ui-tb-focus");
        }).on("keydown.form-input", (event2) => {
          let ind = query11(event2.target).prop("_index");
          switch (event2.key) {
            case "ArrowLeft": {
              if (ind > 0) ind--;
              query11(`#${field.name}-tb .w2ui-tb-button`).removeClass("over").eq(ind).addClass("over");
              query11(event2.target).prop("_index", ind);
              break;
            }
            case "ArrowRight": {
        ...
        (function(obj, field2) {
          ...
          field2.el.mapRefresh = function(map, div) {
            ...
            query11(container).off(".mapChange").on("mouseup.mapChange", { delegate: "input, textarea" }, function(event2) {
              ...
              const kbdEvent = event2;
              const $div = query11(kbdEvent.target).closest(".w2ui-map-field");
              ...
              const fld = query11(event2.target).closest("div.w2ui-map-field");
              ...
              const $cnt = query11(event2.target).closest(".w2ui-map-container");
              if (typeof field2.html?.render == "function") {
              ...
              let index;
              let className = "";
              const cnt2 = query11(event2.target).closest(".w2ui-map-container");
  ...
  render(box) {
    ...
    query11(this.box).attr("name", this.name).addClass("w2ui-reset w2ui-form").html(html);
    if (query11(this.box).length > 0) query11(this.box).get(0).style.cssText += this.style;
    w2utils.bindEvents(query11(this.box).find(".w2ui-eaction"), this);
  ...
  setFocus(focus) {
    ...
    if (w2utils.isInt(focus)) {
      ...
      const inputs = query11(this.box).find("div:not(.w2ui-field-helper) > input, select, textarea, div > label:nth-child(1) > [type=radio]").filter(":not(.file-input)");
