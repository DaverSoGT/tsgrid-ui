...
// src/tslayout.ts
var tslayout_exports = {};
__export(tslayout_exports, {
  TsLayout: () => TsLayout
});
module.exports = __toCommonJS(tslayout_exports);
...
var TsLayout = class extends TsBase {
  ...
  html(panel, data, transition2) {
    ...
    if (panel == "css") {
      query11(this.box).find("#layout_" + this.name + "_panel_css").html("<style>" + data + "</style>");
      promise.status = true;
      return promise;
    }
    if (p == null) {
    ...
    const current = query11(this.box).find(pname + '> [data-role="panel-content"]');
    let panelTop = 0;
    if (current.length > 0) {
      ;
      query11(this.box).find(pname).get(0).scrollTop = 0;
      panelTop = query11(current).css("top");
    }
    ...
    if (p.html === "") {
      ...
      if (!p.hidden) {
        if (transition2 != null && transition2 !== "") {
          query11(this.box).addClass("animating");
          const div1 = query11(this.box).find(pname + '> [data-role="panel-content"]');
          div1.after('<div class="tsg-panel-content new-panel" data-role="panel-content" style="' + div1[0].style.cssText + '"></div>');
          const div2 = query11(this.box).find(pname + '> [data-role="panel-content"].new-panel');
          ...
          switch (transition2) {
            case "slide-left":
              style1 = "left: -" + TsUtils.getSize(query11(this.box), "width") + "px";
              style2 = "left: 0px";
              break;
            case "slide-right":
              style1 = "left: " + TsUtils.getSize(query11(this.box), "width") + "px";
              ...
              break;
            case "slide-down":
              style1 = "top: -" + TsUtils.getSize(query11(this.box), "height") + "px";
          ...
          setTimeout(() => {
            query11(this.box).removeClass("animating");
            div1.remove();
            div2.removeClass("new-panel current");
            query11(this.box).find(pname + '> [data-role="panel-content"]').css({ "cssText": "" });
            edata.finish();
          }, 500);
        } else {
          this.refresh(panel);
  ...
  message(panel, options) {
    const p = this.get(panel);
    const box = query11(this.box).find("#layout_" + this.name + "_panel_" + p.type);
  ...
  sizeTo(panel, size, instant) {
    ...
    query11(this.box).find(":scope > div > .tsg-panel").css("transition", instant !== true ? ".2s" : "0s");
    setTimeout(() => {
      this.set(panel, { size });
    }, 1);
    setTimeout(() => {
      query11(this.box).find(":scope > div > .tsg-panel").css("transition", "0s");
      this.resize();
    }, 300);
    return true;
  }
  show(panel, immediate) {
    ...
    p.hidden = false;
    if (immediate === true) {
      query11(this.box).find("#layout_" + this.name + "_panel_" + panel).css({ "opacity": "1" });
      edata.finish();
      this.resize();
    } else {
      query11(this.box).addClass("animating");
      query11(this.box).find("#layout_" + this.name + "_panel_" + panel).css({ "opacity": "0" });
      query11(this.box).find(":scope > div > .tsg-panel").css("transition", ".2s");
      setTimeout(() => {
        this.resize();
      }, 1);
      setTimeout(() => {
  ...
  el(panel) {
    const el = query11(this.box).find("#layout_" + this.name + "_panel_" + panel + '> [data-role="panel-content"]');
  ...
  hideToolbar(panel) {
    ...
    query11(this.box).find(`#layout_${this.name}_panel_${panel} > [data-role="panel-toolbar"]`).hide();
    this.resize();
  }
  showToolbar(panel) {
    const pan = this.get(panel);
    if (!pan) return;
    pan.show.toolbar = true;
    query11(this.box).find(`#layout_${this.name}_panel_${panel} > [data-role="panel-toolbar"]`).show();
    this.resize();
  }
  toggleToolbar(panel) {
    const pan = this.get(panel);
  ...
  assignToolbar(panel, toolbar) {
    ...
    const pan = this.get(panel);
    pan.toolbar = toolbar;
    const tmp = query11(this.box).find(panel + '> [data-role="panel-toolbar"]');
    if (pan.toolbar != null) {
  ...
  hideTabs(panel) {
    ...
    if (!pan) return;
    pan.show.tabs = false;
    query11(this.box).find("#layout_" + this.name + "_panel_" + panel + '> [data-role="panel-tabs"]').hide();
    this.resize();
  }
  showTabs(panel) {
    const pan = this.get(panel);
    if (!pan) return;
    pan.show.tabs = true;
    query11(this.box).find("#layout_" + this.name + "_panel_" + panel + '> [data-role="panel-tabs"]').show();
    this.resize();
  }
  toggleTabs(panel) {
    const pan = this.get(panel);
  ...
  assignTabs(panel, tabs) {
    ...
    const pan = this.get(panel);
    pan.tabs = tabs;
    const tmp = query11(this.box).find(panel + '> [data-role="panel-tabs"]');
    if (pan.tabs != null) {
  ...
  render(box) {
    ...
    if (!this.box) return false;
    query11(this.box).attr("name", this.name).addClass("tsg-layout").html("<div></div>");
    ...
    for (let p1 = 0; p1 < w2panels.length; p1++) {
      ...
      query11(this.box).find(":scope > div").append(html);
    }
    query11(this.box).find(":scope > div").append('<div id="layout_' + this.name + '_panel_css" style="position: absolute; top: 10000px;"></div>');
    this.refresh();
    this.last["observeResize"] = new ResizeObserver(() => {
      this.resize();
    });
    ...
    function resizeStart(type, evnt) {
      ...
      query11(document).off("mousemove", self.last["events"].mouseMove).on("mousemove", self.last["events"].mouseMove);
      query11(document).off("mouseup", self.last["events"].mouseUp).on("mouseup", self.last["events"].mouseUp);
      self.last["resize"] = {
        type,
        x: evnt.screenX,
        y: evnt.screenY,
      ...
      w2panels.forEach((panel) => {
        const $tmp = query11(self.el(panel)).find(".tsg-lock");
        if ($tmp.length > 0) {
          $tmp.data("locked", "yes");
        } else {
      ...
      const el = query11(self.box).find("#layout_" + self.name + "_resizer_" + type).get(0);
    ...
    function mouseUp(evnt) {
      if (!self.box) return;
      if (!evnt) evnt = window.event;
      query11(document).off("mousemove", self.last["events"].mouseMove);
      query11(document).off("mouseup", self.last["events"].mouseUp);
      ...
      if (self.last["diff_x"] !== 0 || self.last["resize"].diff_y !== 0) {
        ...
        const width = TsUtils.getSize(query11(self.box), "width");
        const height = TsUtils.getSize(query11(self.box), "height");
      ...
      query11(self.box).find("#layout_" + self.name + "_resizer_" + self.last["resize"].type).removeClass(null).addClass("active");
      query11(self.box).find("#layout_" + self.name + "_resizer_" + self.last["resize"].type).removeClass("active");
    ...
    function mouseMove(evnt) {
      ...
      if (edata2.isCancelled === true) return;
      const p = query11(self.box).find("#layout_" + self.name + "_resizer_" + tmp.type);
  ...
  destroy() {
    ...
    if (query11(this.box).find("#layout_" + this.name + "_panel_main").length > 0) {
      this.unmount();
    }
    delete _TsUiRegistry()[this.name];
    edata.finish();
    if (this.last["events"] && this.last["events"].resize) {
      query11(window).off("resize", this.last["events"].resize);
    }
    return true;
  }
  refresh(panel) {
    ...
    if (typeof panel == "string") {
      ...
      query11(self.box).find(pname).css({ display: p.hidden ? "none" : "block" });
      if (p.resizable) {
        query11(self.box).find(rname).show();
      } else {
        query11(self.box).find(rname).hide();
      }
      if (typeof p.html == "object" && typeof p.html.render === "function") {
        ;
        p.html.box = query11(self.box).find(pname + '> [data-role="panel-content"]')[0];
        setTimeout(() => {
          if (query11(self.box).find(pname + '> [data-role="panel-content"]').length > 0) {
            const $content = query11(self.box).find(pname + '> [data-role="panel-content"]').removeClass(null).removeAttr("name").addClass("tsg-panel-content");
      ...
      let tmp = query11(self.box).find(pname + '> [data-role="panel-tabs"]');
      if (p.show.tabs) {
      ...
      tmp = query11(self.box).find(pname + '> [data-role="panel-toolbar"]');
      if (p.show.toolbar) {
      ...
      tmp = query11(self.box).find(pname + "> .tsg-panel-title");
      if (p.title) {
        ;
      ...
      if (query11(self.box).find("#layout_" + self.name + "_panel_main").length === 0) {
        self.render();
        return;
      }
      self.resize();
  ...
  resize() {
    ...
    const width = TsUtils.getSize(query11(this.box), "width");
    const height = TsUtils.getSize(query11(this.box), "height");
    const self = this;
    ...
    if (ptop != null && ptop.hidden !== true) {
      ...
      query11(this.box).find("#layout_" + this.name + "_panel_top").css({
        "display": "block",
        "left": l + "px",
        "top": t + "px",
        "width": w + "px",
      ...
      if (ptop.resizable) {
        ...
        query11(this.box).find("#layout_" + this.name + "_resizer_top").css({
          "display": "block",
          "left": l + "px",
          "top": t + "px",
          "width": w + "px",
      ...
      query11(this.box).find("#layout_" + this.name + "_panel_top").hide();
      query11(this.box).find("#layout_" + this.name + "_resizer_top").hide();
    }
    if (pleft != null && pleft.hidden !== true) {
      ...
      query11(this.box).find("#layout_" + this.name + "_panel_left").css({
        "display": "block",
        "left": l + "px",
        "top": t + "px",
        "width": w + "px",
      ...
      if (pleft.resizable) {
        ...
        query11(this.box).find("#layout_" + this.name + "_resizer_left").css({
          "display": "block",
          "left": l + "px",
          "top": t + "px",
          "width": w + "px",
      ...
      query11(this.box).find("#layout_" + this.name + "_panel_left").hide();
      query11(this.box).find("#layout_" + this.name + "_resizer_left").hide();
    }
    if (pright != null && pright.hidden !== true) {
      ...
      h = height - (stop ? ptop.sizeCalculated + this.padding : 0) - (sbottom ? pbottom.sizeCalculated + this.padding : 0);
      query11(this.box).find("#layout_" + this.name + "_panel_right").css({
        "display": "block",
        "left": l + "px",
        "top": t + "px",
        "width": w + "px",
      ...
      if (pright.resizable) {
        l = l - this.padding;
        w = this.resizer > this.padding ? this.resizer : this.padding;
        query11(this.box).find("#layout_" + this.name + "_resizer_right").css({
          "display": "block",
          "left": l + "px",
          "top": t + "px",
          "width": w + "px",
      ...
      query11(this.box).find("#layout_" + this.name + "_panel_right").hide();
      query11(this.box).find("#layout_" + this.name + "_resizer_right").hide();
    }
    if (pbottom != null && pbottom.hidden !== true) {
      ...
      query11(this.box).find("#layout_" + this.name + "_panel_bottom").css({
        "display": "block",
        "left": l + "px",
        "top": t + "px",
        "width": w + "px",
      ...
      if (pbottom.resizable) {
        ...
        h = this.resizer > this.padding ? this.resizer : this.padding;
        query11(this.box).find("#layout_" + this.name + "_resizer_bottom").css({
          "display": "block",
          "left": l + "px",
          "top": t + "px",
          "width": w + "px",
      ...
      query11(this.box).find("#layout_" + this.name + "_panel_bottom").hide();
      query11(this.box).find("#layout_" + this.name + "_resizer_bottom").hide();
    }
    ...
    query11(this.box).find("#layout_" + this.name + "_panel_main").css({
      "display": "block",
      "left": l + "px",
      "top": t + "px",
      "width": w + "px",
    ...
    if (pprev != null && pprev.hidden !== true) {
      ...
      query11(this.box).find("#layout_" + this.name + "_panel_preview").css({
        "display": "block",
        "left": l + "px",
        "top": t + "px",
        "width": w + "px",
      ...
      if (pprev.resizable) {
        t = t - (this.padding === 0 ? 0 : this.padding);
        h = this.resizer > this.padding ? this.resizer : this.padding;
        query11(this.box).find("#layout_" + this.name + "_resizer_preview").css({
          "display": "block",
          "left": l + "px",
          "top": t + "px",
          "width": w + "px",
      ...
      query11(this.box).find("#layout_" + this.name + "_panel_preview").hide();
      query11(this.box).find("#layout_" + this.name + "_resizer_preview").hide();
    }
    this.resizeBoxes();
    edata.finish();
    return Date.now() - time;
  ...
  resizeBoxes(panel) {
    ...
    panels.forEach((pname, ind) => {
      ...
      if (pan) {
        if (pan.title) {
          const el = query11(this.box).find(tmp2 + ".tsg-panel-title").css({ top: topHeight + "px", display: "block" });
          topHeight += TsUtils.getSize(el, "height");
        }
        if (pan.show.tabs) {
          const el = query11(this.box).find(tmp2 + '[data-role="panel-tabs"]').css({ top: topHeight + "px", display: "block" });
          topHeight += TsUtils.getSize(el, "height");
        }
        if (pan.show.toolbar) {
          const el = query11(this.box).find(tmp2 + '[data-role="panel-toolbar"]').css({ top: topHeight + "px", display: "block" });
          topHeight += TsUtils.getSize(el, "height");
        }
      }
      query11(this.box).find(tmp2 + '[data-role="panel-content"]').css({
        display: "block",
        top: topHeight + "px"
      });
    });
