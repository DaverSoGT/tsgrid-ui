...
// src/tsgrid.ts
var tsgrid_exports = {};
__export(tsgrid_exports, {
  TsGrid: () => TsGrid
});
module.exports = __toCommonJS(tsgrid_exports);
...
var TsBase = class {
  ...
  unmount() {
    ...
    query2(this.box).off().removeClass(remove2).removeAttr("name").html("");
    this.box = null;
    edata.finish();
  }
};
...
function notify(text, options, deps) {
  return new Promise((resolve) => {
    ...
    if (textStr) {
      ...
      if (opts["actions"]) {
        query4(where).find("#tsg-notify .tsg-notify-link").on("click", (event2) => {
          const value = query4(event2.target).attr("value") ?? "";
          opts["actions"][value]();
          query4(where).find("#tsg-notify").remove();
          resolve();
        });
...
function status(grid, msg) {
  if (msg != null) {
    query8(grid.box).find(`#grid_${grid.name}_footer`).find(".tsg-footer-left").html(msg);
  } else {
    let msgLeft = "";
    ...
    query8(grid.box).find("#grid_" + grid.name + "_footer .tsg-footer-left").html(msgLeft);
  }
}
...
function unlock2(grid, speed) {
  setTimeout(() => {
    if (query8(grid.box).find(".tsg-message").hasClass("tsg-closing")) return;
...
function add(grid, record, first) {
  ...
  if (!url) {
    ...
    if ((grid.last.vscroll.recIndStart ?? 0) <= indEnd && (grid.last.vscroll.recIndEnd ?? 0) >= indStart) {
      ...
      query9(grid.box).find("#grid_" + grid.name + "_footer .tsg-footer-right .tsg-total").html(TsUtils.formatNumber(grid.total));
    }
  } else {
    grid.refresh();
  }
...
function removeRange(grid, ...names) {
  ...
  for (let a = 0; a < names.length; a++) {
    const name = names[a];
    query9(grid.box).find("#grid_" + grid.name + "_" + name).remove();
    query9(grid.box).find("#grid_" + grid.name + "_f" + name).remove();
...
function refreshRanges(grid) {
  ...
  let range;
  const time = Date.now();
  const rec1 = query9(grid.box).find(`#grid_${grid.name}_frecords`);
  const rec2 = query9(grid.box).find(`#grid_${grid.name}_records`);
  for (let i = 0; i < grid.ranges.length; i++) {
    ...
    let td1 = query9(grid.box).find("#grid_" + grid.name + "_rec_" + TsUtils.escapeId(first.recid) + ' td[col="' + first.column + '"]');
    let td2 = query9(grid.box).find("#grid_" + grid.name + "_rec_" + TsUtils.escapeId(last.recid) + ' td[col="' + last.column + '"]');
    let td1f = query9(grid.box).find("#grid_" + grid.name + "_frec_" + TsUtils.escapeId(first.recid) + ' td[col="' + first.column + '"]');
    let td2f = query9(grid.box).find("#grid_" + grid.name + "_frec_" + TsUtils.escapeId(last.recid) + ' td[col="' + last.column + '"]');
    let _lastColumn = last.column;
    if (first.column < grid.last.vscroll.colIndStart && last.column > grid.last.vscroll.colIndStart) {
      td1 = query9(grid.box).find("#grid_" + grid.name + "_rec_" + TsUtils.escapeId(first.recid) + ' td[col="start"]');
    }
    if (first.column < grid.last.vscroll.colIndEnd && last.column > grid.last.vscroll.colIndEnd) {
      td2 = query9(grid.box).find("#grid_" + grid.name + "_rec_" + TsUtils.escapeId(last.recid) + ' td[col="end"]');
      _lastColumn = "end";
    }
    const index_top = parseInt(query9(grid.box).find("#grid_" + grid.name + "_rec_top").next().attr("index"));
    const index_bottom = parseInt(query9(grid.box).find("#grid_" + grid.name + "_rec_bottom").prev().attr("index"));
    const index_ftop = parseInt(query9(grid.box).find("#grid_" + grid.name + "_frec_top").next().attr("index"));
    const index_fbottom = parseInt(query9(grid.box).find("#grid_" + grid.name + "_frec_bottom").prev().attr("index"));
    if (td1.length === 0 && first.index < index_top && last.index > index_top) {
      td1 = query9(grid.box).find("#grid_" + grid.name + "_rec_top").next().find('td[col="' + first.column + '"]');
    }
    if (td2.length === 0 && last.index > index_bottom && first.index < index_bottom) {
      td2 = query9(grid.box).find("#grid_" + grid.name + "_rec_bottom").prev().find('td[col="' + _lastColumn + '"]');
    }
    if (td1f.length === 0 && first.index < index_ftop && last.index > index_ftop) {
      td1f = query9(grid.box).find("#grid_" + grid.name + "_frec_top").next().find('td[col="' + first.column + '"]');
    }
    if (td2f.length === 0 && last.index > index_fbottom && first.index < index_fbottom) {
      td2f = query9(grid.box).find("#grid_" + grid.name + "_frec_bottom").prev().find('td[col="' + last.column + '"]');
    }
    const edit = query9(grid.box).find("#grid_" + grid.name + "_editable");
    ...
    range = query9(grid.box).find("#grid_" + grid.name + "_f" + rg.name);
    if (td1f.length > 0 || td2f.length > 0) {
      ...
      if (td2f.length === 0) {
        td2f = query9(grid.box).find("#grid_" + grid.name + "_frec_" + TsUtils.escapeId(last.recid) + " td:last-child");
        if (td2f.length === 0) td2f = query9(grid.box).find("#grid_" + grid.name + "_frec_bottom td:first-child");
    ...
    range = query9(grid.box).find("#grid_" + grid.name + "_" + rg.name);
    if (td1.length > 0 || td2.length > 0) {
      ...
      if (td1.length === 0) {
        td1 = query9(grid.box).find("#grid_" + grid.name + "_rec_" + TsUtils.escapeId(first.recid) + " td:first-child");
        if (td1.length === 0) td1 = query9(grid.box).find("#grid_" + grid.name + "_rec_top td:first-child");
      }
      if (td2f.length !== 0) {
  ...
  query9(grid.box).find(".tsg-selection-resizer").off(".resizer").on("mousedown.resizer", mouseStart).on("dblclick.resizer", (event2) => {
  ...
  function mouseStart(event2) {
    ...
    query9("body").off(".tsg-" + self.name).on("mousemove.tsg-" + self.name, mouseMove).on("mouseup.tsg-" + self.name, mouseStop);
    event2.preventDefault();
  }
...
function reset(grid, noRefresh) {
  ...
  query9(grid.box).find(`#grid_${grid.name}_records`).prop("scrollTop", 0);
  if (!noRefresh) grid.refresh();
}
...
function select(grid, ...selectArgs) {
  ...
  if (grid.selectType == "row") {
    for (let a = 0; a < args.length; a++) {
      ...
      if (grid.searchData.length !== 0 || index + 1 >= (grid.last.vscroll.recIndStart ?? 0) && index + 1 <= (grid.last.vscroll.recIndEnd ?? 0)) {
        recEl1 = query10(grid.box).find("#grid_" + grid.name + "_frec_" + TsUtils.escapeId(recid));
        recEl2 = query10(grid.box).find("#grid_" + grid.name + "_rec_" + TsUtils.escapeId(recid));
      }
      if (grid.selectType == "row") {
    ...
    for (const recid in new_sel) {
      ...
      if (index + 1 >= (grid.last.vscroll.recIndStart ?? 0) && index + 1 <= (grid.last.vscroll.recIndEnd ?? 0)) {
        recEl1 = query10(grid.box).find("#grid_" + grid.name + "_rec_" + TsUtils.escapeId(recid));
        recEl2 = query10(grid.box).find("#grid_" + grid.name + "_frec_" + TsUtils.escapeId(recid));
      }
    ...
    for (let c = 0; c < col_sel.length; c++) {
      query10(grid.box).find("#grid_" + grid.name + "_column_" + col_sel[c] + " .tsg-col-header").addClass("tsg-col-selected");
    }
  }
  ...
  if (areAllSelected || areAllSearchedSelected) {
    query10(grid.box).find("#grid_" + grid.name + "_check_all").prop("checked", true);
  } else {
    query10(grid.box).find("#grid_" + grid.name + "_check_all").prop("checked", false);
  }
  grid.status();
...
function unselect(grid, ...unselectArgs) {
  ...
  for (let a = 0; a < args.length; a++) {
    ...
    const recEl1 = query10(grid.box).find("#grid_" + grid.name + "_frec_" + TsUtils.escapeId(recid));
    const recEl2 = query10(grid.box).find("#grid_" + grid.name + "_rec_" + TsUtils.escapeId(recid));
    if (grid.selectType == "row") {
      if (sel.indexes.indexOf(index) == -1) continue;
      ...
      query10(grid.box).find(`#grid_${grid.name}_rec_${TsUtils.escapeId(recid)} > td[col="${col}"]`).removeClass("tsg-selected tsg-inactive");
      query10(grid.box).find(`#grid_${grid.name}_frec_${TsUtils.escapeId(recid)} > td[col="${col}"]`).removeClass("tsg-selected tsg-inactive");
      ...
      if (!isColSelected) {
        query10(grid.box).find(`.tsg-grid-columns td[col="${col}"] .tsg-col-header, .tsg-grid-fcolumns td[col="${col}"] .tsg-col-header`).removeClass("tsg-col-selected");
      }
      if (!isRowSelected) {
        query10(grid.box).find("#grid_" + grid.name + "_frec_" + TsUtils.escapeId(recid)).find(".tsg-col-number").removeClass("tsg-row-selected");
      }
      unselected++;
      if (s.length === 0) {
...
function selectAll(grid) {
  ...
  if (grid.selectType == "row") {
    query10(grid.box).find(".tsg-grid-records tr:not(.tsg-empty-record)").addClass("tsg-selected").find(".tsg-col-number").addClass("tsg-row-selected");
    query10(grid.box).find(".tsg-grid-frecords tr:not(.tsg-empty-record)").addClass("tsg-selected").find(".tsg-col-number").addClass("tsg-row-selected");
    query10(grid.box).find("input.tsg-grid-select-check").prop("checked", true);
  } else {
    query10(grid.box).find(".tsg-grid-columns td .tsg-col-header, .tsg-grid-fcolumns td .tsg-col-header").addClass("tsg-col-selected");
    query10(grid.box).find(".tsg-grid-records tr .tsg-col-number").addClass("tsg-row-selected");
    query10(grid.box).find(".tsg-grid-records tr:not(.tsg-empty-record)").find(".tsg-grid-data:not(.tsg-col-select)").addClass("tsg-selected");
    query10(grid.box).find(".tsg-grid-frecords tr .tsg-col-number").addClass("tsg-row-selected");
    query10(grid.box).find(".tsg-grid-frecords tr:not(.tsg-empty-record)").find(".tsg-grid-data:not(.tsg-col-select)").addClass("tsg-selected");
    query10(grid.box).find("input.tsg-grid-select-check").prop("checked", true);
  }
...
function selectNone(grid, skipEvent) {
  ...
  const sel = grid.last.selection;
  if (grid.selectType == "row") {
    query10(grid.box).find(".tsg-grid-records tr.tsg-selected").removeClass("tsg-selected tsg-inactive").find(".tsg-col-number").removeClass("tsg-row-selected");
    query10(grid.box).find(".tsg-grid-frecords tr.tsg-selected").removeClass("tsg-selected tsg-inactive").find(".tsg-col-number").removeClass("tsg-row-selected");
    query10(grid.box).find("input.tsg-grid-select-check").prop("checked", false);
  } else {
    query10(grid.box).find(".tsg-grid-columns td .tsg-col-header, .tsg-grid-fcolumns td .tsg-col-header").removeClass("tsg-col-selected");
    query10(grid.box).find(".tsg-grid-records tr .tsg-col-number").removeClass("tsg-row-selected");
    query10(grid.box).find(".tsg-grid-frecords tr .tsg-col-number").removeClass("tsg-row-selected");
    query10(grid.box).find(".tsg-grid-data.tsg-selected").removeClass("tsg-selected tsg-inactive");
    query10(grid.box).find("input.tsg-grid-select-check").prop("checked", false);
  }
  sel.indexes = [];
  sel.columns = {};
...
var Tooltip = class _Tooltip {
  ...
  attach(anchorArg, textArg) {
    ...
    if (options.autoShowOn) {
      ...
      query11(anchor).off(`.${scope}`).on(`${options.autoShowOn}.${scope}`, (event2) => {
    ...
    if (options.autoHideOn) {
      ...
      query11(anchor).off(`.${scope}`).on(`${options.autoHideOn}.${scope}`, (event2) => {
  ...
  show(name, extraOptions) {
    if (name instanceof HTMLElement || name instanceof Object) {
      ...
      query11(ret.overlay.anchor).off(".autoShow-" + ret.overlay.name).off(".autoHide-" + ret.overlay.name);
      setTimeout(() => {
    ...
    if (options.html === "" || options.html == null) {
      ...
      query11(overlay.box).find(".tsg-overlay-body").attr("style", (options.style || "") + "; " + overlayStyles).removeClass(null).addClass("tsg-overlay-body " + options.class + (options.draggable ? " tsg-draggable" : "")).html(options.html);
      this.resize(overlay.name);
    } else {
      ...
      overlay.box = query11("#" + TsUtils.escapeId(overlay.id))[0];
      overlay.displayed = true;
      const names = query11(overlay.anchor).data("tooltipName") ?? [];
      names.push(name);
      query11(overlay.anchor).data("tooltipName", names);
      TsUtils.bindEvents(overlay.box, {});
      overlay.tmp.originalCSS = "";
      if (query11(overlay.anchor).length > 0) {
        overlay.tmp.originalCSS = query11(overlay.anchor)[0].style.cssText;
      }
      this.resize(overlay.name);
    }
    ...
    if (options.anchorClass) {
      if (!(options.anchorClass == "tsg-focus" && overlay.anchor == document.body)) {
        query11(overlay.anchor).addClass(options.anchorClass);
      }
    }
    ...
    query11(overlay.box).css("opacity", 1).find(".tsg-overlay-body").html(options.html);
    setTimeout(() => {
      query11(overlay.box).css({ "pointer-events": "auto" }).data("ready", "yes");
    }, 100);
    TsUtils.bindEvents(query11(overlay.box).find(".tsg-eaction"), this);
    delete overlay.needsUpdate;
    overlay.box.overlay = overlay;
    query11(overlay.box).off("mousedown.tsg-bringfront").on("mousedown.tsg-bringfront", () => {
    ...
    function addWatchEvents(el) {
      ...
      query11(queryEl).off(`.${scope}`).on(`scroll.${scope}`, (_event) => {
        Object.assign(overlay.tmp, {
    ...
    function addHideEvents() {
      ...
      const $anchor = query11(overlay.anchor);
      const scope = "tooltip-" + overlay.name;
      query11("html").off(`.${scope}`);
      if (options.hideOn.includes("doc-click")) {
        ...
        query11("html").on(`click.${scope}`, hide);
      }
      if (options.hideOn.includes("tooltip-click")) {
        query11(overlay.box).off(`click.${scope}`).on(`click.${scope}`, hide);
      }
      if (options.hideOn.includes("focus-change") || options.hideOn.includes("blur")) {
        query11("html").on(`focusin.${scope}`, (_e) => {
  ...
  hide(name) {
    ...
    if (name instanceof HTMLElement) {
      const names2 = query11(name).data("tooltipName") ?? [];
    ...
    if (names.length == 0) {
      query11(overlay.anchor).removeData("tooltipName");
    } else {
      query11(overlay.anchor).data("tooltipName", names);
    }
    ...
    query11(overlay.anchor).off(`.${scope}`).removeClass(overlay.options.anchorClass);
  ...
  resize(name) {
    ...
    const qBox = query11(overlay.box).css({
      left: pos.left + "px",
      top: pos.top + "px"
    });
    qBox.then((q) => {
  ...
  getPosition(name) {
    ...
    if (overlay.tmp.resizedY || overlay.tmp.resizedX) {
      query11(overlay.box).css({ width: "", height: "", scroll: "auto" });
    }
  ...
  startDrag(event2) {
    ...
    const el = query11(event2.target).closest(".tsg-overlay");
    ...
    query11(document).off(".tsg-drag").on("selectstart.tsg-drag, dragstart.tsg-drag", (e) => e.preventDefault()).find("body").addClass("tsg-overlay-dragging");
    query11("html").off(".TsColor").on("mousemove.TsColor", mouseMove).on("mouseup.TsColor", mouseUp);
    function mouseUp(_event) {
      query11("html").off(".TsColor");
      query11(document).off("selectstart.tsg-drag");
      query11(document).off("dragstart.tsg-drag");
      query11(document.body).removeClass("tsg-overlay-dragging");
      if (initial["moved"]) {
...
var ColorTooltip = class _ColorTooltip extends Tooltip {
  ...
  attach(anchor, text) {
    ...
    overlay.on("show:after.attach", (_event) => {
      if (ret.overlay?.box) {
        const actions = query11(ret.overlay.box).find(".tsg-eaction");
  ...
  select(color, name) {
    ...
    if (typeof name != "string") {
      target = name.target;
      this.index = (query11(target).attr("index") ?? "").split(":").map(Number);
      name = query11(target).closest(".tsg-overlay").attr("name");
    }
    ...
    if (["INPUT", "TEXTAREA"].includes(overlay.anchor.tagName) && overlay.options.updateInput) {
      query11(overlay.anchor).val(color);
    }
    overlay.newColor = color;
    query11(overlay.box).find(".tsg-color.tsg-selected").removeClass("tsg-selected");
    if (target) {
      query11(target).addClass("tsg-selected");
    }
    edata.finish();
  }
  ...
  tabClick(index, name) {
    if (typeof name != "string") {
      name = query11(name.target).closest(".tsg-overlay").attr("name");
    }
    const overlay = this.get(name);
    const tab = query11(overlay.box).find(`.tsg-color-tab:nth-child(${index})`);
    query11(overlay.box).find(".tsg-color-tab").removeClass("tsg-selected");
    query11(tab).addClass("tsg-selected");
    query11(overlay.box).find(".tsg-tab-content").hide().closest(".tsg-colors").find(".tab-" + index).show();
  }
  ...
  initControls(overlay) {
    ...
    query11(overlay.box).off(".TsColor").on("contextmenu.TsColor", (event2) => {
      event2.preventDefault();
    }).find("input").off(".TsColor").on("change.TsColor", (event2) => {
      const el = query11(event2.target);
    ...
    query11(overlay.box).find(".color-original").off(".TsColor").on("click.TsColor", (event2) => {
      const tmp = TsUtils.parseColor(query11(event2.target).css("background-color"));
      if (tmp != null) {
        rgb = tmp;
        hsv = TsUtils.rgb2hsv(rgb);
        setColor(hsv, true);
    ...
    query11(overlay.box).find(".palette, .rainbow, .alpha").off(".TsColor").on(`${mDown}.TsColor`, mouseDown);
    ...
    function setColor(color2, fullUpdate, initial2) {
      ...
      query11(overlay.box).find(".color-preview").css("background-color", "#" + newColor);
      query11(overlay.box).find("input").each((el) => {
        if (el.name) {
      ...
      if (initial2 != null) {
        ...
        query11(overlay.box).find(".color-original").css("background-color", "#" + color3);
        query11(overlay.box).find(".tsg-color.tsg-selected").removeClass("tsg-selected");
        query11(overlay.box).find(`.tsg-colors [name="${color3}"], .tsg-colors [name="${initial2}"]`).addClass("tsg-selected");
    ...
    function updateSliders() {
      const el1 = query11(overlay.box).find(".palette .value1");
      const el2 = query11(overlay.box).find(".rainbow .value2");
      const el3 = query11(overlay.box).find(".alpha .value2");
    ...
    function refreshPalette() {
      ...
      query11(overlay.box).find(".palette").css("background-image", `linear-gradient(90deg, rgba(${rgb2},0) 0%, rgba(${rgb2},1) 100%)`);
    }
    function mouseDown(event2) {
      const el = query11(this).find(".value1, .value2");
      ...
      mouseMove(event2);
      query11("html").off(".TsColor").on(mMove, mouseMove).on(mUp, mouseUp);
    }
    function mouseUp(_event) {
      query11("html").off(".TsColor");
    }
    function mouseMove(event2) {
      ...
      const name = query11(el.get(0).parentNode).attr("name");
  ...
  async pickAndSelect(name, event2) {
    ...
    if (typeof color == "string" && color.substr(0, 1) == "#" && [7, 9].includes(color.length)) {
      ...
      const cnt = query11(event2.target).closest(".tsg-colors-custom");
...
var MenuTooltip = class extends Tooltip {
  ...
  attach(anchor, text) {
    ...
    overlay.on("show:after.attach, update:after.attach", (_event) => {
      if (ret.overlay?.box) {
        ...
        if (["INPUT", "TEXTAREA"].includes(overlay.anchor.tagName)) {
          ...
          query11(overlay.anchor).on("input.search-trigger", () => {
            overlay.tmp._new_search = true;
            query11(overlay.anchor).off("input.search-trigger");
          });
        }
        TsUtils.bindEvents(actions, this);
    ...
    overlay.click = () => {
      query11(overlay.box).find(".tsg-selected").each((el) => {
        el.click();
      });
    };
  ...
  initControls(overlay) {
    ...
    query11(overlay.box).find(".tsg-menu:not(.tsg-sub-menu)").off(".TsMenu").on("contextmenu.TsMenu", (event2) => {
      event2.preventDefault();
    }).on(`${mdown}.TsMenu`, { delegate: ".tsg-menu-item" }, (event2) => {
      const dt = event2.delegate.dataset;
      const parents = query11(event2.delegate).closest(".tsg-menu").data("parents");
      ...
      const _menu = query11(event2.target).closest(".tsg-menu").get(0);
    ...
    if (["INPUT", "TEXTAREA"].includes(overlay.anchor.tagName)) {
      query11(overlay.anchor).off(".TsMenu").on("input.TsMenu", (_event) => {
    ...
    if (overlay.options.search) {
      query11(overlay.box).find("#menu-search").off(".TsMenu").on("keyup.TsMenu", (event2) => {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  openSubMenu(event2) {
    const anchor = query11(event2.originalEvent.target).get(0);
    ...
    query11(event2.target).addClass("expanded");
    TsMenu.show({
      ...
      query11(event2.target).removeClass("expanded");
    });
    setTimeout(() => {
      query11("#w2overlay-" + overlay.name + "-submenu").on("mouseenter", (event3) => {
  ...
  refreshIndex(name, instant) {
    ...
    const view = query11(overlay.box).find(".tsg-overlay-body").get(0);
    const search2 = query11(overlay.box).find(".tsg-menu-search, .tsg-menu-top").get(0);
    query11(overlay.box).find(".tsg-menu-item.tsg-selected").removeClass("tsg-selected");
    const el = query11(overlay.box).find(`.tsg-menu-item[index="${overlay.selected}"]`).addClass("tsg-selected").get(0);
    if (el) {
  ...
  showTooltip(name, options) {
    ...
    const anchor = options?.anchor ?? query11(overlay.box).find(`.tsg-menu-item[index="${overlay.selected}"]`).get(0);
  ...
  refreshSearch(name) {
    ...
    TsTooltip.hide(overlay.name + "-tooltip");
    query11(overlay.box).find(".tsg-no-items").hide();
    query11(overlay.box).find(".tsg-menu-item, .tsg-menu-divider").each((el) => {
    ...
    query11(overlay.box).find(".tsg-sub-menu").each((sub) => {
      const hasItems = query11(sub).find(".tsg-menu-item").get().some((el) => {
        return el.style.display != "none" ? true : false;
      });
      const parent = this.getCurrent(name, sub.dataset?.parent);
      if (parent.item.expanded) {
        if (!hasItems) {
          query11(sub).parent().hide();
        } else {
          query11(sub).parent().show();
        }
      }
    });
    if (overlay.tmp["searchCount"] == 0 || (overlay.options?.items?.length ?? 0) == 0) {
      if (query11(overlay.box).find(".tsg-no-items").length == 0) {
        query11(overlay.box).find(".tsg-menu:not(.tsg-sub-menu)").append(`
                    <div class="tsg-no-items">
                        ${TsUtils.lang(overlay.options.msgNoItems)}
                    </div>`);
      }
      query11(overlay.box).find(".tsg-no-items").show();
    }
  }
  /**
  ...
  applyFilter(name, items, search2, debounce2) {
    ...
    if (items == null && options.url && remote.hasMore && remote.search !== search2) {
      ...
      query11(overlay.box).find(".tsg-no-items").html(msg);
  ...
  menuDown(overlay, event2, index, parents) {
    const options = overlay.options;
    let items = options.items;
    const icon = query11(event2.delegate).find(".tsg-icon");
    const menu = query11(event2.target).closest(".tsg-menu:not(.tsg-sub-menu)");
    ...
    if ((options.type === "check" || options.type === "radio") && item.group !== false && !query11(event2.target).hasClass("menu-remove") && !query11(event2.target).hasClass("menu-help") && !query11(event2.target).closest(".tsg-menu-item").hasClass("has-sub-menu")) {
    ...
    if (!query11(event2.target).hasClass("menu-remove") && !query11(event2.target).hasClass("menu-help")) {
      ...
      if (!query11(event2.delegate).hasClass("has-sub-menu")) {
        query11(event2.delegate).addClass("tsg-selected");
      }
    }
  }
  // any: callback parameter — caller signature varies; TsTooltip overlay options merge from multiple user sources at runtime
  ...
  menuClick(overlay, event2, index, parents) {
    const options = overlay.options;
    let items = options.items;
    const $item = query11(event2.delegate).closest(".tsg-menu-item");
    ...
    const item = items[index];
    if (!item || item.disabled && !query11(event2.target).hasClass("menu-remove")) {
      return;
    }
    let edata;
    ...
    if (query11(event2.target).hasClass("menu-remove")) {
  ...
  keyUp(overlay, event2) {
    ...
    switch (event2.keyCode) {
      ...
      case 13: {
        ...
        event2.delegate = query11(overlay.box).find(".tsg-selected").get(0);
      ...
      case 37: {
        ...
        if (Array.isArray(item?.items) && item.items.length > 0 && item.expanded) {
          event2.delegate = query11(overlay.box).find(`.tsg-menu-item[index="${index}"]`).get(0);
          overlay.selected = index;
          this.menuClick(overlay, event2, parseInt(String(index)), parents);
        }
        filter = false;
...
var DateTooltip = class extends Tooltip {
  ...
  initControls(overlay) {
    ...
    const moveMonth = (inc) => {
      ...
      Object.assign(overlay.tmp, cal);
      query11(overlay.box).find(".tsg-overlay-body").html(cal.html);
      this.initControls(overlay);
    };
    const checkJump = (event2, dblclick) => {
      query11(event2.target).parent().find(".tsg-jump-month, .tsg-jump-year").removeClass("tsg-selected");
      query11(event2.target).addClass("tsg-selected");
      const dt = /* @__PURE__ */ new Date();
    ...
    query11(overlay.box).find(".tsg-cal-title").off(".calendar").on("click.calendar", (event2) => {
      ...
      if (overlay.tmp.jump) {
        ...
        query11(overlay.box).find(".tsg-overlay-body .tsg-cal-days").replace(this.getYearHTML());
        const el = query11(overlay.box).find(`[name="${overlay.tmp.year}"]`).get(0);
    ...
    query11(overlay.box).find(".tsg-cal-now").off(".calendar").on("click.calendar", (_event) => {
    ...
    query11(overlay.box).off(".calendar").on("contextmenu.calendar", (event2) => {
      event2.preventDefault();
    }).on("click.calendar", { delegate: ".tsg-day.tsg-date" }, (event2) => {
      if (options.type == "datetime") {
        overlay.newDate = query11(event2.target).attr("date");
        query11(overlay.box).find(".tsg-overlay-body").html(this.getHourHTML(overlay.options).html);
        this.initControls(overlay);
      } else {
        overlay.newValue = query11(event2.target).attr("date");
        this.hide(overlay.name);
      }
    }).on("click.calendar", { delegate: ".tsg-jump-month" }, (event2) => {
      overlay.tmp.jumpMonth = parseInt(query11(event2.target).attr("name") ?? "0");
      checkJump(event2);
    }).on("dblclick.calendar", { delegate: ".tsg-jump-month" }, (event2) => {
      overlay.tmp.jumpMonth = parseInt(query11(event2.target).attr("name") ?? "0");
      checkJump(event2, true);
    }).on("click.calendar", { delegate: ".tsg-jump-year" }, (event2) => {
      overlay.tmp.jumpYear = parseInt(query11(event2.target).attr("name") ?? "0");
      checkJump(event2);
    }).on("dblclick.calendar", { delegate: ".tsg-jump-year" }, (event2) => {
      overlay.tmp.jumpYear = parseInt(query11(event2.target).attr("name") ?? "0");
      checkJump(event2, true);
    }).on("click.calendar", { delegate: ".tsg-time.hour" }, (event2) => {
      const hour = Number(query11(event2.target).attr("hour"));
      ...
      if (options.noMinutes) {
        ...
        query11(overlay.box).find(".tsg-overlay-body").html(html);
        this.initControls(overlay);
      }
    }).on("click.calendar", { delegate: ".tsg-time.min" }, (event2) => {
      const hour = Math.floor((this.str2min(overlay.newValue) ?? 0) / 60);
      const time = hour * 60 + parseInt(query11(event2.target).attr("min"));
    ...
    if (options.type === "date") {
      ...
      if (dt) {
        if (options.start || options.end) {
          const st = typeof options.start === "string" ? options.start : query11(options.start).val();
          const en = typeof options.end === "string" ? options.end : query11(options.end).val();
...
var TsToolbar = class extends TsBase {
  ...
  constructor(options) {
    ...
    options.items = items;
    if (typeof this.box == "string") this.box = query16(this.box).get(0);
  ...
  remove(...args) {
    ...
    args.forEach((item) => {
      ...
      effected++;
      query16(this.box).find("#tb_" + this.name + "_item_" + TsUtils.escapeId(it.id)).remove();
  ...
  setCount(id, count, className, style) {
    const btn = query16(this.box).find(`#tb_${this.name}_item_${TsUtils.escapeId(id)} .tsg-tb-count > span`);
    if (btn.length > 0) {
      ...
      this.set(id, { count });
      const after = query16(this.box).find(`#tb_${this.name}_item_${TsUtils.escapeId(id)} .tsg-tb-count > span`);
  ...
  click(id, event2) {
    ...
    if (it && !it.disabled) {
      ...
      query16(this.box).find(btn).removeClass("down");
      if (it.type == "radio") {
        ...
        it.checked = true;
        query16(this.box).find(btn).addClass("checked");
      }
      if (["menu", "menu-radio", "menu-check", "drop", "color", "text-color"].includes(it.type)) {
        ...
        if (it.checked) {
          ...
          setTimeout(() => {
            ...
            const el = query16(this.box).find("#tb_" + this.name + "_item_" + TsUtils.escapeId(it.id));
      ...
      if (["check", "menu", "menu-radio", "menu-check", "drop", "color", "text-color"].includes(it.type)) {
        ...
        if (it.checked) {
          ...
          query16(this.box).find(btn).removeClass("checked");
        }
      }
      if (it.route) {
  ...
  scroll(direction, line, instant) {
    return new Promise((resolve, _reject) => {
      const scrollBox = query16(this.box).find(`.tsg-tb-line:nth-child(${line}) .tsg-scroll-wrapper`);
  ...
  render(box) {
    const time = Date.now();
    if (typeof box == "string") box = query16(box).get(0);
    ...
    query16(this.box).attr("name", this.name).addClass("tsg-reset tsg-toolbar").html(html);
    if (query16(this.box).length > 0) {
      query16(this.box)[0].style.cssText += this["style"];
    }
    TsUtils.bindEvents(query16(this.box).find(".tsg-tb-line .tsg-eaction"), this);
  ...
  refresh(id) {
    ...
    const btn = query16(this.box).find(selector);
    const html = this.getItemHTML(it);
    this.tooltipHide(id);
    if (it.type == "spacer") {
      query16(this.box).find(`.tsg-tb-line:nth-child(${it.line ?? 1})`).find(".tsg-tb-right").css("width", "auto");
    }
    if (btn.length === 0) {
      const next = parseInt(this.get(id, true)) + 1;
      let $next = query16(this.box).find(`#tb_${this.name}_item_${TsUtils.escapeId(this.items[next] ? this.items[next].id : "--")}`);
      if ($next.length == 0) {
        $next = query16(this.box).find(`.tsg-tb-line:nth-child(${it.line})`).find(".tsg-tb-right").before(html);
      } else {
        $next.after(html);
      }
      TsUtils.bindEvents(query16(this.box).find(`${selector}, ${selector} .tsg-eaction`), this);
    } else {
      query16(this.box).find(selector).replace(query.html(html));
      const newBtn = query16(this.box).find(selector);
  ...
  resize() {
    ...
    if (edata.isCancelled === true) return;
    query16(this.box).find(".tsg-tb-line").each((el) => {
      const box = query16(el);
  ...
  destroy() {
    ...
    TsTooltip.hide(this.name + "-drop");
    if (query16(this.box).find(".tsg-scroll-wrapper").length > 0) {
      this.unmount();
    }
  ...
  change(id, value, dynamic) {
    const it = this.get(id);
    const input = query16(this.box).find("#tb_" + this.name + "_item_" + TsUtils.escapeId(id)).find("input.tsg-toolbar-input");
  ...
  tooltipShow(id) {
    ...
    const el = query16(this.box).find("#tb_" + this.name + "_item_" + TsUtils.escapeId(id)).get(0);
    const item = this.get(id);
    ...
    switch (action) {
      case "Enter":
        if (!["label", "input"].includes(btn.type)) {
          query16(target).addClass("over");
        }
        this.tooltipShow(id);
        break;
      case "Leave":
        if (!["label", "input"].includes(btn.type)) {
          query16(target).removeClass("over down");
        }
        this.tooltipHide(id);
        break;
      case "Down":
        if (!["label", "input"].includes(btn.type)) {
          query16(target).addClass("down");
        }
        break;
      case "Up":
        if (!["label", "input"].includes(btn.type)) {
          query16(target).removeClass("down");
        }
        break;
    }
    edata.finish();
