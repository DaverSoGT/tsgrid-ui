...
// src/tsform.ts
var tsform_exports = {};
__export(tsform_exports, {
  TsForm: () => TsForm
});
module.exports = __toCommonJS(tsform_exports);
...
var TsTabs = class extends TsBase {
  ...
  remove(...ids) {
    ...
    ids.forEach((it) => {
      ...
      query9(this.box).find(`#tabs_${this.name}_tab_${TsUtils.escapeId(tab.id)}`).remove();
    });
    this.resize();
    return effected;
  }
  ...
  dragMove(event) {
    ...
    const $el = query9(this.box).find("#tabs_" + this.name + "_tab_" + TsUtils.escapeId(tab.id));
    if (info.divX > 0 && next) {
      const $nextEl = query9(this.box).find("#tabs_" + this.name + "_tab_" + TsUtils.escapeId(next.id));
    ...
    if (info.divX < 0 && prev) {
      const $prevEl = query9(this.box).find("#tabs_" + this.name + "_tab_" + TsUtils.escapeId(prev.id));
      let width1 = $el.get(0).clientWidth;
  ...
  tooltipShow(id) {
    const tab = this.get(id);
    const el = query9(this.box).find("#tabs_" + this.name + "_tab_" + TsUtils.escapeId(id)).get(0);
  ...
  refresh(id) {
    const time = Date.now();
    if (this.flow == "up") {
      query9(this.box).addClass("tsg-tabs-up");
    } else {
      query9(this.box).removeClass("tsg-tabs-up");
    }
    ...
    if (id == null) {
      ...
      const $tab = query9(this.box).find(selector);
      const tabHTML = this.getTabHTML(id);
      if ($tab.length === 0) {
        if (tabHTML) query9(this.box).find("#tabs_" + this.name + "_right").before(tabHTML);
      } else {
        if (query9(this.box).find(".tab-animate-insert").length == 0) {
          if (tabHTML) $tab.replace(tabHTML);
        }
      }
      TsUtils.bindEvents(query9(this.box).find(`${selector}, ${selector} .tsg-eaction`), this);
    }
    query9(this.box).find("#tabs_" + this.name + "_right").html(this.right);
    edata.finish();
  ...
  render(box) {
    ...
    query9(this.box).attr("name", this.name).addClass("tsg-reset tsg-tabs").html(html);
    if (query9(this.box).length > 0) {
      query9(this.box)[0].style.cssText += this.style;
    }
    TsUtils.bindEvents(query9(this.box).find(".tsg-eaction"), this);
  ...
  initReorder(id, event) {
    ...
    const self = this;
    const $tab = query9(this.box).find("#tabs_" + this.name + "_tab_" + TsUtils.escapeId(id));
    const tabIndex = this.get(id, true);
    const $ghost = query9($tab.get(0).cloneNode(true));
    let edata;
    ...
    this.last.moving = {
      ...
      parentX: query9(this.box).get(0).getBoundingClientRect().left,
      x: event.pageX,
      opacity: $tab.css("opacity")
    };
    query9(document).off(".w2uiTabReorder").on("mousemove.w2uiTabReorder", function(event2) {
      ...
      if (!self.last.reordering) {
        ...
        $tab.css("opacity", 0);
        query9(self.box).find(".tsg-scroll-wrapper").append($ghost.get(0));
        query9(self.box).find(".tsg-tab-close").hide();
      }
  ...
  scroll(direction, instant) {
    return new Promise((resolve, _reject) => {
      const scrollBox = query9(this.box).find(".tsg-scroll-wrapper");
  ...
  scrollIntoView(id, instant) {
    return new Promise((resolve, _reject) => {
      ...
      const tab = this.get(id);
      if (tab == null) return;
      const tabEl = query9(this.box).find("#tabs_" + this.name + "_tab_" + TsUtils.escapeId(id)).get(0);
  ...
  destroy() {
    ...
    if (edata.isCancelled === true) return;
    if (query9(this.box).find("#tabs_" + this.name + "_right").length > 0) {
      this.unmount();
    }
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  click(id, event) {
    if (event && query9(event.target).hasClass("tsg-tab-close")) {
      return;
    }
    const tab = this.get(id);
    ...
    if (edata.isCancelled === true) return;
    query9(this.box).find("#tabs_" + this.name + "_tab_" + TsUtils.escapeId(this.active)).removeClass("active");
    this.active = tab.id;
    query9(this.box).find("#tabs_" + this.name + "_tab_" + TsUtils.escapeId(this.active)).addClass("active");
  ...
  animateInsert(id, tab) {
    return new Promise((resolve, _reject) => {
      let $before = query9(this.box).find("#tabs_" + this.name + "_tab_" + TsUtils.escapeId(id));
      const tabHTML = this.getTabHTML(tab.id);
      const $tab = query.html(tabHTML);
      if ($before.length == 0) {
        $before = query9(this.box).find("#tabs_tabs_right");
        $before.before($tab);
        this.resize();
      } else {
        $tab.css({ opacity: 0 });
        query9(this.box).find("#tabs_tabs_right").before($tab.get(0));
        const $tmp = query9(this.box).find("#" + $tab.attr("id"));
...
var TsField = class extends TsBase {
  ...
  init() {
    ...
    switch (this.type) {
      ...
      case "date": {
        ...
        options = this.options;
        if (query11(this.el).attr("placeholder") == null) {
          query11(this.el).attr("placeholder", options.format);
        }
        break;
      }
      case "time": {
      ...
      case "datetime": {
        ...
        options = this.options;
        if (query11(this.el).attr("placeholder") == null) {
          query11(this.el).attr("placeholder", options.placeholder || options.format);
        }
        break;
      }
      case "list":
      case "combo": {
        ...
        if (this.type === "list") {
          query11(this.el).addClass("tsg-select");
        ...
        query11(this.el).attr("autocapitalize", "off").attr("autocomplete", "off").attr("autocorrect", "off").attr("spellcheck", "false");
        if (options.selected.text != null) {
          query11(this.el).val(options.selected.text);
        }
        break;
      }
      case "enum": {
      ...
      case "file": {
        ...
        this.selected = options.selected;
        if (query11(this.el).attr("placeholder") == null) {
          query11(this.el).attr("placeholder", TsUtils.lang("Attach files by dragging and dropping or Click to Select"));
        }
        break;
      }
      default: {
  ...
  set(val, append) {
    if (["list", "enum", "file"].indexOf(this.type) !== -1) {
      ...
      if (this.type !== "list" && append) {
        ...
        query11(this.el).trigger("input").trigger("change");
      } else {
        if (val == null) val = [];
  ...
  refresh() {
    ...
    if (this.type == "color") {
      ...
      query11(this.helpers.suffix).find(":scope > div").css("background-color", color);
    }
    if (this.type == "list") {
      if (this.helpers.prefix) query11(this.helpers.prefix).hide();
      ...
      const focus = query11(this.helpers.search_focus);
      const icon = query11(focus.get(0).previousElementSibling);
      ...
      if (focus.val() === "") {
        ...
        if (this.selected?.id != null) {
          ...
          if (text != null) {
            ;
            query11(this.el).val(TsUtils.lang(text)).data({
              selected: text,
              selectedIndex: ind[0]
            });
          }
          ...
          this.el.value = "";
          query11(this.el).removeData("selected selectedIndex");
        }
      } else {
        focus.css("opacity", 1);
        icon.css("opacity", 1);
        ...
        setTimeout(() => {
          ...
          if (options.icon) {
            ...
            query11(this.helpers.search).find('[data-icon="search"]').addClass("show-search");
          } else {
            focus.css("margin-left", "0px");
            query11(this.helpers.search).find('[data-icon="search"]').removeClass("show-search");
          }
        }, 1);
      }
      if (query11(this.el).prop("readOnly") || query11(this.el).prop("disabled")) {
        setTimeout(() => {
          if (this.helpers.prefix) query11(this.helpers.prefix).css("opacity", "0.6");
          if (this.helpers.suffix) query11(this.helpers.suffix).css("opacity", "0.6");
        }, 1);
      } else {
        setTimeout(() => {
          if (this.helpers.prefix) query11(this.helpers.prefix).css("opacity", "1");
          if (this.helpers.suffix) query11(this.helpers.suffix).css("opacity", "1");
        }, 1);
      }
    }
    ...
    if (["enum", "file"].includes(this.type) && div) {
      ...
      if (this.selected?.length > 0) {
        query11(this.el).attr("placeholder", "");
      }
      div.find(".tsg-enum-placeholder").remove();
      ul.find(".li-item").remove();
      if (html !== "") {
        ul.prepend(html);
      } else if (query11(this.el).attr("placeholder") != null && div.find("input").val() === "") {
        const style = TsUtils.stripSpaces(`
                ...
                `);
        div.prepend(`<div class="tsg-enum-placeholder" style="${style}">${query11(this.el).attr("placeholder")}</div>`);
      }
      div.off(".w2item").on("scroll.w2item", (event) => {
        ...
        const target = query11(mouseEvent.target).closest(".li-item");
        const index = target.attr("index");
        const item = index != null ? this.selected[Number(index)] : void 0;
        if (query11(target).hasClass("li-search")) return;
        mouseEvent.stopPropagation();
        let edata;
        if (query11(mouseEvent.target).hasClass("tsg-list-remove")) {
          if (query11(this.el).prop("readOnly") || query11(this.el).prop("disabled")) return;
          edata = this.trigger("remove", { target: this.el, originalEvent: mouseEvent, item });
          if (edata.isCancelled === true) return;
          const transfer = new DataTransfer();
          const input = query11(mouseEvent.target).closest(".tsg-list").find("input.file-input").get(0);
          if (input) {
          ...
          query11(this.el).trigger("input").trigger("change");
          query11(mouseEvent.target).remove();
        } else {
          ...
          if (preview) {
            ...
            TsTooltip2.show({
              ...
              const $img = query11(`#w2overlay-${name} img`);
              $img.on("load", function(_event2) {
                const w = this.clientWidth;
                const h = this.clientHeight;
                if (w < 300 && h < 300) return;
                if (w >= h && w > 300) query11(this).css("width", "300px");
                if (w < h && h > 300) query11(this).css("height", "300px");
        ...
        const target = query11(mouseEvent.target).closest(".li-item");
        if (query11(target).hasClass("li-search")) return;
        const idx = query11(mouseEvent.target).attr("index");
  ...
  resize() {
    ...
    if (focus) {
      query11(focus).css("width", width);
    }
    if (multi) {
      query11(multi).css("width", width - parseInt(styles["margin-left"], 10) - parseInt(styles["margin-right"], 10));
    }
    if (suffix) {
      this.addSuffix();
    }
    ...
    const div = this.helpers.multi;
    if (["enum", "file"].includes(this.type) && div) {
      query11(this.el).css("height", "");
      let cntHeight = query11(div).find(":scope div.tsg-multi-items").get(0).clientHeight + 5;
      ...
      query11(div).css("height", cntHeight + "px");
      query11(this.el).css({ "height": cntHeight + "px" });
    }
  ...
  reset() {
    if (this.tmp != null) {
      ...
      ["padding-left", "padding-right", "background-color", "border-color"].forEach((prop) => {
        if (this.tmp && this.tmp["old-" + prop] != null) {
          query11(this.el).css(prop, this.tmp["old-" + prop]);
    ...
    ;
    query11(this.el).val(this.clean(query11(this.el).val())).removeClass("TsField tsg-input").removeData("selected selectedIndex").off(".TsField");
    Object.keys(this.helpers).forEach((key) => {
      query11(this.helpers[key]).remove();
    });
    this.helpers = {};
  ...
  change(event) {
    if (["int", "float", "money", "currency", "percent"].indexOf(this.type) !== -1) {
      const val = query11(this.el).val();
      const new_val = this.format(this.clean(query11(this.el).val()));
      if (val !== "" && val != new_val) {
        query11(this.el).val(new_val);
        event.stopPropagation();
        event.preventDefault();
        return false;
      }
    ...
    if (this.type === "color") {
      let color = query11(this.el).val();
      if (color.substr(0, 3).toLowerCase() !== "rgb") {
        color = "#" + color;
        const len = query11(this.el).val().length;
        if (len !== 8 && len !== 6 && len !== 3) color = "";
      }
      const next = query11(this.el).get(0).nextElementSibling;
      query11(next).find("div").css("background-color", color);
      if (query11(this.el).hasClass("has-focus")) {
        this.updateOverlay();
      }
    }
    if (["list", "enum", "file"].indexOf(this.type) !== -1) {
    ...
    if (["date", "time", "datetime"].indexOf(this.type) !== -1) {
      ...
      if (TsUtils.isInt(this.el.value) && tmp > 3e3) {
        ...
        query11(this.el).val(String(tmp)).trigger("input").trigger("change");
      }
    }
  }
  click(event) {
  ...
  focus(event) {
    if (this.type == "list" && document.activeElement == this.el) {
      ...
      if (event.showMenu !== false && this.options.openOnFocus !== false && query11(this.el).hasClass("has-focus") && !this.tmp.overlay?.overlay?.displayed) {
        setTimeout(() => {
    ...
    if (["list", "combo", "enum"].indexOf(this.type) !== -1) {
      if (query11(this.el).prop("readOnly") || query11(this.el).prop("disabled")) {
        query11(this.el).addClass("has-focus");
        return;
      }
      ...
      if (this.type == "enum") {
        const search = query11(this.el.previousElementSibling).find(".li-search input").get(0);
    ...
    if (this.type == "file") {
      const prev = query11(this.el).get(0).previousElementSibling;
      query11(prev).addClass("has-focus");
    }
    query11(this.el).addClass("has-focus");
  }
  blur(_event) {
    const val = query11(this.el).val().trim();
    query11(this.el).removeClass("has-focus");
    if (["int", "float", "money", "currency", "percent"].includes(this.type)) {
      if (val !== "") {
        let newVal = val;
        let error = "";
        ...
        if (this.options.autoCorrect) {
          ;
          query11(this.el).val(newVal).trigger("input").trigger("change");
          if (error) {
            TsTooltip2.show({
    ...
    if (["date", "time", "datetime"].includes(this.type) && this.options.autoCorrect) {
      if (val !== "") {
        ...
        if (!TsDate2.inRange(this.el.value, this.options) || !check.bind(TsUtils)(this.el.value, this.options.format)) {
          ;
          query11(this.el).val("").trigger("input").trigger("change");
        }
      }
    }
    if (this.type === "enum") {
      ;
      query11(this.helpers.multi).find("input").val("").css("width", "15px");
    }
    if (this.type == "file") {
      const prev = this.el.previousElementSibling;
      query11(prev).removeClass("has-focus");
    }
    if (this.type === "list") {
  ...
  keyDown(event, extra) {
    ...
    if (["int", "float", "money", "currency", "percent"].includes(this.type)) {
      if (!options.keyboard || query11(this.el).prop("readOnly") || query11(this.el).prop("disabled")) return;
      val = parseFloat(query11(this.el).val().replace(options.moneyRE, "")) || 0;
      inc = options.step;
      ...
      switch (key) {
        case 38:
          ...
          query11(this.el).val(String(newValue)).trigger("input").trigger("change");
          cancel = true;
          break;
        case 40:
          if (event.shiftKey) break;
    ...
    if (["date", "datetime"].includes(this.type)) {
      ...
      dt = is(query11(this.el).val(), options.format, true);
      if (!dt) {
        dt = /* @__PURE__ */ new Date();
        daymil = 0;
      }
      switch (key) {
        case 38:
          ...
          query11(this.el).val(newDT).trigger("input").trigger("change");
          cancel = true;
          break;
        case 40:
          if (event.shiftKey) break;
    ...
    if (this.type === "time") {
      ...
      if (cancel) {
        event.preventDefault();
        query11(this.el).val(TsDate2.min2str(time)).trigger("input").trigger("change");
        this.moveCaret2end();
      }
    }
    if (["list", "enum"].includes(this.type)) {
      switch (key) {
        ...
        case 46:
          if (this.type == "list") {
            const search = query11(this.helpers.search_focus);
            if (search.val() == "") {
            ...
            const search = query11(this.helpers.multi).find("input");
            if (search.val() == "") {
  ...
  keyUp(event) {
    if (this.type == "list") {
      ...
      if (search.val() !== "") {
        ...
        query11(this.el).attr("placeholder", this.tmp.pholder);
      }
      if (event.keyCode == 13) {
        setTimeout(() => {
          search.val("");
  ...
  updateOverlay(_indexOnly) {
    ...
    if (this.type === "color") {
      ...
      TsColor2.show(TsUtils.extend({
        ...
        query11(this.el).val(color).trigger("input").trigger("change");
      }).liveUpdate((event) => {
        const color = event.detail["color"];
        query11(this.helpers.suffix).find(":scope > div").css("background-color", "#" + color);
      });
    ...
    if (["list", "combo", "enum"].includes(this.type)) {
      ...
      if (this.type === "enum") {
        ...
        input = query11(el).find("input").get(0) ?? this.el;
      }
      if (this.type === "list") {
      ...
      if (query11(this.el).hasClass("has-focus") && !this.el.readOnly && !this.el.disabled) {
        ...
        this.tmp.overlay = TsMenu2.show(params).select((event) => {
          if (["list", "combo"].includes(this.type)) {
            ...
            query11(input).val("");
            query11(this.el).val(this.selected.text).trigger("input").trigger("change");
            ...
            if (newItem) {
              ...
              query11(this.el).trigger("input").trigger("change");
              query11(this.helpers.multi).find("input").val("");
              const overlay = TsMenu2.get(this.el.id + "_menu");
              if (overlay) overlay.options.selected = this.selected;
              edata.finish();
            }
    ...
    if (["date", "time", "datetime"].includes(this.type)) {
      ...
      TsDate2.show(TsUtils.extend({
        ...
        if (date != null) {
          ;
          query11(this.el).val(date).trigger("input").trigger("change");
        }
      });
    }
  }
  ...
  */
  ...
  addPrefix() {
    ...
    if (this.helpers.prefix) query11(this.helpers.prefix).remove();
    query11(this.el).before(`<div class="tsg-field-helper">${this.options.prefix}</div>`);
    const helper = query11(this.el).get(0).previousElementSibling;
    query11(helper).css({
      "color": styles["color"],
      "font-family": styles["font-family"],
      "font-size": styles["font-size"],
    ...
    query11(this.el).css("padding-left", helper.clientWidth + "px !important");
  ...
  addSuffix() {
    ...
    if (this.options.arrows) {
      if (this.helpers.arrows) query11(this.helpers.arrows).remove();
      query11(this.el).after(
      ...
      const arrowHelper = query11(this.el).get(0).nextElementSibling;
      const $arrowHelper = query11(arrowHelper);
      $arrowHelper.css({
        "color": styles["color"],
        "font-family": styles["font-family"],
        "font-size": styles["font-size"],
      ...
      $arrowHelper.on("mousedown", (event) => {
        const mouseEvent = event;
        if (query11(mouseEvent.target).hasClass("arrow-up")) {
          this.keyDown(mouseEvent, { keyCode: 38 });
        }
        if (query11(mouseEvent.target).hasClass("arrow-down")) {
          this.keyDown(mouseEvent, { keyCode: 40 });
        }
      });
      pr += arrowHelper.clientWidth;
      query11(this.el).css("padding-right", pr + "px !important");
      this.helpers.arrows = arrowHelper;
    }
    if (this.options.suffix !== "") {
      if (this.helpers.suffix) query11(this.helpers.suffix).remove();
      query11(this.el).after(`<div class="tsg-field-helper">${this.options.suffix}</div>`);
      const suffixHelper = query11(this.el).get(0).nextElementSibling;
      query11(suffixHelper).css({
        "color": styles["color"],
        "font-family": styles["font-family"],
        "font-size": styles["font-size"],
        "height": this.el.clientHeight + "px",
      ...
      query11(this.el).css("padding-right", suffixHelper.clientWidth + "px !important");
  ...
  addSearch() {
    ...
    if (this.helpers.search) query11(this.helpers.search).remove();
    let tabIndex = parseInt(query11(this.el).attr("tabIndex"));
    ...
    let searchId = "";
    if (query11(this.el).attr("id") != null) {
      searchId = 'id="' + query11(this.el).attr("id") + '_search"';
    }
    const html = `
            <div class="tsg-field-helper">
                <span class="tsg-icon" data-icon="search">${searchIcon()}</span>
                <input ${searchId} type="text" tabIndex="${tabIndex}" ${query11(this.el).prop("readOnly") ? "readonly" : ""}
                    autocapitalize="off" autocomplete="off" autocorrect="off" spellcheck="false"/>
            </div>`;
    query11(this.el).attr("tabindex", String(-1)).before(html);
    const helper = query11(this.el).get(0).previousElementSibling;
    this.helpers.search = helper;
    this.helpers.search_focus = query11(helper).find("input").get(0);
    const styles = getComputedStyle(this.el);
    const $helperSearch = query11(helper);
    $helperSearch.css({
    ...
    query11(helper).find("input").off(".tsg-helper").on("focus.tsg-helper", (event) => {
      const focusEvent = event;
      query11(focusEvent.target).val("");
      this.tmp.pholder = query11(this.el).attr("placeholder") ?? "";
      this.focus(focusEvent);
      focusEvent.stopPropagation();
    }).on("blur.tsg-helper", (event) => {
      const focusEvent = event;
      query11(focusEvent.target).val("");
      if (this.tmp.pholder != null) query11(this.el).attr("placeholder", this.tmp.pholder);
    ...
    query11(helper).off(".tsg-helper").on("click.tsg-helper", (_event) => {
      query11(helper).find("input").get(0).focus();
    });
  }
  // Used in enum/file
  addMultiSearch() {
    ...
    query11(this.helpers.multi).remove();
    let html = "";
    const styles = getComputedStyle(this.el);
                ...
                <div class="tsg-multi-items">
                    <div class="li-search">
                        <input ${searchId} type="text" autocapitalize="off" autocomplete="off" autocorrect="off" spellcheck="false"
                            tabindex="${tabIndex}"
                            ${query11(this.el).prop("readOnly") ? "readonly" : ""}
                            ${query11(this.el).prop("disabled") ? "disabled" : ""}>
                    </div>
                </div>
            </div>`;
    }
            ...
            <div class="tsg-field-helper tsg-list" style="${margin}">
                <div class="tsg-multi-file">
                    <input name="attachment" class="file-input" type="file" tabindex="-1"'
                        ...
                        ${query11(this.el).prop("readOnly") || query11(this.el).prop("disabled") ? "disabled" : ""}
                        ${query11(this.el).attr("accept") ? ' accept="' + query11(this.el).attr("accept") + '"' : ""}>
                </div>
                <div class="tsg-multi-items">
    ...
    query11(this.el).before(html).css({
      "border-color": "transparent",
      "background-color": "transparent"
    });
    const div = query11(this.el.previousElementSibling);
    this.helpers.multi = div;
    query11(this.el).attr("tabindex", String(-1));
    div.on("mousedown", (event) => {
      query11(event.target).addClass("has-focus");
    }).on("mouseup", (event) => {
      query11(event.target).removeClass("has-focus");
    }).on("click", (event) => {
      this.focus(event);
      this.updateOverlay();
    });
...
var TsForm = class extends TsBase {
  ...
  getFieldValue(name) {
    ...
    if (["radio"].includes(field.type)) {
      const selected2 = query12(el).closest(".tsg-field-group").find("input:checked").get(0);
      if (selected2) {
        const item = field.options.items[query12(selected2).data("index")];
        current = item.id;
      } else {
        current = null;
      }
    ...
    if (["check", "checks"].includes(field.type)) {
      current = [];
      const selected2 = query12(el).closest(".tsg-field-group").find("input:checked");
      if (selected2.length > 0) {
        selected2.each((node) => {
          const el2 = node;
          const item = field.options.items[query12(el2).data("index")];
    ...
    if (["map", "array"].includes(field.type)) {
      ...
      field.$el.parent().find(".tsg-map-field").each((div, _ind) => {
        const key = query12(div).find(".tsg-map.key").val();
        const value = query12(div).find(".tsg-map.value").val();
        if (typeof field.html?.render == "function") {
          current[_ind] ??= {};
          query12(div).find("input, textarea").each((node) => {
            const inp = node;
  ...
  setFieldValue(name, value) {
    ...
    switch (field.type) {
      ...
      case "radio": {
        ...
        const inputs = query12(el).closest(".tsg-field-group").find("input");
      ...
      case "checks": {
        ...
        const inputs = query12(el).closest("div.tsg-field-group").find("input");
        const items = field.options.items;
        items.forEach((it, ind) => {
          const input = inputs.filter(`[data-index="${ind}"]`);
      ...
      case "array": {
        ...
        const container = query12(field.el).parent().find(".tsg-map-container");
        field.el.mapRefresh(value, container);
        break;
      }
      case "div":
      case "custom": {
        query12(el).html(value);
        break;
      }
      case "color": {
        el.value = value ?? "";
  ...
  updateEmptyGroups() {
    query12(this.box).find(".tsg-group").each((node) => {
      const group = node;
      if (isHidden(query12(group).find(".tsg-field"))) {
        query12(group).hide();
      } else {
        query12(group).show();
      }
  ...
  toggleGroup(groupName, show) {
    const el = query12(this.box).find('.tsg-group-title[data-group="' + TsUtils.base64encode(groupName) + '"]');
    if (el.length === 0) return;
    const el_next = query12(el.prop("nextElementSibling"));
  ...
  showErrors() {
    ...
    query12(errors[0].field.$el).parents(".tsg-field").get(0).scrollIntoView({ block: "nearest", inline: "nearest" });
    errors.forEach((error) => {
      ...
      if (error.field.type === "radio") {
        anchor = query12(error.field.el).closest("div").get(0);
      } else if (["enum", "file"].includes(error.field.type)) {
      }
      TsTooltip3.show(TsUtils.extend({
        anchor,
    ...
    query12(errors[0].field.$el).parents(".tsg-page").off(".hideErrors").on("scroll.hideErrors", (_evt) => {
  ...
  hideErrors() {
    ...
    this.fields.forEach((field) => {
      TsTooltip3.hide(`${this.name}-${field.field}-error`);
    });
  }
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  getChanges() {
  ...
  lockPage(page, msg, spinner) {
    const $page = query12(this.box).find(".page-" + page);
    if ($page.length) {
  ...
  goto(page) {
    ...
    if (query12(this.box).data("autoSize") === true) {
      query12(this.box).get(0).clientHeight = 0;
    }
    this.refresh();
  }
  generateHTML() {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  getAction(action) {
    const ret = query12(this.box).find('.tsg-buttons button[name="' + action + '"]');
    if (ret.length === 0) {
  ...
  resize() {
    ...
    if (this.box != null) {
      ...
      const header = query12(this.box).find(":scope > div .tsg-form-header");
      const toolbar = query12(this.box).find(":scope > div .tsg-form-toolbar");
      const tabs = query12(this.box).find(":scope > div .tsg-form-tabs");
      const page = query12(this.box).find(":scope > div .tsg-page");
      const dpage = query12(this.box).find(":scope > div .tsg-page.page-" + this.page + " > div");
      const buttons = query12(this.box).find(":scope > div .tsg-buttons");
      const { headerHeight, tbHeight, tabsHeight } = resizeElements2();
      if (this.autosize) {
        const cHeight = query12(this.box).get(0).clientHeight;
        if (cHeight === 0 || query12(this.box).data("autosize") == "yes") {
          query12(this.box).css({
          ...
          query12(this.box).data("autosize", "yes");
        }
        resizeElements2();
      }
      this.tabs?.resize?.();
  ...
  refresh(...args) {
    ...
    if (!this.isGenerated || !query12(this.box).html()) return 0;
    ...
    if (args.length > 0) {
      ...
      query12(this.box).find("input, textarea, select").each((node) => {
        const el = node;
        const name = query12(el).attr("name") != null ? query12(el).attr("name") : query12(el).attr("id");
        const field = this.get(name);
        if (field) {
          const div = query12(el).closest(".tsg-page");
          if (div.length > 0) {
            for (let i = 0; i < 100; i++) {
      ...
      query12(this.box).find(".tsg-page").hide();
      query12(this.box).find(".tsg-page.page-" + this.page).show();
      query12(this.box).find(".tsg-form-header").html(TsUtils.lang(this.header));
      if (typeof this.tabs === "object" && Array.isArray(this.tabs.tabs) && this.tabs.tabs.length > 0) {
        query12(this.box).find("#form_" + this.name + "_tabs").show();
        this.tabs.active = this.tabs.tabs[this.page].id;
        this.tabs.refresh();
      } else {
        query12(this.box).find("#form_" + this.name + "_tabs").hide();
      }
      if (typeof this.toolbar === "object" && Array.isArray(this.toolbar.items) && this.toolbar.items.length > 0) {
        query12(this.box).find("#form_" + this.name + "_toolbar").show();
        this.toolbar.refresh();
      } else {
        query12(this.box).find("#form_" + this.name + "_toolbar").hide();
      }
    }
    for (let f = 0; f < fields.length; f++) {
      ...
      field.$el = query12(this.box).find(`[name='${String(field.name).replace(/\\/g, "\\\\")}']`);
      ...
      field.$el.off(".TsForm").on("change.TsForm", function(event) {
        ...
        if (["enum", "file"].includes(field.type)) {
          ...
          query12(helper).removeClass("tsg-error");
        }
      ...
      let tmp = field.el;
      if (!tmp) tmp = query12(this.box).find("#" + field.field);
      if (field.hidden) {
        query12(tmp).closest(".tsg-field").hide();
      } else {
        query12(tmp).closest(".tsg-field").show();
      }
    }
    query12(this.box).find("button, input[type=button]").each((node) => {
      const el = node;
      query12(el).off("click").on("click", function(event) {
    ...
    for (let f = 0; f < fields.length; f++) {
      ...
      if (field.type == "switch") {
        ...
        field.$el.off(".form-input").on("focus.form-input", (event) => {
          ...
          query12(event.target).prop("_index", ind);
          query12(field.toolbar.box).addClass("tsg-tb-focus");
        }).on("blur.form-input", (event) => {
          query12(event.target).removeProp("_index");
          query12(`#${field.name}-tb .tsg-tb-button`).removeClass("over");
          query12(field.toolbar.box).removeClass("tsg-tb-focus");
        }).on("keydown.form-input", (event) => {
          let ind = query12(event.target).prop("_index");
          switch (event.key) {
            case "ArrowLeft": {
              if (ind > 0) ind--;
              query12(`#${field.name}-tb .tsg-tb-button`).removeClass("over").eq(ind).addClass("over");
              query12(event.target).prop("_index", ind);
              break;
            }
            case "ArrowRight": {
      ...
      if (["map", "array"].includes(field.type)) {
        (function(obj, field2) {
          field2.el.mapAdd = function(field3, div, cnt, empty) {
            ...
            if (typeof field3.html.render == "function") {
              ...
              box.find("input, textarea").each((el) => {
                if (query12(el).attr("tabindex") == null) {
                  query12(el).attr("tabindex", field3.html.tabindex);
                }
              });
          ...
          field2.el.mapRefresh = function(map, div) {
            ...
            let lastKey = null;
            const container = query12(field2.el).get(0)?.nextSibling;
            query12(container).off(".mapChange").on("mouseup.mapChange", { delegate: "input, textarea" }, function(event) {
              if (document.activeElement != event.target) {
                event.target.focus();
              }
            }).on("keyup.mapChange", { delegate: "input, textarea" }, function(event) {
              const kbdEvent = event;
              const $div = query12(kbdEvent.target).closest(".tsg-map-field");
              const next = $div.get(0).nextElementSibling;
              const prev = $div.get(0).previousElementSibling;
              const className = query12(kbdEvent.target).hasClass("key") ? "key" : "value";
              if (kbdEvent.keyCode == 38 && prev) {
                query12(prev).find(`input.${className}, textarea.${className}, input[name="${kbdEvent.target["name"]}"] textarea[name="${kbdEvent.target["name"]}"]`).get(0)?.select();
              ...
              if (kbdEvent.keyCode == 40 && next) {
                ...
                query12(next2).find(`input.${className}, textarea.${className}, input[name="${kbdEvent.target["name"]}"] textarea[name="${kbdEvent.target["name"]}"]`).get(0)?.select();
                kbdEvent.preventDefault();
              }
              ...
              const $cnt = query12(event.target).closest(".tsg-map-container");
              if (typeof field2.html?.render == "function") {
              ...
              let index;
              let className = "";
              const cnt2 = query12(event.target).closest(".tsg-map-container");
              if (field2.type == "array" || lastKey == "tab") {
                cnt2.find("input, textarea").each((node, ind) => {
                  if (node == event.target) index = ind;
                });
              } else {
                className = query12(event.target).hasClass("key") ? ".key" : ".value";
  ...
  render(box) {
    ...
    query12(this.box).attr("name", this.name).addClass("tsg-reset tsg-form").html(html);
    if (query12(this.box).length > 0) query12(this.box).get(0).style.cssText += this.style;
    ...
    if (typeof this.toolbar === "object" && typeof this.toolbar.render === "function") {
      this.toolbar.render(query12(this.box).find("#form_" + this.name + "_toolbar").get(0));
    }
    ...
    if (typeof this.tabs === "object" && typeof this.tabs.render === "function") {
      this.tabs.render(query12(this.box).find("#form_" + this.name + "_tabs").get(0));
    ...
    if (this.focus != -1) {
      let setCount = 0;
      const setFocus = () => {
        if (query12(self.box).find("input, select, textarea").length > 0) {
          self.setFocus();
        } else {
          setCount++;
  ...
  destroy() {
    ...
    if (query12(this.box).find("#form_" + this.name + "_tabs").length > 0) {
      this.unmount();
    }
    this.last.observeResize?.disconnect();
    delete TsUi[this.name];
  ...
  setFocus(focus) {
    ...
    if (TsUtils.isInt(focus)) {
      ...
      const inputs = query12(this.box).find("div:not(.tsg-field-helper) > input, select, textarea, div > label:nth-child(1) > [type=radio]").filter(":not(.file-input)");
      while (inputs.get(focus)?.offsetParent == null && inputs.length > focus) {
        focus++;
      }
      if (inputs.get(focus)) {
        $input = query12(inputs.get(focus));
      }
    } else if (typeof focus === "string") {
      $input = query12(this.box).find(`[name='${focus}']`);
    }
    if ($input?.length > 0) {
      $input.get(0).focus();
    }
