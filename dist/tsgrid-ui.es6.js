...
import {
  TsLayout
} from "./chunks/chunk-QTT73B7C.js";
import {
  TsForm
} from "./chunks/chunk-52GT6557.js";
import {
  TsField
...
function status(grid, msg) {
  if (msg != null) {
    query3(grid.box).find(`#grid_${grid.name}_footer`).find(".tsg-footer-left").html(msg);
  } else {
    let msgLeft = "";
    ...
    query3(grid.box).find("#grid_" + grid.name + "_footer .tsg-footer-left").html(msgLeft);
  }
}
function lock(grid, msg, showSpinner) {
  const args = [grid.box, msg, showSpinner];
  setTimeout(() => {
    query3(grid.box).find("#grid_" + grid.name + "_empty_msg").remove();
    TsUtils.lock(...args);
  }, 10);
}
function unlock(grid, speed) {
  setTimeout(() => {
    if (query3(grid.box).find(".tsg-message").hasClass("tsg-closing")) return;
...
function add(grid, record, first) {
  ...
  if (!url) {
    ...
    if ((grid.last.vscroll.recIndStart ?? 0) <= indEnd && (grid.last.vscroll.recIndEnd ?? 0) >= indStart) {
      ...
      query4(grid.box).find("#grid_" + grid.name + "_footer .tsg-footer-right .tsg-total").html(TsUtils.formatNumber(grid.total));
    }
  } else {
    grid.refresh();
  }
...
function removeRange(grid, ...names) {
  ...
  for (let a = 0; a < names.length; a++) {
    const name = names[a];
    query4(grid.box).find("#grid_" + grid.name + "_" + name).remove();
    query4(grid.box).find("#grid_" + grid.name + "_f" + name).remove();
...
function refreshRanges(grid) {
  ...
  let range;
  const time = Date.now();
  const rec1 = query4(grid.box).find(`#grid_${grid.name}_frecords`);
  const rec2 = query4(grid.box).find(`#grid_${grid.name}_records`);
  for (let i = 0; i < grid.ranges.length; i++) {
    ...
    let td1 = query4(grid.box).find("#grid_" + grid.name + "_rec_" + TsUtils.escapeId(first.recid) + ' td[col="' + first.column + '"]');
    let td2 = query4(grid.box).find("#grid_" + grid.name + "_rec_" + TsUtils.escapeId(last.recid) + ' td[col="' + last.column + '"]');
    let td1f = query4(grid.box).find("#grid_" + grid.name + "_frec_" + TsUtils.escapeId(first.recid) + ' td[col="' + first.column + '"]');
    let td2f = query4(grid.box).find("#grid_" + grid.name + "_frec_" + TsUtils.escapeId(last.recid) + ' td[col="' + last.column + '"]');
    let _lastColumn = last.column;
    if (first.column < grid.last.vscroll.colIndStart && last.column > grid.last.vscroll.colIndStart) {
      td1 = query4(grid.box).find("#grid_" + grid.name + "_rec_" + TsUtils.escapeId(first.recid) + ' td[col="start"]');
    }
    if (first.column < grid.last.vscroll.colIndEnd && last.column > grid.last.vscroll.colIndEnd) {
      td2 = query4(grid.box).find("#grid_" + grid.name + "_rec_" + TsUtils.escapeId(last.recid) + ' td[col="end"]');
      _lastColumn = "end";
    }
    const index_top = parseInt(query4(grid.box).find("#grid_" + grid.name + "_rec_top").next().attr("index"));
    const index_bottom = parseInt(query4(grid.box).find("#grid_" + grid.name + "_rec_bottom").prev().attr("index"));
    const index_ftop = parseInt(query4(grid.box).find("#grid_" + grid.name + "_frec_top").next().attr("index"));
    const index_fbottom = parseInt(query4(grid.box).find("#grid_" + grid.name + "_frec_bottom").prev().attr("index"));
    if (td1.length === 0 && first.index < index_top && last.index > index_top) {
      td1 = query4(grid.box).find("#grid_" + grid.name + "_rec_top").next().find('td[col="' + first.column + '"]');
    }
    if (td2.length === 0 && last.index > index_bottom && first.index < index_bottom) {
      td2 = query4(grid.box).find("#grid_" + grid.name + "_rec_bottom").prev().find('td[col="' + _lastColumn + '"]');
    }
    if (td1f.length === 0 && first.index < index_ftop && last.index > index_ftop) {
      td1f = query4(grid.box).find("#grid_" + grid.name + "_frec_top").next().find('td[col="' + first.column + '"]');
    }
    if (td2f.length === 0 && last.index > index_fbottom && first.index < index_fbottom) {
      td2f = query4(grid.box).find("#grid_" + grid.name + "_frec_bottom").prev().find('td[col="' + last.column + '"]');
    }
    const edit = query4(grid.box).find("#grid_" + grid.name + "_editable");
    ...
    range = query4(grid.box).find("#grid_" + grid.name + "_f" + rg.name);
    if (td1f.length > 0 || td2f.length > 0) {
      ...
      if (td2f.length === 0) {
        td2f = query4(grid.box).find("#grid_" + grid.name + "_frec_" + TsUtils.escapeId(last.recid) + " td:last-child");
        if (td2f.length === 0) td2f = query4(grid.box).find("#grid_" + grid.name + "_frec_bottom td:first-child");
    ...
    range = query4(grid.box).find("#grid_" + grid.name + "_" + rg.name);
    if (td1.length > 0 || td2.length > 0) {
      ...
      if (td1.length === 0) {
        td1 = query4(grid.box).find("#grid_" + grid.name + "_rec_" + TsUtils.escapeId(first.recid) + " td:first-child");
        if (td1.length === 0) td1 = query4(grid.box).find("#grid_" + grid.name + "_rec_top td:first-child");
      }
      if (td2f.length !== 0) {
  ...
  query4(grid.box).find(".tsg-selection-resizer").off(".resizer").on("mousedown.resizer", mouseStart).on("dblclick.resizer", (event2) => {
  ...
  function mouseStart(event2) {
    ...
    query4("body").off(".tsg-" + self.name).on("mousemove.tsg-" + self.name, mouseMove).on("mouseup.tsg-" + self.name, mouseStop);
  ...
  function mouseMove(event2) {
    ...
    if (tmp.tagName.toUpperCase() != "TD") tmp = query4(tmp).closest("td")[0];
    if (query4(tmp).attr("col") != null) column = parseInt(query4(tmp).attr("col"));
    if (column == null) {
      return;
    }
    tmp = query4(tmp).closest("tr")[0];
    const index = parseInt(query4(tmp).attr("index"));
...
function reset(grid, noRefresh) {
  ...
  query4(grid.box).find(`#grid_${grid.name}_records`).prop("scrollTop", 0);
  if (!noRefresh) grid.refresh();
}
...
function select(grid, ...selectArgs) {
  ...
  if (grid.selectType == "row") {
    for (let a = 0; a < args.length; a++) {
      ...
      if (grid.searchData.length !== 0 || index + 1 >= (grid.last.vscroll.recIndStart ?? 0) && index + 1 <= (grid.last.vscroll.recIndEnd ?? 0)) {
        recEl1 = query5(grid.box).find("#grid_" + grid.name + "_frec_" + TsUtils.escapeId(recid));
        recEl2 = query5(grid.box).find("#grid_" + grid.name + "_rec_" + TsUtils.escapeId(recid));
      }
      if (grid.selectType == "row") {
    ...
    for (const recid in new_sel) {
      ...
      if (index + 1 >= (grid.last.vscroll.recIndStart ?? 0) && index + 1 <= (grid.last.vscroll.recIndEnd ?? 0)) {
        recEl1 = query5(grid.box).find("#grid_" + grid.name + "_rec_" + TsUtils.escapeId(recid));
        recEl2 = query5(grid.box).find("#grid_" + grid.name + "_frec_" + TsUtils.escapeId(recid));
      }
    ...
    for (let c = 0; c < col_sel.length; c++) {
      query5(grid.box).find("#grid_" + grid.name + "_column_" + col_sel[c] + " .tsg-col-header").addClass("tsg-col-selected");
    }
  }
  ...
  if (areAllSelected || areAllSearchedSelected) {
    query5(grid.box).find("#grid_" + grid.name + "_check_all").prop("checked", true);
  } else {
    query5(grid.box).find("#grid_" + grid.name + "_check_all").prop("checked", false);
  }
  grid.status();
...
function unselect(grid, ...unselectArgs) {
  ...
  for (let a = 0; a < args.length; a++) {
    ...
    const recEl1 = query5(grid.box).find("#grid_" + grid.name + "_frec_" + TsUtils.escapeId(recid));
    const recEl2 = query5(grid.box).find("#grid_" + grid.name + "_rec_" + TsUtils.escapeId(recid));
    if (grid.selectType == "row") {
      if (sel.indexes.indexOf(index) == -1) continue;
      ...
      query5(grid.box).find(`#grid_${grid.name}_rec_${TsUtils.escapeId(recid)} > td[col="${col}"]`).removeClass("tsg-selected tsg-inactive");
      query5(grid.box).find(`#grid_${grid.name}_frec_${TsUtils.escapeId(recid)} > td[col="${col}"]`).removeClass("tsg-selected tsg-inactive");
      ...
      if (!isColSelected) {
        query5(grid.box).find(`.tsg-grid-columns td[col="${col}"] .tsg-col-header, .tsg-grid-fcolumns td[col="${col}"] .tsg-col-header`).removeClass("tsg-col-selected");
      }
      if (!isRowSelected) {
        query5(grid.box).find("#grid_" + grid.name + "_frec_" + TsUtils.escapeId(recid)).find(".tsg-col-number").removeClass("tsg-row-selected");
      }
      unselected++;
      if (s.length === 0) {
...
function selectAll(grid) {
  ...
  if (grid.selectType == "row") {
    query5(grid.box).find(".tsg-grid-records tr:not(.tsg-empty-record)").addClass("tsg-selected").find(".tsg-col-number").addClass("tsg-row-selected");
    query5(grid.box).find(".tsg-grid-frecords tr:not(.tsg-empty-record)").addClass("tsg-selected").find(".tsg-col-number").addClass("tsg-row-selected");
    query5(grid.box).find("input.tsg-grid-select-check").prop("checked", true);
  } else {
    query5(grid.box).find(".tsg-grid-columns td .tsg-col-header, .tsg-grid-fcolumns td .tsg-col-header").addClass("tsg-col-selected");
    query5(grid.box).find(".tsg-grid-records tr .tsg-col-number").addClass("tsg-row-selected");
    query5(grid.box).find(".tsg-grid-records tr:not(.tsg-empty-record)").find(".tsg-grid-data:not(.tsg-col-select)").addClass("tsg-selected");
    query5(grid.box).find(".tsg-grid-frecords tr .tsg-col-number").addClass("tsg-row-selected");
    query5(grid.box).find(".tsg-grid-frecords tr:not(.tsg-empty-record)").find(".tsg-grid-data:not(.tsg-col-select)").addClass("tsg-selected");
    query5(grid.box).find("input.tsg-grid-select-check").prop("checked", true);
  }
...
function selectNone(grid, skipEvent) {
  ...
  const sel = grid.last.selection;
  if (grid.selectType == "row") {
    query5(grid.box).find(".tsg-grid-records tr.tsg-selected").removeClass("tsg-selected tsg-inactive").find(".tsg-col-number").removeClass("tsg-row-selected");
    query5(grid.box).find(".tsg-grid-frecords tr.tsg-selected").removeClass("tsg-selected tsg-inactive").find(".tsg-col-number").removeClass("tsg-row-selected");
    query5(grid.box).find("input.tsg-grid-select-check").prop("checked", false);
  } else {
    query5(grid.box).find(".tsg-grid-columns td .tsg-col-header, .tsg-grid-fcolumns td .tsg-col-header").removeClass("tsg-col-selected");
    query5(grid.box).find(".tsg-grid-records tr .tsg-col-number").removeClass("tsg-row-selected");
    query5(grid.box).find(".tsg-grid-frecords tr .tsg-col-number").removeClass("tsg-row-selected");
    query5(grid.box).find(".tsg-grid-data.tsg-selected").removeClass("tsg-selected tsg-inactive");
    query5(grid.box).find("input.tsg-grid-select-check").prop("checked", false);
  }
  sel.indexes = [];
  sel.columns = {};
...
function editField(grid, recid, column, value, event2) {
  ...
  if (grid.last.inEditMode === true) {
    if (event2 && event2.keyCode == 13) {
      ...
      const input2 = query6(grid.box).find("div.tsg-edit-box .tsg-input");
      if (input2.length > 0) {
  ...
  const tr = query6(grid.box).find("#grid_" + grid.name + prefix + "rec_" + TsUtils.escapeId(recid));
  let div = tr.find('[col="' + column + '"] > div');
  grid.last._edit["tr"] = tr;
  grid.last._edit["div"] = div;
  query6(grid.box).find("div.tsg-edit-box").remove();
  if (grid.selectType != "row") {
    query6(grid.box).find("#grid_" + grid.name + prefix + "selection").attr("id", "grid_" + grid.name + "_editable").removeClass("tsg-selection").addClass("tsg-edit-box").prepend('<div style="position: absolute; top: 0px; bottom: 0px; left: 0px; right: 0px;"></div>').find(".tsg-selection-resizer").remove();
    div = query6(grid.box).find("#grid_" + grid.name + "_editable > div:first-child");
  }
  edit.attr = edit.attr ?? "";
  edit.text = edit.text ?? "";
  ...
  switch (edit.type) {
    ...
    default: {
      ...
      const doHide = (event3) => {
        ...
        let selected = false;
        const name = query6(input).data("tooltipName");
  ...
  query6(input).off(".tsg-editable").on("blur.tsg-editable", (event3) => {
  ...
  function expand2(input2) {
    try {
      ...
      const editBox = query6(self.box).find("#grid_" + self.name + "_editable").get(0);
      ...
      if (width + 20 > editBox.clientWidth) {
        query6(editBox).css("width", width + 20 + "px");
      }
    } catch (e) {
    }
  }
...
function editDone(grid, index, column, event2) {
  ...
  const summary = index < 0;
  const cell = query6(grid.last._edit?.["tr"]).find('[col="' + column + '"]');
  const rec = grid.records[index];
  const col = grid.columns[column];
  ...
  grid.updateToolbar();
  setTimeout(() => {
    const input = query6(grid.box).find(`#grid_${grid.name}_focus`).get(0);
...
function search(grid, field, value) {
  ...
  const overlay = query7(`#w2overlay-${grid.name}-search-overlay`);
  if (value === "") value = null;
  for (let i = 0; i < grid.searches.length; i++) {
...
function searchOpen(grid, options = {}) {
  ...
  const $btn = query7(grid.toolbar.box).find(".tsg-grid-search-input .tsg-search-drop");
  $btn.addClass("checked");
  TsTooltip3.show({
    name: grid.name + "-search-overlay",
    anchor: query7(grid.box).find("#grid_" + grid.name + "_search_all").get(0),
    position: "bottom|top",
...
function searchFieldTooltip(grid, ind, sd_ind, el) {
  ...
  TsTooltip3.show({
    ...
    query7(event2.detail.overlay.box).find("#remove").on("click", () => {
...
function searchSuggest(grid, imediate, forceHide, anchor) {
  ...
  if (forceHide === true || anchor != null && query7(`#w2overlay-${grid.name}-search-suggest`).length > 0) {
    TsTooltip3.hide(grid.name + "-search-suggest");
    return;
  }
  if (query7(`#w2overlay-${grid.name}-search-suggest`).length > 0) {
    return;
  }
  if (!imediate) {
  ...
  const el = anchor ?? query7(grid.box).find(`#grid_${grid.name}_search_all`).get(0);
  const searches = [
...
function searchSave(grid) {
  ...
  grid.message({
    ...
    query7(event2.detail.box).find("input, button").eq(0).val(value);
    await event2.complete;
    query7(event2.detail.box).find("#grid-search-cancel").on("click", () => {
      grid.message();
    });
    query7(event2.detail.box).find("#grid-search-save").on("click", () => {
      const input = query7(event2.detail.box).find(".tsg-message .search-name");
      ...
      if (grid["searchSelected"]) {
        ...
        query7(grid.box).find(`#grid_${grid.name}_search_name .name-text`).html(name);
      } else {
        grid["searchSelected"] = {
          text: name,
          logic: grid.last.logic,
        ...
        query7(event2.detail.box).find(`#grid_${grid.name}_search_all`).val(" ").prop("readOnly", true);
        query7(event2.detail.box).find(`#grid_${grid.name}_search_name`).show().find(".name-text").html(name);
      }
      edata.finish({ name });
    });
    await TsUtils.wait(100);
    query7(event2.detail.box).find("input, button").off(".message").on("keydown.message", (evt) => {
      const val = String(query7(event2.detail.box).find(".tsg-message-body input").val()).trim();
      if (evt.keyCode == 13 && val != "") {
        query7(event2.detail.box).find("#grid-search-save").trigger("click");
      }
      if (evt.keyCode == 27) {
        grid.message();
      }
    }).eq(0).on("input.message", (_evt) => {
      const $save = query7(event2.detail.box).closest(".tsg-message").find("#grid-search-save");
      if (String(query7(event2.detail.box).val()).trim() === "") {
...
function searchReset(grid, noReload) {
  ...
  if (edata.isCancelled === true) return;
  const input = query7(grid.box).find("#grid_" + grid.name + "_search_all");
  grid.searchData = edata.detail["searchData"];
...
function searchShowFields(grid, forceHide) {
  ...
  TsMenu2.show({
    ...
    anchor: query7(grid.box).find("#grid_" + grid.name + "_search_name").parent().find(".tsg-search-down").get(0),
    items,
    align: "none",
...
function searchInitInput(grid, field, _value) {
  let srch;
  const el = query7(grid.box).find("#grid_" + grid.name + "_search_all");
  if (field == "all") {
  ...
  if (grid["searchSelected"]) {
    query7(grid.box).find(`#grid_${grid.name}_search_all`).val(" ").prop("readOnly", true);
    query7(grid.box).find(`#grid_${grid.name}_search_name`).show().find(".name-text").html(grid["searchSelected"].text);
  } else {
    query7(grid.box).find(`#grid_${grid.name}_search_all`).prop("readOnly", false);
    query7(grid.box).find(`#grid_${grid.name}_search_name`).hide().find(".name-text").html("");
  }
}
...
function click(grid, recid, event2) {
  ...
  if (column == null && event2.target) {
    ...
    if (trg.tagName != "TD") trg = query8(trg).closest("td")[0];
    if (query8(trg).attr("col") != null) column = parseInt(query8(trg).attr("col"));
  }
  const index = grid.get(recid, true);
  const rec = index != null ? grid.records[index] : null;
  if (rec?.TsUi?.selectable === false && (rec?.TsUi?.children?.length ?? 0) > 0) {
    if (!query8(event2.target).hasClass("tsg-show-children")) {
      grid.toggle(recid);
      return;
    }
  }
  ...
  if (event2.shiftKey && sel.length > 0 && grid.multiSelect) {
    ...
    let fselect = false;
    if (query8(event2.target).closest("td").hasClass("tsg-col-select")) fselect = true;
...
function columnContextMenu(grid, field, event2) {
  ...
  TsMenu3.show({
    ...
    query8("#w2overlay-context-menu .tsg-grid-skip").off(".tsg-grid").on("click.tsg-grid", (evt) => {
      evt.stopPropagation();
    }).on("keypress", (evt) => {
      if (evt.keyCode == 13) {
...
function columnAutoSize(grid, colIndex) {
  ...
  const el = query8(`#grid_${grid.name}_column_${colIndex} .tsg-col-header`)[0];
  ...
  query8(grid.box).find(`.tsg-grid-records td[col="${colIndex}"] > div`, grid.box).each((el2) => {
    const htmlEl = el2;
...
function focus(grid, event2) {
  ...
  grid.hasFocus = true;
  query8(grid.box).removeClass("tsg-inactive").find(".tsg-inactive").removeClass("tsg-inactive");
  setTimeout(() => {
    const txt = query8(grid.box).find(`#grid_${grid.name}_focus`).get(0);
...
function blur(grid, event2) {
  ...
  if (edata.isCancelled === true) return false;
  grid.hasFocus = false;
  query8(grid.box).addClass("tsg-inactive").find(".tsg-selected").addClass("tsg-inactive");
  query8(grid.box).find(".tsg-selection").addClass("tsg-inactive");
  edata.finish();
}
function keydown(grid, event2) {
  ...
  if (edata.isCancelled === true) return;
  if (query8(grid.box).find(".tsg-message").length > 0) {
    if (event2.keyCode == 27) grid.message();
    return;
  }
  let empty = false;
  const records = query8(obj.box).find("#grid_" + obj.name + "_records");
  ...
  const recEL = query8(obj.box).find(`#grid_${obj.name}_rec_${ind >= 0 ? TsUtils.escapeId(obj.records[ind].recid) : "none"}`);
  ...
  switch (key) {
    ...
    case 91: {
      ...
      if (TsUtils.isSafari) {
        ...
        const focus2 = query8(obj.box).find("#grid_" + obj.name + "_focus");
...
function dblClick(grid, recid, event2) {
  ...
  if (column == null && event2.target) {
    let tmp = event2.target;
    if (tmp.tagName.toUpperCase() != "TD") tmp = query8(tmp).closest("td")[0];
    column = parseInt(query8(tmp).attr("col"));
  }
  const index = grid.get(recid, true);
  const rec = index != null ? grid.records[index] : null;
...
function showContextMenu(grid, event2, options) {
  ...
  if (event2 == null) {
    event2 = { offsetX: 0, offsetY: 0, target: query8(grid.box).find(`#grid_${grid.name}_rec_${recid}`)[0] };
  }
...
function expand(grid, recid, noRefresh) {
  ...
  if (Array.isArray(children)) {
    ...
    if (query8(grid.box).find("#grid_" + grid.name + "_rec_" + id + "_expanded_row").length > 0 || grid.show.expandColumn !== true) return false;
    if (rec.TsUi.expanded == "none") return false;
    query8(grid.box).find("#grid_" + grid.name + "_rec_" + id).after(
    ...
    query8(grid.box).find("#grid_" + grid.name + "_frec_" + id).after(
    ...
    if (edata.isCancelled === true) {
      query8(grid.box).find("#grid_" + grid.name + "_rec_" + id + "_expanded_row").remove();
      query8(grid.box).find("#grid_" + grid.name + "_frec_" + id + "_expanded_row").remove();
      return false;
    }
    const row1 = query8(grid.box).find("#grid_" + grid.name + "_rec_" + recid + "_expanded");
    const row2 = query8(grid.box).find("#grid_" + grid.name + "_frec_" + recid + "_expanded");
    ...
    query8(grid.box).find("#grid_" + grid.name + "_rec_" + id).attr("expanded", "yes").addClass("tsg-expanded");
    query8(grid.box).find("#grid_" + grid.name + "_frec_" + id).attr("expanded", "yes").addClass("tsg-expanded");
    query8(grid.box).find("#grid_" + grid.name + "_cell_" + grid.get(recid, true) + "_expand div").html("-");
    rec.TsUi.expanded = true;
    edata.finish();
    grid.resizeRecords();
  }
...
function collapse(grid, recid, noRefresh) {
  ...
  if (Array.isArray(children)) {
    ...
    if (edata.isCancelled === true) return false;
    query8(grid.box).find("#grid_" + grid.name + "_rec_" + id).removeAttr("expanded").removeClass("tsg-expanded");
    query8(grid.box).find("#grid_" + grid.name + "_frec_" + id).removeAttr("expanded").removeClass("tsg-expanded");
    query8(grid.box).find("#grid_" + grid.name + "_cell_" + grid.get(recid, true) + "_expand div").html("+");
    query8(grid.box).find("#grid_" + grid.name + "_rec_" + id + "_expanded").css("height", "0px");
    query8(grid.box).find("#grid_" + grid.name + "_frec_" + id + "_expanded").css("height", "0px");
    setTimeout(() => {
      query8(grid.box).find("#grid_" + grid.name + "_rec_" + id + "_expanded_row").remove();
      query8(grid.box).find("#grid_" + grid.name + "_frec_" + id + "_expanded_row").remove();
...
function sort(grid, field, direction, multiField) {
  ...
  if (!url) {
    ...
    grid.last.vscroll.scrollTop = 0;
    query8(grid.box).find(`#grid_${grid.name}_records`).prop("scrollTop", 0);
...
function resize(grid) {
  const time = Date.now();
  if (!grid.box || query9(grid.box).attr("name") != grid.name) return;
...
function refreshCell(grid, recid, field) {
  ...
  const cell = query9(grid.box).find(`${isSummary ? ".tsg-grid-summary " : ""}#grid_${grid.name}_data_${index}_${col_ind}`);
  if (cell.length == 0) return false;
  cell.replace(grid.getCellHTML(index, col_ind, isSummary));
  return true;
}
function refreshRow(grid, recid, ind = null) {
  let tr1 = query9(grid.box).find("#grid_" + grid.name + "_frec_" + TsUtils.escapeId(recid));
  let tr2 = query9(grid.box).find("#grid_" + grid.name + "_rec_" + TsUtils.escapeId(recid));
  if (tr1.length > 0) {
    ...
    if (typeof st == "string") {
      tr1 = query9(grid.box).find("#grid_" + grid.name + "_frec_" + TsUtils.escapeId(recid));
      tr2 = query9(grid.box).find("#grid_" + grid.name + "_rec_" + TsUtils.escapeId(recid));
...
function refresh(grid) {
  ...
  if (edata.isCancelled === true) return;
  if (grid.show.header) {
    query9(grid.box).find(`#grid_${grid.name}_header`).html(TsUtils.lang(grid.header) + "&#160;").show();
  } else {
    query9(grid.box).find(`#grid_${grid.name}_header`).hide();
  }
  if (grid.show.toolbar) {
    query9(grid.box).find("#grid_" + grid.name + "_toolbar").show();
  } else {
    query9(grid.box).find("#grid_" + grid.name + "_toolbar").hide();
  }
  grid.searchClose();
  ...
  const sInput = query9(grid.box).find("#grid_" + grid.name + "_search_all");
  ...
  grid.refreshBody();
  if (grid.show.footer) {
    query9(grid.box).find(`#grid_${grid.name}_footer`).html(grid.getFooterHTML()).show();
  } else {
    query9(grid.box).find(`#grid_${grid.name}_footer`).hide();
  }
  ...
  if (areAllSelected || areAllSearchedSelected) {
    query9(grid.box).find("#grid_" + grid.name + "_check_all").prop("checked", true);
  } else {
    query9(grid.box).find("#grid_" + grid.name + "_check_all").prop("checked", false);
  }
  grid.status();
    ...
    setTimeout(() => {
      ...
      if (search2.length > 0) {
        search2.forEach((item) => {
          const el = query9(grid.box).find('td[col="' + item.col + '"]:not(.tsg-head)');
...
function refreshSearch(grid) {
  if (grid.multiSearch && grid.searchData.length > 0) {
    if (query9(grid.box).find(".tsg-grid-searches").length == 0) {
      query9(grid.box).find(".tsg-grid-toolbar").css("height", grid.last.toolbar_height + 35 + "px").append(`<div id="grid_${grid.name}_searches" class="tsg-grid-searches"></div>`);
    }
    let searches = `
    ...
    searches += `
        ...
        `;
    query9(grid.box).find(`#grid_${grid.name}_searches`).html(searches);
    query9(grid.box).find(`#grid_${grid.name}_search_logic`).html(TsUtils.lang(grid.last.logic == "AND" ? "All" : "Any"));
  } else {
    query9(grid.box).find(".tsg-grid-toolbar").css("height", grid.last.toolbar_height + "px").find(".tsg-grid-searches").remove();
  }
  if (grid["searchSelected"]) {
    query9(grid.box).find(`#grid_${grid.name}_search_all`).val(" ").prop("readOnly", true);
    query9(grid.box).find(`#grid_${grid.name}_search_name`).show().find(".name-text").html(grid["searchSelected"].text);
  } else {
    query9(grid.box).find(`#grid_${grid.name}_search_all`).prop("readOnly", false);
    query9(grid.box).find(`#grid_${grid.name}_search_name`).hide().find(".name-text").html("");
  }
  TsUtils.bindEvents(query9(grid.box).find(`#grid_${grid.name}_searches .tsg-action, #grid_${grid.name}_searches button`), grid);
}
function refreshBody(grid) {
  ...
  const gridBody = query9(grid.box).find(`#grid_${grid.name}_body`, grid.box).html(bodyHTML);
  const records = query9(grid.box).find(`#grid_${grid.name}_records`, grid.box);
  const frecords = query9(grid.box).find(`#grid_${grid.name}_frecords`, grid.box);
  if (grid.selectType == "row") {
    records.on("mouseover mouseout", { delegate: "tr" }, (event2) => {
      ...
      query9(grid.box).find(`#grid_${grid.name}_frec_${TsUtils.escapeId(recid)}`).toggleClass("tsg-record-hover", event2.type == "mouseover");
    });
    frecords.on("mouseover mouseout", { delegate: "tr" }, (event2) => {
      const ind = query9(event2.delegate).attr("index");
      const recid = grid.records[ind]?.recid;
      query9(grid.box).find(`#grid_${grid.name}_rec_${TsUtils.escapeId(recid)}`).toggleClass("tsg-record-hover", event2.type == "mouseover");
    });
  }
  if (TsUtils.isMobile) {
  ...
  query9(grid.box).find(".tsg-grid-body").off(".body-global").on("click.body-global dblclick.body-global contextmenu.body-global", { delegate: "td.tsg-head" }, (event2) => {
  ...
  if (grid.records.length === 0 && grid.msgEmpty) {
    query9(grid.box).find(`#grid_${grid.name}_body`).append(`<div id="grid_${grid.name}_empty_msg" class="tsg-grid-empty-msg"><div>${TsUtils.lang(grid.msgEmpty)}</div></div>`);
  } else if (query9(grid.box).find(`#grid_${grid.name}_empty_msg`).length > 0) {
    query9(grid.box).find(`#grid_${grid.name}_empty_msg`).remove();
  }
  if (grid.summary.length > 0) {
    const sumHTML = grid.getSummaryHTML();
    query9(grid.box).find(`#grid_${grid.name}_fsummary`).html(sumHTML?.[0] ?? "").show();
    query9(grid.box).find(`#grid_${grid.name}_summary`).html(sumHTML?.[1] ?? "").show();
  } else {
    query9(grid.box).find(`#grid_${grid.name}_fsummary`).hide();
    query9(grid.box).find(`#grid_${grid.name}_summary`).hide();
  }
}
function destroy(grid) {
  ...
  if (query9(grid.box).find(`#grid_${grid.name}_body`).length > 0) {
    grid.unmount();
  }
...
function initColumnDrag(grid, _box) {
  ...
  const hasInvalidClass = (target, lastColumn) => {
    ...
    for (let i = 0; i < iClass.length; i++) {
      if (query9(target).closest(".tsg-head").hasClass(iClass[i])) {
        return true;
      }
    }
    return false;
  ...
  function dragColStart(event2) {
    ...
    if (!query9(event2.target).parents().hasClass("tsg-head") || hasInvalidClass(event2.target)) return;
    ...
    const origColumn = dragData.columnHead = query9(event2.target).closest(".tsg-head");
    ...
    if (edata.isCancelled === true) return false;
    const columns = dragData["columns"] = query9(self.box).find(".tsg-head:not(.tsg-head-last)");
  ...
  function dragColEnd(event2) {
    ...
    let target;
    const finish = () => {
      const ghosts = query9(self.box).find(".tsg-grid-ghost");
      query9(self.box).find(".tsg-intersection-marker").hide();
  ...
  function markIntersection(event2) {
    ...
    if (dragData.targetPos != newPos) {
      const rect1 = query9(self.box).find(".tsg-grid-body").get(0).getBoundingClientRect();
      const rect2 = query9(event2.target).closest("td").get(0).getBoundingClientRect();
      query9(self.box).find(".tsg-intersection-marker").show().css({
  ...
  if (grid.show.toolbarSearch) {
    ...
    grid.toolbar.items.push({
      ...
      onRefresh: async (event2) => {
        await event2.complete;
        const input = query9(grid.box).find(`#grid_${grid.name}_search_all`);
        TsUtils.bindEvents(query9(grid.box).find(`#grid_${grid.name}_search_all, .tsg-action`), grid);
    ...
    switch (event2.detail.item.id) {
      ...
      case "tsg-column-on-off":
        if (event2.detail.subItem) {
          ...
          setTimeout(() => {
            query9(`#w2overlay-${grid.name}_toolbar-drop .tsg-grid-skip`).off(".tsg-grid").on("click.tsg-grid", (evt) => {
              evt.stopPropagation();
            }).on("keypress", (evt) => {
              if (evt.keyCode == 13) {
                grid.skip(evt.target.value);
...
function initResize(grid) {
  const obj = grid;
  query9(grid.box).find(".tsg-resizer").off(".grid-col-resize").on("click.grid-col-resize", function(event2) {
    event2.stopPropagation();
    event2.preventDefault();
...
function resizeBoxes(grid) {
  const header = query9(grid.box).find(`#grid_${grid.name}_header`);
  const toolbar = query9(grid.box).find(`#grid_${grid.name}_toolbar`);
  const fsummary = query9(grid.box).find(`#grid_${grid.name}_fsummary`);
  const summary = query9(grid.box).find(`#grid_${grid.name}_summary`);
  const footer = query9(grid.box).find(`#grid_${grid.name}_footer`);
  const body = query9(grid.box).find(`#grid_${grid.name}_body`);
  if (grid.show.header) {
...
function resizeRecords(grid) {
  const obj = grid;
  query9(grid.box).find(".tsg-empty-record").remove();
  const box = query9(grid.box);
  const gridEl = query9(grid.box).find(":scope > div.tsg-grid-box");
  const header = query9(grid.box).find(`#grid_${grid.name}_header`);
  const toolbar = query9(grid.box).find(`#grid_${grid.name}_toolbar`);
  const summary = query9(grid.box).find(`#grid_${grid.name}_summary`);
  const fsummary = query9(grid.box).find(`#grid_${grid.name}_fsummary`);
  const footer = query9(grid.box).find(`#grid_${grid.name}_footer`);
  const body = query9(grid.box).find(`#grid_${grid.name}_body`);
  const columns = query9(grid.box).find(`#grid_${grid.name}_columns`);
  const fcolumns = query9(grid.box).find(`#grid_${grid.name}_fcolumns`);
  const records = query9(grid.box).find(`#grid_${grid.name}_records`);
  const frecords = query9(grid.box).find(`#grid_${grid.name}_frecords`);
  const scroll1 = query9(grid.box).find(`#grid_${grid.name}_scroll1`);
  ...
  if (body[0]?.clientHeight - (columns[0]?.clientHeight ?? 0) < (query9(records).find(":scope > table")[0]?.clientHeight ?? 0) + (bodyOverflowX ? TsUtils.scrollBarSize() : 0)) {
    bodyOverflowY = true;
  }
  if (!grid.fixedBody) {
    const bodyHeight = TsUtils.getSize(columns, "height") + TsUtils.getSize(query9(grid.box).find("#grid_" + grid.name + "_records table"), "height") + (bodyOverflowX ? TsUtils.scrollBarSize() : 0);
  ...
  columns.find(":scope > table > tbody > tr:nth-child(1) td").add(fcolumns.find(":scope > table > tbody > tr:nth-child(1) td")).each((el) => {
    if (query9(el).hasClass("tsg-col-number")) {
    ...
    if (query9(el).hasClass("tsg-head-last")) {
      if ((obj.last.vscroll.colIndEnd ?? 0) + 1 < obj.columns.length) {
        ...
        query9(el).css("width", TsUtils.scrollBarSize() + (width_diff > 0 && percent === 0 ? width_diff : 0) + "px");
      }
    }
  });
  ...
  records.find(":scope > table > tbody > tr:nth-child(1) td").add(frecords.find(":scope > table > tbody > tr:nth-child(1) td")).each((el) => {
    ...
    if (query9(el).hasClass("tsg-grid-data-last") && query9(el).parents(".tsg-grid-frecords").length === 0) {
      if ((obj.last.vscroll.colIndEnd ?? 0) + 1 < obj.columns.length) {
        let width = 0;
        for (let i = (obj.last.vscroll.colIndEnd ?? 0) + 1; i < obj.columns.length; i++) {
          if (!obj.columns[i] || obj.columns[i].frozen || obj.columns[i].hidden) continue;
...
function columnTooltipShow(grid, ind, _event) {
  const $el = query9(grid.box).find("#grid_" + grid.name + "_column_" + ind);
...
function clipboardCopy(grid, ind, col_ind, summary) {
  ...
  query9(grid.box).find("#grid_" + grid.name + "_focus").text(txt).get(0).select();
...
function showBubble(grid, ind, col_ind, summary) {
  ...
  let html = "";
  const rec = grid.records[ind];
  const el = query9(grid.box).find(`${summary ? ".tsg-grid-summary" : ""} #grid_${grid.name}_data_${ind}_${col_ind} .tsg-info`);
  if (grid.last.bubbleEl) {
    TsTooltip4.hide(grid.name + "-bubble");
  }
...
function scroll(grid, event2) {
  ...
  if (event2) {
    ...
    grid.last.vscroll.scrollTop = sTop;
    grid.last.vscroll.scrollLeft = sLeft;
    const cols = query9(grid.box).find(`#grid_${grid.name}_columns`)[0];
    const summary = query9(grid.box).find(`#grid_${grid.name}_summary`)[0];
  ...
  query9(grid.box).find("#grid_" + grid.name + "_footer .tsg-footer-right").html(
  ...
  if (e + 10 > buffered && grid.last.vscroll.pull_more !== true && (buffered < grid.total - grid.offset || grid.total == -1 && grid.last.fetch.hasMore)) {
    ...
    const more = query9(grid.box).find("#grid_" + grid.name + "_rec_more, #grid_" + grid.name + "_frec_more");
    more.show().eq(1).off(".load-more").on("click.load-more", function() {
      query9(this).find("td").html('<div><div style="width: 20px; height: 20px;" class="tsg-spinner"></div></div>');
  ...
  function markSearch() {
    ...
    obj.last.marker_timer = setTimeout(() => {
      ...
      if (search2.length > 0) {
        search2.forEach((item) => {
          const el = query9(obj.box).find('td[col="' + item.col + '"]:not(.tsg-head)');
          TsUtils.marker(el, item.search);
        });
      }
    }, 50);
