  ...
  on(events, optionsOrCallback, callback) {
    ...
    eventsStr.forEach((eventName) => {
      ...
      if (delegate) {
        const fun = cb;
        cb = (evt) => {
          const parent = query2(evt.target).parents(delegate);
          if (parent.length > 0) {
...
var query2 = function(selector, context) {
...
query2.html = (str) => {
  ...
  return query2(frag.children, frag);
};
query2.version = Query.version;
...
var Utils = class {
  ...
  transition(div_old, div_new, type, callBack) {
    return new Promise((resolve, _reject) => {
      ...
      setTimeout(() => {
        ...
        if (div_new) {
          ;
          query(div_new).css({ "opacity": "1" }).css({ "transition": "", "transform": "" });
        }
        if (div_old) {
          ;
          query(div_old).css({ "opacity": "1" }).css({ "transition": "", "transform": "" });
        }
  ...
  message(where, options) {
    ...
    const closeComplete = (options2) => {
      ...
      if (query(where.box).find(".tsg-message").length <= 1) {
        ...
        query(where.box).find(`#tsg-message-${where.owner?.name}-${options2["msgIndex"] - 1}`).css("z-index", "1500");
      }
      if (focus) {
    ...
    if (msgBase.on == null) {
      const opts = msgBase;
      msgBase = new TsBase();
      TsUtils.extend(msgBase, opts);
    }
    const msgOpts = msgBase;
    msgOpts["on"]("open", (event2) => {
      TsUtils.bindEvents(query(msgOpts["box"]).find(".tsg-eaction"), msgOpts);
      const detail = event2["detail"];
      query(detail["box"]).find("button, input, textarea, [name=hidden-first]").off(".message").on("keydown.message", function(evt) {
        const keyEvt = evt;
    ...
    const head = query(where.box).find(where.after);
    if (!msgBase.tmp) {
      msgBase.tmp = {
    ...
    if (msgBase.html === "" && msgBase.body === "" && msgBase.buttons === "") {
      ...
      msgBase.box = query(where.box).find(`#tsg-message-${where.owner?.name}-${msgBase.msgIndex}`)[0];
      TsUtils.bindEvents(msgBase.box, this);
      query(msgBase.box).addClass("animating");
      ...
      openTimer = setTimeout(() => {
        query(where.box).find(`#tsg-message-${where.owner?.name}-${msgBase.msgIndex}`).removeClass("animating").css({ "transition": "0s" });
        edata?.["finish"]?.();
      }, 300);
    }
    ...
    msgBase.close = () => {
      ...
      query(msgBase.box).addClass("tsg-closing animating").css({
        "transition": "0.15s",
    ...
    msgBase.setFocus = (focus) => {
      ...
      query(where.box).find(".tsg-message").find(sel + ",[name=hidden-first],[name=hidden-last]").off(".keep-focus");
      query(box).find(sel + ",[name=hidden-first],[name=hidden-last]").on("blur.keep-focus", function(_event) {
        setTimeout(() => {
          const focus2 = document.activeElement;
          const inside = focus2 != null && query(box).find(sel).filter(focus2).length > 0;
          const name = query(focus2).attr("name");
  ...
  confirm(where, options) {
    ...
    const prom = TsUtils.message(where, msgOpts);
    if (prom) {
      prom.action((event2) => {
  ...
  prompt(where, options) {
    ...
    if (prom) {
      ...
      prom.action((event2) => {
        ...
        const d = event2["detail"];
        (d?.["self"])["input"] = query(d?.["box"]).find("#TsPrompt").get(0);
        query(d?.["box"]).find("#TsPrompt").on("keydown", (evt) => {
  ...
  notify(text, options) {
    return new Promise((resolve) => {
      ...
      if (textStr) {
        ...
        query(opts["where"]).find("#tsg-notify").find(".tsg-notify-close").on("click", (_event) => {
          query(opts["where"]).find("#tsg-notify").remove();
          resolve();
        });
        if (opts["actions"]) {
          query(opts["where"]).find("#tsg-notify .tsg-notify-link").on("click", (event2) => {
            const value = query(event2.target).attr("value") ?? "";
            opts["actions"][value]();
            query(opts["where"]).find("#tsg-notify").remove();
            resolve();
          });
  ...
  prepareParams(url, fetchOptions, options = {}) {
    const dataType = options?.["dataType"] ?? TsUtils.settings.dataType;
  ...
  bindEvents(selector, subject) {
    ...
    query(normalizedSelector).each((el) => {
      ...
      Object.keys(actions).forEach((name) => {
        ...
        query(el).off(name + ".TsUtils-bind").on(name + ".TsUtils-bind", function(event2) {
          switch (method) {
            case "alert":
              alert(params[0]);
              break;
...
var TsDialog = class extends TsBase {
  ...
  open(options, extraOptions) {
    const self = this;
    if (this.status == "closing" || query3("#tsg-popup").hasClass("animating")) {
      this.close(true);
    }
    ...
    if (query3("#tsg-popup").length === 0) {
      ...
      msg = `
            ...
            `;
      query3("#tsg-popup").html(msg);
      if (options.title) query3("#tsg-popup .tsg-popup-title").append(TsUtils.lang(options.title));
      if (options.buttons) query3("#tsg-popup .tsg-popup-buttons").append(options.buttons);
      if (options.body) query3("#tsg-popup .tsg-popup-body").append(options.body);
      setTimeout(() => {
        ;
        query3("#tsg-popup").css("transition", options.speed + "s").removeClass("tsg-anim-open");
        TsUtils.bindEvents("#tsg-popup .tsg-eaction", this);
        query3("#tsg-popup").find(".tsg-popup-body").show();
        this._promCreated();
      }, 1);
      ...
      this._timer = setTimeout(() => {
        ...
        edata.finish();
        this._promOpened();
        query3("#tsg-popup").removeClass("animating");
      }, options.speed * 1e3);
    } else {
      ...
      const cloned = query3("#tsg-popup .tsg-box").get(0).cloneNode(true);
      query3(cloned).removeClass("tsg-box").addClass("tsg-box-temp").find(".tsg-popup-body").empty().append(options.body);
      query3("#tsg-popup .tsg-box").after(cloned);
      if (options.buttons) {
        ;
        query3("#tsg-popup .tsg-popup-buttons").show().html("").append(options.buttons);
        query3("#tsg-popup .tsg-popup-body").removeClass("tsg-popup-no-buttons");
        query3("#tsg-popup .tsg-box, #tsg-popup .tsg-box-temp").css("bottom", "");
      } else {
        query3("#tsg-popup .tsg-popup-buttons").hide().html("");
        query3("#tsg-popup .tsg-popup-body").addClass("tsg-popup-no-buttons");
        query3("#tsg-popup .tsg-box, #tsg-popup .tsg-box-temp").css("bottom", "0px");
      }
      if (options.title) {
        query3("#tsg-popup .tsg-popup-title").show().html(TsUtils.lang(options.title));
        query3("#tsg-popup .tsg-popup-body").removeClass("tsg-popup-no-title");
        query3("#tsg-popup .tsg-box, #tsg-popup .tsg-box-temp").css("top", "");
      } else {
        query3("#tsg-popup .tsg-popup-title").hide().html("");
        query3("#tsg-popup .tsg-popup-body").addClass("tsg-popup-no-title");
        query3("#tsg-popup .tsg-box, #tsg-popup .tsg-box-temp").css("top", "0px");
      }
      if (titleBtns) {
        query3("#tsg-popup .tsg-popup-title-btns").show().html(titleBtns);
      } else {
        query3("#tsg-popup .tsg-popup-title-btns").hide();
      }
      const div_old = query3("#tsg-popup .tsg-box")[0];
      const div_new = query3("#tsg-popup .tsg-box-temp")[0];
      query3("#tsg-popup").addClass("animating");
      TsUtils.transition(div_old, div_new, options.transition, () => {
        query3(div_old).remove();
        query3(div_new).removeClass("tsg-box-temp").addClass("tsg-box");
        const $body = query3(div_new).find(".tsg-popup-body");
        if ($body.length == 1) {
    ...
    if (options.keyboard) {
      query3(document.body).off(".TsPopup").on("keydown.TsPopup", (event2) => {
        this.keydown(event2);
      });
    }
    ...
    query3("#tsg-popup .tsg-popup-title").off("mousedown").on("mousedown", function(event2) {
      if (!self.options.maximized) mvStart(event2);
    });
    if (options.resizable) {
      query3("#tsg-popup .tsg-popup-resizer").show();
      query3("#tsg-popup .tsg-popup-resizer").off("mousedown").on("mousedown", (event2) => {
        mvStart(event2, true);
      });
    } else {
      query3("#tsg-popup .tsg-popup-resizer").hide();
    }
    return prom;
    function mvStart(evt, resizer) {
      if (!evt) evt = window.event;
      self.status = resizer ? "resizing" : "moving";
      const rect = query3("#tsg-popup").get(0).getBoundingClientRect();
      Object.assign(tmp, {
        changing: true,
        isLocked: query3("#tsg-popup > .tsg-lock").length == 1 ? true : false,
        x: evt.screenX,
        y: evt.screenY,
        pos_x: rect.x,
        pos_y: rect.y,
      ...
      query3(document.body).on("mousemove.tsg-popup", tmp.mvMove).on("mouseup.tsg-popup", tmp.mvStop);
    ...
    function mvStop(evt) {
      ...
      tmp.changing = false;
      self.status = "open";
      query3(document.body).off(".tsg-popup");
  ...
  keydown(event2) {
    ...
    switch (event2.keyCode) {
      case 27:
        event2.preventDefault();
        if (query3("#tsg-popup .tsg-message").length == 0) {
  ...
  close(immediate) {
    ...
    if (query3("#tsg-popup").length === 0 || this.status == "closed") {
      return;
    }
    ...
    this.status = "closing";
    query3("#tsg-popup").css("transition", this.options.speed + "s").addClass("tsg-anim-close animating");
  ...
  clear() {
    query3("#tsg-popup .tsg-popup-title").html("");
    query3("#tsg-popup .tsg-popup-body").html("");
    query3("#tsg-popup .tsg-popup-buttons").html("");
  }
  reset() {
  ...
  lock(msg, showSpinner) {
    TsUtils.lock(query3("#tsg-popup"), msg, showSpinner);
  }
  // any: callback parameter — caller signature varies; TsPopup options accept untyped user payloads at runtime
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  unlock(speed) {
    TsUtils.unlock(query3("#tsg-popup"), speed);
  }
  // any: callback parameter — caller signature varies; TsPopup options accept untyped user payloads at runtime
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  // internal function
  resizeMessages() {
    query3("#tsg-popup .tsg-message").each((node) => {
      const msg = node;
      const mopt = msg._msg_options;
      const popup = query3("#tsg-popup");
...
function TsAlert(msg, title, callBack) {
  ...
  if (query3("#tsg-popup").length > 0 && TsPopup.status != "closing") {
...
function TsPrompt(label, title, callBack) {
  ...
  if (prom.self.box) {
    prom.self["input"] = query3(prom.self.box).find("#TsPrompt").get(0);
  } else {
    prom.self["input"] = query3("#tsg-popup .tsg-popup-body #TsPrompt").get(0);
  }
  ...
  prom.self.off(".prompt").on("open:after.prompt", (event2) => {
    const box = event2.detail.box ? event2.detail.box : query3("#tsg-popup .tsg-popup-body").get(0);
    TsUtils.bindEvents(query3(box).find("#TsPrompt"), {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      keydown(evt) {
    ...
    query3(box).find(".tsg-eaction").trigger("keyup");
...
var Tooltip = class _Tooltip {
  ...
  show(name, extraOptions) {
    ...
    if (options.html === "" || options.html == null) {
      ...
      query4(overlay.box).find(".tsg-overlay-body").attr("style", (options.style || "") + "; " + overlayStyles).removeClass(null).addClass("tsg-overlay-body " + options.class + (options.draggable ? " tsg-draggable" : "")).html(options.html);
      this.resize(overlay.name);
    } else {
      ...
      overlay.box = query4("#" + TsUtils.escapeId(overlay.id))[0];
    ...
    query4(overlay.box).css("opacity", 1).find(".tsg-overlay-body").html(options.html);
    setTimeout(() => {
      query4(overlay.box).css({ "pointer-events": "auto" }).data("ready", "yes");
    }, 100);
    TsUtils.bindEvents(query4(overlay.box).find(".tsg-eaction"), this);
    delete overlay.needsUpdate;
    overlay.box.overlay = overlay;
    query4(overlay.box).off("mousedown.tsg-bringfront").on("mousedown.tsg-bringfront", () => {
  ...
  startDrag(event2) {
    ...
    const el = query4(event2.target).closest(".tsg-overlay");
    ...
    query4(document).off(".tsg-drag").on("selectstart.tsg-drag, dragstart.tsg-drag", (e) => e.preventDefault()).find("body").addClass("tsg-overlay-dragging");
    query4("html").off(".TsColor").on("mousemove.TsColor", mouseMove).on("mouseup.TsColor", mouseUp);
    function mouseUp(_event) {
      query4("html").off(".TsColor");
      query4(document).off("selectstart.tsg-drag");
      query4(document).off("dragstart.tsg-drag");
      query4(document.body).removeClass("tsg-overlay-dragging");
      if (initial["moved"]) {
...
var ColorTooltip = class _ColorTooltip extends Tooltip {
  ...
  attach(anchor, text) {
    ...
    overlay.on("show:after.attach", (_event) => {
      if (ret.overlay?.box) {
        const actions = query4(ret.overlay.box).find(".tsg-eaction");
  ...
  select(color, name) {
    ...
    if (typeof name != "string") {
      ...
      name = query4(target).closest(".tsg-overlay").attr("name");
    }
    ...
    query4(overlay.box).find(".tsg-color.tsg-selected").removeClass("tsg-selected");
    if (target) {
      query4(target).addClass("tsg-selected");
    }
    edata.finish();
  }
  ...
  tabClick(index, name) {
    if (typeof name != "string") {
      name = query4(name.target).closest(".tsg-overlay").attr("name");
    }
    const overlay = this.get(name);
    const tab = query4(overlay.box).find(`.tsg-color-tab:nth-child(${index})`);
    query4(overlay.box).find(".tsg-color-tab").removeClass("tsg-selected");
    query4(tab).addClass("tsg-selected");
    query4(overlay.box).find(".tsg-tab-content").hide().closest(".tsg-colors").find(".tab-" + index).show();
  }
  ...
  initControls(overlay) {
    ...
    query4(overlay.box).off(".TsColor").on("contextmenu.TsColor", (event2) => {
      event2.preventDefault();
    ...
    query4(overlay.box).find(".color-original").off(".TsColor").on("click.TsColor", (event2) => {
      const tmp = TsUtils.parseColor(query4(event2.target).css("background-color"));
      if (tmp != null) {
        rgb = tmp;
        hsv = TsUtils.rgb2hsv(rgb);
        setColor(hsv, true);
    ...
    query4(overlay.box).find(".palette, .rainbow, .alpha").off(".TsColor").on(`${mDown}.TsColor`, mouseDown);
    ...
    function setColor(color2, fullUpdate, initial2) {
      ...
      if (initial2 != null) {
        ...
        query4(overlay.box).find(".tsg-color.tsg-selected").removeClass("tsg-selected");
        query4(overlay.box).find(`.tsg-colors [name="${color3}"], .tsg-colors [name="${initial2}"]`).addClass("tsg-selected");
    ...
    function mouseDown(event2) {
      ...
      mouseMove(event2);
      query4("html").off(".TsColor").on(mMove, mouseMove).on(mUp, mouseUp);
    }
    function mouseUp(_event) {
      query4("html").off(".TsColor");
    }
  ...
  async pickAndSelect(name, event2) {
    ...
    if (typeof color == "string" && color.substr(0, 1) == "#" && [7, 9].includes(color.length)) {
      ...
      const cnt = query4(event2.target).closest(".tsg-colors-custom");
...
var MenuTooltip = class extends Tooltip {
  ...
  attach(anchor, text) {
    ...
    overlay.click = () => {
      query4(overlay.box).find(".tsg-selected").each((el) => {
        el.click();
      });
    };
  ...
  initControls(overlay) {
    ...
    query4(overlay.box).find(".tsg-menu:not(.tsg-sub-menu)").off(".TsMenu").on("contextmenu.TsMenu", (event2) => {
      event2.preventDefault();
    }).on(`${mdown}.TsMenu`, { delegate: ".tsg-menu-item" }, (event2) => {
      const dt = event2.delegate.dataset;
      const parents = query4(event2.delegate).closest(".tsg-menu").data("parents");
      ...
      const _menu = query4(event2.target).closest(".tsg-menu").get(0);
    ...
    if (["INPUT", "TEXTAREA"].includes(overlay.anchor.tagName)) {
      query4(overlay.anchor).off(".TsMenu").on("input.TsMenu", (_event) => {
    ...
    if (overlay.options.search) {
      query4(overlay.box).find("#menu-search").off(".TsMenu").on("keyup.TsMenu", (event2) => {
  ...
  refreshIndex(name, instant) {
    ...
    const view = query4(overlay.box).find(".tsg-overlay-body").get(0);
    const search = query4(overlay.box).find(".tsg-menu-search, .tsg-menu-top").get(0);
    query4(overlay.box).find(".tsg-menu-item.tsg-selected").removeClass("tsg-selected");
    const el = query4(overlay.box).find(`.tsg-menu-item[index="${overlay.selected}"]`).addClass("tsg-selected").get(0);
    if (el) {
  ...
  showTooltip(name, options) {
    ...
    const anchor = options?.anchor ?? query4(overlay.box).find(`.tsg-menu-item[index="${overlay.selected}"]`).get(0);
  ...
  refreshSearch(name) {
    ...
    TsTooltip.hide(overlay.name + "-tooltip");
    query4(overlay.box).find(".tsg-no-items").hide();
    query4(overlay.box).find(".tsg-menu-item, .tsg-menu-divider").each((el) => {
    ...
    query4(overlay.box).find(".tsg-sub-menu").each((sub) => {
      const hasItems = query4(sub).find(".tsg-menu-item").get().some((el) => {
    ...
    if (overlay.tmp["searchCount"] == 0 || (overlay.options?.items?.length ?? 0) == 0) {
      if (query4(overlay.box).find(".tsg-no-items").length == 0) {
        query4(overlay.box).find(".tsg-menu:not(.tsg-sub-menu)").append(`
                    <div class="tsg-no-items">
                        ${TsUtils.lang(overlay.options.msgNoItems)}
                    </div>`);
      }
      query4(overlay.box).find(".tsg-no-items").show();
    }
  }
  /**
  ...
  applyFilter(name, items, search, debounce) {
    ...
    if (items == null && options.url && remote.hasMore && remote.search !== search) {
      ...
      query4(overlay.box).find(".tsg-no-items").html(msg);
  ...
  menuDown(overlay, event2, index, parents) {
    const options = overlay.options;
    let items = options.items;
    const icon = query4(event2.delegate).find(".tsg-icon");
    const menu = query4(event2.target).closest(".tsg-menu:not(.tsg-sub-menu)");
    ...
    if ((options.type === "check" || options.type === "radio") && item.group !== false && !query4(event2.target).hasClass("menu-remove") && !query4(event2.target).hasClass("menu-help") && !query4(event2.target).closest(".tsg-menu-item").hasClass("has-sub-menu")) {
      item.checked = options.type == "radio" ? true : !item.checked;
      if (item.checked) {
        if (options.type === "radio") {
          query4(event2.target).closest(".tsg-menu").find(".tsg-icon").removeClass("tsg-icon-check").addClass("tsg-icon-empty");
        }
    ...
    if (!query4(event2.target).hasClass("menu-remove") && !query4(event2.target).hasClass("menu-help")) {
      ...
      if (!query4(event2.delegate).hasClass("has-sub-menu")) {
        query4(event2.delegate).addClass("tsg-selected");
      }
    }
  }
  // any: callback parameter — caller signature varies; TsTooltip overlay options merge from multiple user sources at runtime
  ...
  menuClick(overlay, event2, index, parents) {
    const options = overlay.options;
    let items = options.items;
    const $item = query4(event2.delegate).closest(".tsg-menu-item");
  ...
  keyUp(overlay, event2) {
    ...
    switch (event2.keyCode) {
      ...
      case 13: {
        ...
        event2.delegate = query4(overlay.box).find(".tsg-selected").get(0);
      ...
      case 37: {
        ...
        if (Array.isArray(item?.items) && item.items.length > 0 && item.expanded) {
          event2.delegate = query4(overlay.box).find(`.tsg-menu-item[index="${index}"]`).get(0);
          overlay.selected = index;
          this.menuClick(overlay, event2, parseInt(String(index)), parents);
        }
        filter = false;
...
var DateTooltip = class extends Tooltip {
  ...
  initControls(overlay) {
    ...
    const moveMonth = (inc) => {
      ...
      Object.assign(overlay.tmp, cal);
      query4(overlay.box).find(".tsg-overlay-body").html(cal.html);
      this.initControls(overlay);
    };
    const checkJump = (event2, dblclick) => {
      query4(event2.target).parent().find(".tsg-jump-month, .tsg-jump-year").removeClass("tsg-selected");
      query4(event2.target).addClass("tsg-selected");
    ...
    query4(overlay.box).find(".tsg-cal-title").off(".calendar").on("click.calendar", (event2) => {
      ...
      if (overlay.tmp.jump) {
        ...
        query4(overlay.box).find(".tsg-overlay-body .tsg-cal-days").replace(this.getYearHTML());
    ...
    query4(overlay.box).find(".tsg-cal-now").off(".calendar").on("click.calendar", (_event) => {
    ...
    query4(overlay.box).off(".calendar").on("contextmenu.calendar", (event2) => {
      ...
      if (options.type == "datetime") {
        ...
        query4(overlay.box).find(".tsg-overlay-body").html(this.getHourHTML(overlay.options).html);
        this.initControls(overlay);
      } else {
      ...
      if (options.noMinutes) {
        ...
        query4(overlay.box).find(".tsg-overlay-body").html(html);
        this.initControls(overlay);
      }
...
var TsToolbar = class extends TsBase {
  ...
  constructor(options) {
    ...
    options.items = items;
    if (typeof this.box == "string") this.box = query5(this.box).get(0);
  ...
  remove(...args) {
    ...
    args.forEach((item) => {
      ...
      effected++;
      query5(this.box).find("#tb_" + this.name + "_item_" + TsUtils.escapeId(it.id)).remove();
  ...
  setCount(id, count, className, style) {
    const btn = query5(this.box).find(`#tb_${this.name}_item_${TsUtils.escapeId(id)} .tsg-tb-count > span`);
    if (btn.length > 0) {
  ...
  click(id, event2) {
    ...
    if (it && !it.disabled) {
      ...
      query5(this.box).find(btn).removeClass("down");
      if (it.type == "radio") {
        ...
        it.checked = true;
        query5(this.box).find(btn).addClass("checked");
      }
      if (["menu", "menu-radio", "menu-check", "drop", "color", "text-color"].includes(it.type)) {
        ...
        if (it.checked) {
          ...
          setTimeout(() => {
            ...
            const el = query5(this.box).find("#tb_" + this.name + "_item_" + TsUtils.escapeId(it.id));
      ...
      if (["check", "menu", "menu-radio", "menu-check", "drop", "color", "text-color"].includes(it.type)) {
        ...
        if (it.checked) {
          ...
          query5(this.box).find(btn).removeClass("checked");
        }
      }
      if (it.route) {
  ...
  scroll(direction, line, instant) {
    return new Promise((resolve, _reject) => {
      const scrollBox = query5(this.box).find(`.tsg-tb-line:nth-child(${line}) .tsg-scroll-wrapper`);
  ...
  render(box) {
    const time = Date.now();
    if (typeof box == "string") box = query5(box).get(0);
    ...
    query5(this.box).attr("name", this.name).addClass("tsg-reset tsg-toolbar").html(html);
    if (query5(this.box).length > 0) {
      query5(this.box)[0].style.cssText += this["style"];
    }
    TsUtils.bindEvents(query5(this.box).find(".tsg-tb-line .tsg-eaction"), this);
  ...
  refresh(id) {
    ...
    const btn = query5(this.box).find(selector);
    const html = this.getItemHTML(it);
    this.tooltipHide(id);
    if (it.type == "spacer") {
      query5(this.box).find(`.tsg-tb-line:nth-child(${it.line ?? 1})`).find(".tsg-tb-right").css("width", "auto");
    }
    if (btn.length === 0) {
      const next = parseInt(this.get(id, true)) + 1;
      let $next = query5(this.box).find(`#tb_${this.name}_item_${TsUtils.escapeId(this.items[next] ? this.items[next].id : "--")}`);
      if ($next.length == 0) {
        $next = query5(this.box).find(`.tsg-tb-line:nth-child(${it.line}`).find(".tsg-tb-right").before(html);
      } else {
        $next.after(html);
      }
      TsUtils.bindEvents(query5(this.box).find(`${selector}, ${selector} .tsg-eaction`), this);
    } else {
      query5(this.box).find(selector).replace(query2.html(html));
      const newBtn = query5(this.box).find(selector);
  ...
  resize() {
    ...
    if (edata.isCancelled === true) return;
    query5(this.box).find(".tsg-tb-line").each((el) => {
      const box = query5(el);
  ...
  destroy() {
    ...
    if (edata.isCancelled === true) return;
    if (query5(this.box).find(".tsg-scroll-wrapper").length > 0) {
      this.unmount();
    }
  ...
  change(id, value, dynamic) {
    const it = this.get(id);
    const input = query5(this.box).find("#tb_" + this.name + "_item_" + TsUtils.escapeId(id)).find("input.tsg-toolbar-input");
  ...
  tooltipShow(id) {
    ...
    const el = query5(this.box).find("#tb_" + this.name + "_item_" + TsUtils.escapeId(id)).get(0);
    const item = this.get(id);
    ...
    switch (action) {
      case "Enter":
        if (!["label", "input"].includes(btn.type)) {
          query5(target).addClass("over");
        }
        this.tooltipShow(id);
        break;
      case "Leave":
        if (!["label", "input"].includes(btn.type)) {
          query5(target).removeClass("over down");
        }
        this.tooltipHide(id);
        break;
      case "Down":
        if (!["label", "input"].includes(btn.type)) {
          query5(target).addClass("down");
        }
        break;
      case "Up":
        if (!["label", "input"].includes(btn.type)) {
          query5(target).removeClass("down");
        }
        break;
    }
    edata.finish();
...
var TsSidebar = class extends TsBase {
  ...
  setCount(id, count, options = {}) {
    ...
    const btn = query6(this.box).find(`#node_${TsUtils.escapeId(id)} .tsg-node-badge`);
    if (btn.length > 0) {
  ...
  select(id) {
    ...
    const $el = query6(this.box).find("#node_" + TsUtils.escapeId(id));
  ...
  unselect(id) {
    ...
    query6(this.box).find("#node_" + TsUtils.escapeId(id)).removeClass("tsg-selected").find(".tsg-icon").removeClass("tsg-icon-selected");
  ...
  collapse(id) {
    ...
    if (edata.isCancelled === true) return;
    query6(this.box).find("#node_" + TsUtils.escapeId(id) + "_sub").hide();
    query6(this.box).find("#node_" + TsUtils.escapeId(id) + " .tsg-expanded").removeClass("tsg-expanded").addClass("tsg-collapsed");
    nd.expanded = false;
    edata.finish();
    this.refresh(id);
    return true;
  ...
  expand(id) {
    ...
    if (edata.isCancelled === true) return;
    query6(this.box).find("#node_" + TsUtils.escapeId(id) + "_sub").show();
    query6(this.box).find("#node_" + TsUtils.escapeId(id) + " .tsg-collapsed").removeClass("tsg-collapsed").addClass("tsg-expanded");
    nd.expanded = true;
    edata.finish();
    this.refresh(id);
    return true;
  ...
  click(id, event2) {
    ...
    const newNode = query6(this.box).find("#node_" + TsUtils.escapeId(id));
  ...
  flatMenu(el, items) {
    const self = this;
    const $el = query6(el).find(".tsg-node-data");
    TsMenu.show({
  ...
  focus(event2) {
    ...
    this.hasFocus = true;
    query6(this.box).find(".tsg-sidebar-body").addClass("tsg-focus");
    setTimeout(() => {
      const input = query6(this.box).find("#sidebar_" + this.name + "_focus").get(0);
  ...
  blur(event2) {
    ...
    if (edata.isCancelled === true) return false;
    this.hasFocus = false;
    query6(this.box).find(".tsg-sidebar-body").removeClass("tsg-focus");
    edata.finish();
  }
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  next(node, noSubs) {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  inView(id) {
    const item = query6(this.box).find("#node_" + TsUtils.escapeId(id)).get(0);
    if (!item) {
      return false;
    }
    const div = query6(this.box).find(".tsg-sidebar-body").get(0);
    if (!div) return false;
  ...
  mouseDown(id, event2) {
    ...
    if (this.reorder) {
      ...
      const mv = this.last.move;
      const body = query6(this.box).find(".tsg-sidebar-body");
      if (!mv.ghost) {
        const node = query6(this.box).find(`#node_${TsUtils.escapeId(id)}`);
        mv.offsetY = event2.offsetY;
        mv.target = id;
        const nodeEl = node.get(0);
        mv.pos = { top: nodeEl.offsetTop - 1, left: nodeEl.offsetLeft };
        const clone = query6(node.find(".tsg-node-data").get(0).cloneNode(true));
        mv.node = node;
        mv.nodeSub = node.next();
        body.append('<div id="sidebar_' + this.name + '_ghost" class="tsg-node tsg-ghost"></div>');
        query6(this.box).find("#sidebar_" + this.name + "_ghost").append(clone);
        mv.ghost = query6(this.box).find("#sidebar_" + this.name + "_ghost");
        ...
        mv.resetReorder = () => {
          this.last.move = null;
          query6(this.box).find(`#sidebar_${this.name}_ghost`).remove();
          query6(document).off(`.tsg-${this.name}-reorder`);
        };
      }
      query6(document).on(`mousemove.tsg-${this.name}-reorder`, _mouseMove).on(`mouseup.tsg-${this.name}-reorder`, _mouseStop);
    }
    function _mouseMove(event3) {
      ...
      const over = query6(event3.target).closest(".tsg-node, .tsg-node-group");
      const id2 = over.attr("data-id");
      if (query6(event3.target).hasClass("tsg-sidebar-body") && event3.layerY > 5 && !mv.append) {
        ...
        const el = query6(self.box).find(`#node_${TsUtils.escapeId(id2)}`);
        el.before(mv.node);
  ...
  render(box) {
    ...
    if (!this.box) return;
    query6(this.box).attr("name", this.name).addClass("tsg-reset tsg-sidebar").html(`<div>
    ...
    const boxEl3 = query6(this.box).get(0);
    const rect = boxEl3.getBoundingClientRect();
    query6(this.box).find(":scope > div").css({
      width: rect.width + "px",
      height: rect.height + "px"
    });
    boxEl3.style.cssText += this.style;
    let kbd_timer;
    query6(this.box).find("#sidebar_" + this.name + "_focus").on("focus", function(event2) {
    ...
    query6(this.box).off("mousedown").on("mousedown", function(event2) {
      setTimeout(() => {
        if (["INPUT", "TEXTAREA", "SELECT"].indexOf(event2.target?.tagName?.toUpperCase()) == -1) {
          const $input = query6(obj.box).find("#sidebar_" + obj.name + "_focus");
    ...
    if (this["topHTML"] !== "" || flatHTML !== "") {
      query6(this.box).find(".tsg-sidebar-top").html(this["topHTML"] + flatHTML);
      query6(this.box).find(".tsg-sidebar-body").css("top", query6(this.box).find(".tsg-sidebar-top").get(0)?.clientHeight + "px");
      query6(this.box).find(".tsg-flat").off("click").on("click", (_event) => {
        this.goFlat();
      });
    }
    if (this["bottomHTML"] !== "") {
      query6(this.box).find(".tsg-sidebar-bottom").html(this["bottomHTML"]);
      query6(this.box).find(".tsg-sidebar-body").css("bottom", query6(this.box).find(".tsg-sidebar-bottom").get(0)?.clientHeight + "px");
    }
    this.last.observeResize = new ResizeObserver(() => {
      this.resize();
    });
  ...
  update(id, options = {}) {
    ...
    let level;
    if (nd) {
      const $el = query6(this.box).find("#node_" + TsUtils.escapeId(nd.id));
      if (nd.group) {
        if (options.text) {
  ...
  refresh(id, options = {}) {
    ...
    const body = query6(this.box).find(":scope > div > .tsg-sidebar-body").get(0);
    ...
    if (this.flatButton == true) {
      query6(this.box).find(".tsg-sidebar-top .tsg-flat").show().removeClass("tsg-flat-left tsg-flat-right").addClass(` tsg-flat-${this.flat ? "right" : "left"}`);
    } else {
      query6(this.box).find(".tsg-sidebar-top .tsg-flat").hide();
    }
    const boxEl2 = query6(this.box).get(0);
    query6(this.box).find(":scope > div").removeClass("tsg-sidebar-flat").addClass(this.flat ? "tsg-sidebar-flat" : "").css({
    ...
    if (node !== this) {
      ...
      query6(this.box).find(nodeId).before('<div id="sidebar_' + this.name + '_tmp"></div>');
      query6(this.box).find(nodeId).remove();
      query6(this.box).find(nodeSubId).remove();
      query6(this.box).find("#sidebar_" + this.name + "_tmp").before(nodeHTML);
      query6(this.box).find("#sidebar_" + this.name + "_tmp").remove();
    }
    const div = query6(this.box).find(":scope > div").get(0);
    const scroll = {
      top: div?.scrollTop,
      left: div?.scrollLeft
    };
    const cnt = node == this ? query6(this.box).find(":scope > div > .tsg-sidebar-body") : query6(body).find(nodeSubId);
    cnt.html("");
    ...
    if (!options.recursive) {
      const els = query6(this.box).find(`${nodeId}, ${nodeId} .tsg-eaction, ${nodeSubId} .tsg-eaction`);
      TsUtils.bindEvents(els, this);
      query6(body).prop({ scrollLeft, scrollTop });
    }
    edata.finish();
    return Date.now() - time;
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  showPlus(el, color) {
    query6(el).find("span:nth-child(1)").css("color", color);
  }
  resize() {
    const time = Date.now();
    const edata = this.trigger("resize", { target: this.name });
    if (edata.isCancelled === true) return;
    if (this.box != null) {
      const boxEl = query6(this.box).get(0);
      const rect = boxEl.getBoundingClientRect();
      query6(this.box).css("overflow", "hidden");
      query6(this.box).find(":scope > div").css({
        width: rect.width + "px",
        height: rect.height + "px"
      });
  ...
  destroy() {
    const edata = this.trigger("destroy", { target: this.name });
    if (edata.isCancelled === true) return;
    if (query6(this.box).find(".tsg-sidebar-body").length > 0) {
      this.unmount();
    }
    delete TsUi[this.name];
    edata.finish();
...
var TsTabs = class extends TsBase {
  ...
  remove(...ids) {
    ...
    ids.forEach((it) => {
      ...
      query7(this.box).find(`#tabs_${this.name}_tab_${TsUtils.escapeId(tab.id)}`).remove();
    });
    this.resize();
    return effected;
  }
  ...
  dragMove(event2) {
    ...
    const $el = query7(this.box).find("#tabs_" + this.name + "_tab_" + TsUtils.escapeId(tab.id));
    if (info.divX > 0 && next) {
      const $nextEl = query7(this.box).find("#tabs_" + this.name + "_tab_" + TsUtils.escapeId(next.id));
    ...
    if (info.divX < 0 && prev) {
      const $prevEl = query7(this.box).find("#tabs_" + this.name + "_tab_" + TsUtils.escapeId(prev.id));
      let width1 = $el.get(0).clientWidth;
  ...
  tooltipShow(id) {
    const tab = this.get(id);
    const el = query7(this.box).find("#tabs_" + this.name + "_tab_" + TsUtils.escapeId(id)).get(0);
  ...
  refresh(id) {
    const time = Date.now();
    if (this.flow == "up") {
      query7(this.box).addClass("tsg-tabs-up");
    } else {
      query7(this.box).removeClass("tsg-tabs-up");
    }
    ...
    if (id == null) {
      ...
      const $tab = query7(this.box).find(selector);
      const tabHTML = this.getTabHTML(id);
      if ($tab.length === 0) {
        if (tabHTML) query7(this.box).find("#tabs_" + this.name + "_right").before(tabHTML);
      } else {
        if (query7(this.box).find(".tab-animate-insert").length == 0) {
          if (tabHTML) $tab.replace(tabHTML);
        }
      }
      TsUtils.bindEvents(query7(this.box).find(`${selector}, ${selector} .tsg-eaction`), this);
    }
    query7(this.box).find("#tabs_" + this.name + "_right").html(this.right);
    edata.finish();
    return Date.now() - time;
  }
  // any: callback parameter — caller signature varies; TsTabs tab item shape is user-defined at runtime
  ...
  render(box) {
    ...
    query7(this.box).attr("name", this.name).addClass("tsg-reset tsg-tabs").html(html);
    if (query7(this.box).length > 0) {
      query7(this.box)[0].style.cssText += this.style;
    }
    TsUtils.bindEvents(query7(this.box).find(".tsg-eaction"), this);
    this.last.observeResize = new ResizeObserver(() => {
      this.resize();
    });
    this.last.observeResize.observe(this.box);
  ...
  initReorder(id, event2) {
    ...
    const self = this;
    const $tab = query7(this.box).find("#tabs_" + this.name + "_tab_" + TsUtils.escapeId(id));
    const tabIndex = this.get(id, true);
    const $ghost = query7($tab.get(0).cloneNode(true));
    let edata;
    ...
    this.last.moving = {
      ...
      parentX: query7(this.box).get(0).getBoundingClientRect().left,
      x: event2.pageX,
      opacity: $tab.css("opacity")
    };
    query7(document).off(".w2uiTabReorder").on("mousemove.w2uiTabReorder", function(event3) {
      ...
      if (!self.last.reordering) {
        ...
        $tab.css("opacity", 0);
        query7(self.box).find(".tsg-scroll-wrapper").append($ghost.get(0));
        query7(self.box).find(".tsg-tab-close").hide();
      }
  ...
  scroll(direction, instant) {
    return new Promise((resolve, _reject) => {
      const scrollBox = query7(this.box).find(".tsg-scroll-wrapper");
      const scrollBoxEl = scrollBox.get(0);
      const scrollLeft = scrollBoxEl.scrollLeft;
  ...
  scrollIntoView(id, instant) {
    return new Promise((resolve, _reject) => {
      ...
      const tab = this.get(id);
      if (tab == null) return;
      const tabEl = query7(this.box).find("#tabs_" + this.name + "_tab_" + TsUtils.escapeId(id)).get(0);
  ...
  destroy() {
    const edata = this.trigger("destroy", { target: this.name });
    if (edata.isCancelled === true) return;
    if (query7(this.box).find("#tabs_" + this.name + "_right").length > 0) {
      this.unmount();
    }
    delete TsUi[this.name];
    edata.finish();
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  click(id, event2) {
    if (event2 && query7(event2.target).hasClass("tsg-tab-close")) {
      return;
    }
    const tab = this.get(id);
    ...
    if (edata.isCancelled === true) return;
    query7(this.box).find("#tabs_" + this.name + "_tab_" + TsUtils.escapeId(this.active)).removeClass("active");
    this.active = tab.id;
    query7(this.box).find("#tabs_" + this.name + "_tab_" + TsUtils.escapeId(this.active)).addClass("active");
  ...
  animateInsert(id, tab) {
    return new Promise((resolve, _reject) => {
      let $before = query7(this.box).find("#tabs_" + this.name + "_tab_" + TsUtils.escapeId(id));
      const tabHTML = this.getTabHTML(tab.id);
      const $tab = query2.html(tabHTML);
      if ($before.length == 0) {
        $before = query7(this.box).find("#tabs_tabs_right");
        $before.before($tab);
        this.resize();
      } else {
        $tab.css({ opacity: 0 });
        query7(this.box).find("#tabs_tabs_right").before($tab.get(0));
        const $tmp = query7(this.box).find("#" + $tab.attr("id"));
        const width = $tmp.get(0)?.clientWidth ?? 0;
        const $anim = query2.html('<div class="tab-animate-insert" style="flex-shrink: 0; width: 0; transition: width 0.25s"></div>');
        $before.before($anim);
        $tab.hide();
        $anim.before($tab[0]);
        setTimeout(() => {
...
var TsLayout = class extends TsBase {
  ...
  html(panel, data, transition) {
    ...
    if (panel == "css") {
      query8(this.box).find("#layout_" + this.name + "_panel_css").html("<style>" + data + "</style>");
      promise.status = true;
      return promise;
    }
    if (p == null) {
    ...
    const current = query8(this.box).find(pname + '> [data-role="panel-content"]');
    let panelTop = 0;
    if (current.length > 0) {
      ;
      query8(this.box).find(pname).get(0).scrollTop = 0;
      panelTop = query8(current).css("top");
    }
    ...
    if (p.html === "") {
      ...
      if (!p.hidden) {
        if (transition != null && transition !== "") {
          query8(this.box).addClass("animating");
          const div1 = query8(this.box).find(pname + '> [data-role="panel-content"]');
          div1.after('<div class="tsg-panel-content new-panel" data-role="panel-content" style="' + div1[0].style.cssText + '"></div>');
          const div2 = query8(this.box).find(pname + '> [data-role="panel-content"].new-panel');
          ...
          switch (transition) {
            case "slide-left":
              style1 = "left: -" + TsUtils.getSize(query8(this.box), "width") + "px";
              style2 = "left: 0px";
              break;
            case "slide-right":
              style1 = "left: " + TsUtils.getSize(query8(this.box), "width") + "px";
              ...
              break;
            case "slide-down":
              style1 = "top: -" + TsUtils.getSize(query8(this.box), "height") + "px";
          ...
          setTimeout(() => {
            query8(this.box).removeClass("animating");
            div1.remove();
            div2.removeClass("new-panel current");
            query8(this.box).find(pname + '> [data-role="panel-content"]').css({ "cssText": "" });
            edata.finish();
          }, 500);
        } else {
          this.refresh(panel);
  ...
  message(panel, options) {
    const p = this.get(panel);
    const box = query8(this.box).find("#layout_" + this.name + "_panel_" + p.type);
  ...
  sizeTo(panel, size, instant) {
    ...
    query8(this.box).find(":scope > div > .tsg-panel").css("transition", instant !== true ? ".2s" : "0s");
    setTimeout(() => {
      this.set(panel, { size });
    }, 1);
    setTimeout(() => {
      query8(this.box).find(":scope > div > .tsg-panel").css("transition", "0s");
      this.resize();
    }, 300);
    return true;
  }
  show(panel, immediate) {
    ...
    p.hidden = false;
    if (immediate === true) {
      query8(this.box).find("#layout_" + this.name + "_panel_" + panel).css({ "opacity": "1" });
      edata.finish();
      this.resize();
    } else {
      query8(this.box).addClass("animating");
      query8(this.box).find("#layout_" + this.name + "_panel_" + panel).css({ "opacity": "0" });
      query8(this.box).find(":scope > div > .tsg-panel").css("transition", ".2s");
      setTimeout(() => {
        this.resize();
      }, 1);
      setTimeout(() => {
  ...
  el(panel) {
    const el = query8(this.box).find("#layout_" + this.name + "_panel_" + panel + '> [data-role="panel-content"]');
  ...
  hideToolbar(panel) {
    ...
    query8(this.box).find(`#layout_${this.name}_panel_${panel} > [data-role="panel-toolbar"]`).hide();
    this.resize();
  }
  showToolbar(panel) {
    const pan = this.get(panel);
    if (!pan) return;
    pan.show.toolbar = true;
    query8(this.box).find(`#layout_${this.name}_panel_${panel} > [data-role="panel-toolbar"]`).show();
    this.resize();
  }
  toggleToolbar(panel) {
    const pan = this.get(panel);
  ...
  assignToolbar(panel, toolbar) {
    ...
    const pan = this.get(panel);
    pan.toolbar = toolbar;
    const tmp = query8(this.box).find(panel + '> [data-role="panel-toolbar"]');
    if (pan.toolbar != null) {
  ...
  hideTabs(panel) {
    ...
    if (!pan) return;
    pan.show.tabs = false;
    query8(this.box).find("#layout_" + this.name + "_panel_" + panel + '> [data-role="panel-tabs"]').hide();
    this.resize();
  }
  showTabs(panel) {
    const pan = this.get(panel);
    if (!pan) return;
    pan.show.tabs = true;
    query8(this.box).find("#layout_" + this.name + "_panel_" + panel + '> [data-role="panel-tabs"]').show();
    this.resize();
  }
  toggleTabs(panel) {
    const pan = this.get(panel);
  ...
  assignTabs(panel, tabs) {
    ...
    const pan = this.get(panel);
    pan.tabs = tabs;
    const tmp = query8(this.box).find(panel + '> [data-role="panel-tabs"]');
    if (pan.tabs != null) {
  ...
  render(box) {
    ...
    if (!this.box) return false;
    query8(this.box).attr("name", this.name).addClass("tsg-layout").html("<div></div>");
    if (query8(this.box).length > 0) {
      ;
      query8(this.box)[0].style.cssText += this.style;
    }
    for (let p1 = 0; p1 < w2panels.length; p1++) {
      ...
      query8(this.box).find(":scope > div").append(html);
    }
    query8(this.box).find(":scope > div").append('<div id="layout_' + this.name + '_panel_css" style="position: absolute; top: 10000px;"></div>');
    this.refresh();
    this.last["observeResize"] = new ResizeObserver(() => {
      this.resize();
    });
    ...
    function resizeStart(type, evnt) {
      ...
      query8(document).off("mousemove", self.last["events"].mouseMove).on("mousemove", self.last["events"].mouseMove);
      query8(document).off("mouseup", self.last["events"].mouseUp).on("mouseup", self.last["events"].mouseUp);
      self.last["resize"] = {
        type,
        x: evnt.screenX,
        y: evnt.screenY,
      ...
      w2panels.forEach((panel) => {
        const $tmp = query8(self.el(panel)).find(".tsg-lock");
        if ($tmp.length > 0) {
          $tmp.data("locked", "yes");
        } else {
      ...
      const el = query8(self.box).find("#layout_" + self.name + "_resizer_" + type).get(0);
    ...
    function mouseUp(evnt) {
      if (!self.box) return;
      if (!evnt) evnt = window.event;
      query8(document).off("mousemove", self.last["events"].mouseMove);
      query8(document).off("mouseup", self.last["events"].mouseUp);
      ...
      if (self.last["diff_x"] !== 0 || self.last["resize"].diff_y !== 0) {
        ...
        const width = TsUtils.getSize(query8(self.box), "width");
        const height = TsUtils.getSize(query8(self.box), "height");
      ...
      query8(self.box).find("#layout_" + self.name + "_resizer_" + self.last["resize"].type).removeClass(null).addClass("active");
      query8(self.box).find("#layout_" + self.name + "_resizer_" + self.last["resize"].type).removeClass("active");
    ...
    function mouseMove(evnt) {
      ...
      if (edata2.isCancelled === true) return;
      const p = query8(self.box).find("#layout_" + self.name + "_resizer_" + tmp.type);
  ...
  destroy() {
    ...
    if (query8(this.box).find("#layout_" + this.name + "_panel_main").length > 0) {
      this.unmount();
    }
    delete _TsUiRegistry()[this.name];
    edata.finish();
    if (this.last["events"] && this.last["events"].resize) {
      query8(window).off("resize", this.last["events"].resize);
    }
    return true;
  }
  refresh(panel) {
    ...
    if (typeof panel == "string") {
      ...
      query8(self.box).find(pname).css({ display: p.hidden ? "none" : "block" });
      if (p.resizable) {
        query8(self.box).find(rname).show();
      } else {
        query8(self.box).find(rname).hide();
      }
      if (typeof p.html == "object" && typeof p.html.render === "function") {
        ;
        p.html.box = query8(self.box).find(pname + '> [data-role="panel-content"]')[0];
        setTimeout(() => {
          if (query8(self.box).find(pname + '> [data-role="panel-content"]').length > 0) {
            const $content = query8(self.box).find(pname + '> [data-role="panel-content"]').removeClass(null).removeAttr("name").addClass("tsg-panel-content");
      ...
      let tmp = query8(self.box).find(pname + '> [data-role="panel-tabs"]');
      if (p.show.tabs) {
      ...
      tmp = query8(self.box).find(pname + '> [data-role="panel-toolbar"]');
      if (p.show.toolbar) {
      ...
      tmp = query8(self.box).find(pname + "> .tsg-panel-title");
      if (p.title) {
        ;
      ...
      if (query8(self.box).find("#layout_" + self.name + "_panel_main").length === 0) {
        self.render();
        return;
      }
      self.resize();
  ...
  resize() {
    ...
    const width = TsUtils.getSize(query8(this.box), "width");
    const height = TsUtils.getSize(query8(this.box), "height");
    const self = this;
    ...
    if (ptop != null && ptop.hidden !== true) {
      ...
      query8(this.box).find("#layout_" + this.name + "_panel_top").css({
        "display": "block",
        "left": l + "px",
        "top": t + "px",
        "width": w + "px",
      ...
      if (ptop.resizable) {
        ...
        query8(this.box).find("#layout_" + this.name + "_resizer_top").css({
          "display": "block",
          "left": l + "px",
          "top": t + "px",
          "width": w + "px",
      ...
      query8(this.box).find("#layout_" + this.name + "_panel_top").hide();
      query8(this.box).find("#layout_" + this.name + "_resizer_top").hide();
    }
    if (pleft != null && pleft.hidden !== true) {
      ...
      query8(this.box).find("#layout_" + this.name + "_panel_left").css({
        "display": "block",
        "left": l + "px",
        "top": t + "px",
        "width": w + "px",
      ...
      if (pleft.resizable) {
        ...
        query8(this.box).find("#layout_" + this.name + "_resizer_left").css({
          "display": "block",
          "left": l + "px",
          "top": t + "px",
          "width": w + "px",
      ...
      query8(this.box).find("#layout_" + this.name + "_panel_left").hide();
      query8(this.box).find("#layout_" + this.name + "_resizer_left").hide();
    }
    if (pright != null && pright.hidden !== true) {
      ...
      h = height - (stop ? ptop.sizeCalculated + this.padding : 0) - (sbottom ? pbottom.sizeCalculated + this.padding : 0);
      query8(this.box).find("#layout_" + this.name + "_panel_right").css({
        "display": "block",
        "left": l + "px",
        "top": t + "px",
        "width": w + "px",
      ...
      if (pright.resizable) {
        l = l - this.padding;
        w = this.resizer > this.padding ? this.resizer : this.padding;
        query8(this.box).find("#layout_" + this.name + "_resizer_right").css({
          "display": "block",
          "left": l + "px",
          "top": t + "px",
          "width": w + "px",
      ...
      query8(this.box).find("#layout_" + this.name + "_panel_right").hide();
      query8(this.box).find("#layout_" + this.name + "_resizer_right").hide();
    }
    if (pbottom != null && pbottom.hidden !== true) {
      ...
      query8(this.box).find("#layout_" + this.name + "_panel_bottom").css({
        "display": "block",
        "left": l + "px",
        "top": t + "px",
        "width": w + "px",
      ...
      if (pbottom.resizable) {
        ...
        h = this.resizer > this.padding ? this.resizer : this.padding;
        query8(this.box).find("#layout_" + this.name + "_resizer_bottom").css({
          "display": "block",
          "left": l + "px",
          "top": t + "px",
          "width": w + "px",
      ...
      query8(this.box).find("#layout_" + this.name + "_panel_bottom").hide();
      query8(this.box).find("#layout_" + this.name + "_resizer_bottom").hide();
    }
    ...
    query8(this.box).find("#layout_" + this.name + "_panel_main").css({
      "display": "block",
      "left": l + "px",
      "top": t + "px",
      "width": w + "px",
    ...
    if (pprev != null && pprev.hidden !== true) {
      ...
      query8(this.box).find("#layout_" + this.name + "_panel_preview").css({
        "display": "block",
        "left": l + "px",
        "top": t + "px",
        "width": w + "px",
      ...
      if (pprev.resizable) {
        t = t - (this.padding === 0 ? 0 : this.padding);
        h = this.resizer > this.padding ? this.resizer : this.padding;
        query8(this.box).find("#layout_" + this.name + "_resizer_preview").css({
          "display": "block",
          "left": l + "px",
          "top": t + "px",
          "width": w + "px",
      ...
      query8(this.box).find("#layout_" + this.name + "_panel_preview").hide();
      query8(this.box).find("#layout_" + this.name + "_resizer_preview").hide();
    }
    this.resizeBoxes();
    edata.finish();
    return Date.now() - time;
  ...
  resizeBoxes(panel) {
    ...
    panels.forEach((pname, ind) => {
      ...
      if (pan) {
        if (pan.title) {
          const el = query8(this.box).find(tmp2 + ".tsg-panel-title").css({ top: topHeight + "px", display: "block" });
          topHeight += TsUtils.getSize(el, "height");
        }
        if (pan.show.tabs) {
          const el = query8(this.box).find(tmp2 + '[data-role="panel-tabs"]').css({ top: topHeight + "px", display: "block" });
          topHeight += TsUtils.getSize(el, "height");
        }
        if (pan.show.toolbar) {
          const el = query8(this.box).find(tmp2 + '[data-role="panel-toolbar"]').css({ top: topHeight + "px", display: "block" });
          topHeight += TsUtils.getSize(el, "height");
        }
      }
      query8(this.box).find(tmp2 + '[data-role="panel-content"]').css({
        display: "block",
        top: topHeight + "px"
      });
    });
...
var TsField = class extends TsBase {
  ...
  init() {
    ...
    switch (this.type) {
      ...
      case "date": {
        ...
        options = this.options;
        if (query9(this.el).attr("placeholder") == null) {
          query9(this.el).attr("placeholder", options.format);
        }
        break;
      }
      case "time": {
      ...
      case "datetime": {
        ...
        options = this.options;
        if (query9(this.el).attr("placeholder") == null) {
          query9(this.el).attr("placeholder", options.placeholder || options.format);
        }
        break;
      }
      case "list":
      case "combo": {
        ...
        if (this.type === "list") {
          query9(this.el).addClass("tsg-select");
        ...
        query9(this.el).attr("autocapitalize", "off").attr("autocomplete", "off").attr("autocorrect", "off").attr("spellcheck", "false");
        if (options.selected.text != null) {
          query9(this.el).val(options.selected.text);
        }
        break;
      }
      case "enum": {
      ...
      case "file": {
        ...
        this.selected = options.selected;
        if (query9(this.el).attr("placeholder") == null) {
          query9(this.el).attr("placeholder", TsUtils.lang("Attach files by dragging and dropping or Click to Select"));
        }
        break;
      }
      default: {
  ...
  set(val, append) {
    if (["list", "enum", "file"].indexOf(this.type) !== -1) {
      ...
      if (this.type !== "list" && append) {
        ...
        query9(this.el).trigger("input").trigger("change");
      } else {
        if (val == null) val = [];
  ...
  refresh() {
    ...
    if (this.type == "color") {
      ...
      query9(this.helpers.suffix).find(":scope > div").css("background-color", color);
    }
    if (this.type == "list") {
      if (this.helpers.prefix) query9(this.helpers.prefix).hide();
      ...
      const focus = query9(this.helpers.search_focus);
      const icon = query9(focus.get(0).previousElementSibling);
      ...
      if (focus.val() === "") {
        ...
        if (this.selected?.id != null) {
          ...
          if (text != null) {
            ;
            query9(this.el).val(TsUtils.lang(text)).data({
              selected: text,
              selectedIndex: ind[0]
            });
          }
          ...
          this.el.value = "";
          query9(this.el).removeData("selected selectedIndex");
        }
      } else {
        focus.css("opacity", 1);
        icon.css("opacity", 1);
        ...
        setTimeout(() => {
          ...
          if (options.icon) {
            ...
            query9(this.helpers.search).find(".tsg-icon-search").addClass("show-search");
          } else {
            focus.css("margin-left", "0px");
            query9(this.helpers.search).find(".tsg-icon-search").removeClass("show-search");
          }
        }, 1);
      }
      if (query9(this.el).prop("readOnly") || query9(this.el).prop("disabled")) {
        setTimeout(() => {
          if (this.helpers.prefix) query9(this.helpers.prefix).css("opacity", "0.6");
          if (this.helpers.suffix) query9(this.helpers.suffix).css("opacity", "0.6");
        }, 1);
      } else {
        setTimeout(() => {
          if (this.helpers.prefix) query9(this.helpers.prefix).css("opacity", "1");
          if (this.helpers.suffix) query9(this.helpers.suffix).css("opacity", "1");
        }, 1);
      }
    }
    ...
    if (["enum", "file"].includes(this.type) && div) {
      ...
      if (this.selected?.length > 0) {
        query9(this.el).attr("placeholder", "");
      }
      div.find(".tsg-enum-placeholder").remove();
      ul.find(".li-item").remove();
      if (html !== "") {
        ul.prepend(html);
      } else if (query9(this.el).attr("placeholder") != null && div.find("input").val() === "") {
        const style = TsUtils.stripSpaces(`
                ...
                `);
        div.prepend(`<div class="tsg-enum-placeholder" style="${style}">${query9(this.el).attr("placeholder")}</div>`);
      }
      div.off(".w2item").on("scroll.w2item", (event2) => {
        ...
        const target = query9(mouseEvent.target).closest(".li-item");
        const index = target.attr("index");
        const item = index != null ? this.selected[Number(index)] : void 0;
        if (query9(target).hasClass("li-search")) return;
        mouseEvent.stopPropagation();
        let edata;
        if (query9(mouseEvent.target).hasClass("tsg-list-remove")) {
          if (query9(this.el).prop("readOnly") || query9(this.el).prop("disabled")) return;
          edata = this.trigger("remove", { target: this.el, originalEvent: mouseEvent, item });
          if (edata.isCancelled === true) return;
          const transfer = new DataTransfer();
          const input = query9(mouseEvent.target).closest(".tsg-list").find("input.file-input").get(0);
          if (input) {
          ...
          query9(this.el).trigger("input").trigger("change");
          query9(mouseEvent.target).remove();
        } else {
          ...
          if (preview) {
            ...
            TsTooltip2.show({
              ...
              const $img = query9(`#w2overlay-${name} img`);
              $img.on("load", function(_event2) {
                const w = this.clientWidth;
                const h = this.clientHeight;
                if (w < 300 && h < 300) return;
                if (w >= h && w > 300) query9(this).css("width", "300px");
                if (w < h && h > 300) query9(this).css("height", "300px");
        ...
        const target = query9(mouseEvent.target).closest(".li-item");
        if (query9(target).hasClass("li-search")) return;
        const idx = query9(mouseEvent.target).attr("index");
  ...
  resize() {
    ...
    if (focus) {
      query9(focus).css("width", width);
    }
    if (multi) {
      query9(multi).css("width", width - parseInt(styles["margin-left"], 10) - parseInt(styles["margin-right"], 10));
    }
    if (suffix) {
      this.addSuffix();
    }
    ...
    const div = this.helpers.multi;
    if (["enum", "file"].includes(this.type) && div) {
      query9(this.el).css("height", "");
      let cntHeight = query9(div).find(":scope div.tsg-multi-items").get(0).clientHeight + 5;
      ...
      query9(div).css("height", cntHeight + "px");
      query9(this.el).css({ "height": cntHeight + "px" });
    }
  ...
  reset() {
    if (this.tmp != null) {
      ...
      ["padding-left", "padding-right", "background-color", "border-color"].forEach((prop) => {
        if (this.tmp && this.tmp["old-" + prop] != null) {
          query9(this.el).css(prop, this.tmp["old-" + prop]);
    ...
    ;
    query9(this.el).val(this.clean(query9(this.el).val())).removeClass("TsField tsg-input").removeData("selected selectedIndex").off(".TsField");
    Object.keys(this.helpers).forEach((key) => {
      query9(this.helpers[key]).remove();
    });
    this.helpers = {};
  ...
  change(event2) {
    if (["int", "float", "money", "currency", "percent"].indexOf(this.type) !== -1) {
      const val = query9(this.el).val();
      const new_val = this.format(this.clean(query9(this.el).val()));
      if (val !== "" && val != new_val) {
        query9(this.el).val(new_val);
        event2.stopPropagation();
        event2.preventDefault();
        return false;
      }
    ...
    if (this.type === "color") {
      let color = query9(this.el).val();
      if (color.substr(0, 3).toLowerCase() !== "rgb") {
        color = "#" + color;
        const len = query9(this.el).val().length;
        if (len !== 8 && len !== 6 && len !== 3) color = "";
      }
      const next = query9(this.el).get(0).nextElementSibling;
      query9(next).find("div").css("background-color", color);
      if (query9(this.el).hasClass("has-focus")) {
        this.updateOverlay();
      }
    }
    if (["list", "enum", "file"].indexOf(this.type) !== -1) {
    ...
    if (["date", "time", "datetime"].indexOf(this.type) !== -1) {
      ...
      if (TsUtils.isInt(this.el.value) && tmp > 3e3) {
        ...
        query9(this.el).val(String(tmp)).trigger("input").trigger("change");
      }
    }
  }
  click(event2) {
  ...
  focus(event2) {
    if (this.type == "list" && document.activeElement == this.el) {
      ...
      if (event2.showMenu !== false && this.options.openOnFocus !== false && query9(this.el).hasClass("has-focus") && !this.tmp.overlay?.overlay?.displayed) {
        setTimeout(() => {
    ...
    if (["list", "combo", "enum"].indexOf(this.type) !== -1) {
      if (query9(this.el).prop("readOnly") || query9(this.el).prop("disabled")) {
        query9(this.el).addClass("has-focus");
        return;
      }
      ...
      if (this.type == "enum") {
        const search = query9(this.el.previousElementSibling).find(".li-search input").get(0);
    ...
    if (this.type == "file") {
      const prev = query9(this.el).get(0).previousElementSibling;
      query9(prev).addClass("has-focus");
    }
    query9(this.el).addClass("has-focus");
  }
  blur(_event) {
    const val = query9(this.el).val().trim();
    query9(this.el).removeClass("has-focus");
    if (["int", "float", "money", "currency", "percent"].includes(this.type)) {
      if (val !== "") {
        let newVal = val;
        let error = "";
        ...
        if (this.options.autoCorrect) {
          ;
          query9(this.el).val(newVal).trigger("input").trigger("change");
          if (error) {
            TsTooltip2.show({
    ...
    if (["date", "time", "datetime"].includes(this.type) && this.options.autoCorrect) {
      if (val !== "") {
        ...
        if (!TsDate2.inRange(this.el.value, this.options) || !check.bind(TsUtils)(this.el.value, this.options.format)) {
          ;
          query9(this.el).val("").trigger("input").trigger("change");
        }
      }
    }
    if (this.type === "enum") {
      ;
      query9(this.helpers.multi).find("input").val("").css("width", "15px");
    }
    if (this.type == "file") {
      const prev = this.el.previousElementSibling;
      query9(prev).removeClass("has-focus");
    }
    if (this.type === "list") {
  ...
  keyDown(event2, extra) {
    ...
    if (["int", "float", "money", "currency", "percent"].includes(this.type)) {
      if (!options.keyboard || query9(this.el).prop("readOnly") || query9(this.el).prop("disabled")) return;
      val = parseFloat(query9(this.el).val().replace(options.moneyRE, "")) || 0;
      inc = options.step;
      ...
      switch (key) {
        case 38:
          ...
          query9(this.el).val(String(newValue)).trigger("input").trigger("change");
          cancel = true;
          break;
        case 40:
          if (event2.shiftKey) break;
    ...
    if (["date", "datetime"].includes(this.type)) {
      ...
      dt = is(query9(this.el).val(), options.format, true);
      if (!dt) {
        dt = /* @__PURE__ */ new Date();
        daymil = 0;
      }
      switch (key) {
        case 38:
          ...
          query9(this.el).val(newDT).trigger("input").trigger("change");
          cancel = true;
          break;
        case 40:
          if (event2.shiftKey) break;
    ...
    if (this.type === "time") {
      ...
      if (cancel) {
        event2.preventDefault();
        query9(this.el).val(TsDate2.min2str(time)).trigger("input").trigger("change");
        this.moveCaret2end();
      }
    }
    if (["list", "enum"].includes(this.type)) {
      switch (key) {
        ...
        case 46:
          if (this.type == "list") {
            const search = query9(this.helpers.search_focus);
            if (search.val() == "") {
            ...
            const search = query9(this.helpers.multi).find("input");
            if (search.val() == "") {
  ...
  keyUp(event2) {
    if (this.type == "list") {
      ...
      if (search.val() !== "") {
        ...
        query9(this.el).attr("placeholder", this.tmp.pholder);
      }
  ...
  updateOverlay(_indexOnly) {
    ...
    if (this.type === "color") {
      ...
      TsColor2.show(TsUtils.extend({
        ...
        query9(this.el).val(color).trigger("input").trigger("change");
      }).liveUpdate((event2) => {
        const color = event2.detail.color;
        query9(this.helpers.suffix).find(":scope > div").css("background-color", "#" + color);
      });
    ...
    if (["list", "combo", "enum"].includes(this.type)) {
      ...
      if (this.type === "enum") {
        ...
        input = query9(el).find("input").get(0) ?? this.el;
      }
      if (this.type === "list") {
      ...
      if (query9(this.el).hasClass("has-focus") && !this.el.readOnly && !this.el.disabled) {
        ...
        this.tmp.overlay = TsMenu2.show(params).select((event2) => {
          if (["list", "combo"].includes(this.type)) {
            ...
            query9(input).val("");
            query9(this.el).val(this.selected.text).trigger("input").trigger("change");
            ...
            if (newItem) {
              ...
              query9(this.el).trigger("input").trigger("change");
              query9(this.helpers.multi).find("input").val("");
              const overlay = TsMenu2.get(this.el.id + "_menu");
              if (overlay) overlay.options.selected = this.selected;
              edata.finish();
            }
    ...
    if (["date", "time", "datetime"].includes(this.type)) {
      ...
      TsDate2.show(TsUtils.extend({
        ...
        if (date != null) {
          ;
          query9(this.el).val(date).trigger("input").trigger("change");
        }
      });
    }
  }
  ...
  */
  ...
  addPrefix() {
    ...
    if (this.helpers.prefix) query9(this.helpers.prefix).remove();
    query9(this.el).before(`<div class="tsg-field-helper">${this.options.prefix}</div>`);
    const helper = query9(this.el).get(0).previousElementSibling;
    query9(helper).css({
      "color": styles["color"],
      "font-family": styles["font-family"],
      "font-size": styles["font-size"],
    ...
    query9(this.el).css("padding-left", helper.clientWidth + "px !important");
  ...
  addSuffix() {
    ...
    if (this.options.arrows) {
      if (this.helpers.arrows) query9(this.helpers.arrows).remove();
      query9(this.el).after(
      ...
      const arrowHelper = query9(this.el).get(0).nextElementSibling;
      const $arrowHelper = query9(arrowHelper);
      $arrowHelper.css({
        "color": styles["color"],
        "font-family": styles["font-family"],
        "font-size": styles["font-size"],
      ...
      $arrowHelper.on("mousedown", (event2) => {
        const mouseEvent = event2;
        if (query9(mouseEvent.target).hasClass("arrow-up")) {
          this.keyDown(mouseEvent, { keyCode: 38 });
        }
        if (query9(mouseEvent.target).hasClass("arrow-down")) {
          this.keyDown(mouseEvent, { keyCode: 40 });
        }
      });
      pr += arrowHelper.clientWidth;
      query9(this.el).css("padding-right", pr + "px !important");
      this.helpers.arrows = arrowHelper;
    }
    if (this.options.suffix !== "") {
      if (this.helpers.suffix) query9(this.helpers.suffix).remove();
      query9(this.el).after(`<div class="tsg-field-helper">${this.options.suffix}</div>`);
      const suffixHelper = query9(this.el).get(0).nextElementSibling;
      query9(suffixHelper).css({
        "color": styles["color"],
        "font-family": styles["font-family"],
        "font-size": styles["font-size"],
        "height": this.el.clientHeight + "px",
      ...
      query9(this.el).css("padding-right", suffixHelper.clientWidth + "px !important");
  ...
  addSearch() {
    ...
    if (this.helpers.search) query9(this.helpers.search).remove();
    let tabIndex = parseInt(query9(this.el).attr("tabIndex"));
    ...
    let searchId = "";
    if (query9(this.el).attr("id") != null) {
      searchId = 'id="' + query9(this.el).attr("id") + '_search"';
    }
    const html = `
            <div class="tsg-field-helper">
                <span class="tsg-icon tsg-icon-search"></span>
                <input ${searchId} type="text" tabIndex="${tabIndex}" ${query9(this.el).prop("readOnly") ? "readonly" : ""}
                    autocapitalize="off" autocomplete="off" autocorrect="off" spellcheck="false"/>
            </div>`;
    query9(this.el).attr("tabindex", String(-1)).before(html);
    const helper = query9(this.el).get(0).previousElementSibling;
    this.helpers.search = helper;
    this.helpers.search_focus = query9(helper).find("input").get(0);
    const styles = getComputedStyle(this.el);
    const $helperSearch = query9(helper);
    $helperSearch.css({
    ...
    query9(helper).find("input").off(".tsg-helper").on("focus.tsg-helper", (event2) => {
      const focusEvent = event2;
      query9(focusEvent.target).val("");
      this.tmp.pholder = query9(this.el).attr("placeholder") ?? "";
      this.focus(focusEvent);
      focusEvent.stopPropagation();
    }).on("blur.tsg-helper", (event2) => {
      const focusEvent = event2;
      query9(focusEvent.target).val("");
      if (this.tmp.pholder != null) query9(this.el).attr("placeholder", this.tmp.pholder);
    ...
    query9(helper).off(".tsg-helper").on("click.tsg-helper", (_event) => {
      query9(helper).find("input").get(0).focus();
    });
  }
  // Used in enum/file
  addMultiSearch() {
    ...
    query9(this.helpers.multi).remove();
    let html = "";
    const styles = getComputedStyle(this.el);
                ...
                <div class="tsg-multi-items">
                    <div class="li-search">
                        <input ${searchId} type="text" autocapitalize="off" autocomplete="off" autocorrect="off" spellcheck="false"
                            tabindex="${tabIndex}"
                            ${query9(this.el).prop("readOnly") ? "readonly" : ""}
                            ${query9(this.el).prop("disabled") ? "disabled" : ""}>
                    </div>
                </div>
            </div>`;
    }
            ...
            <div class="tsg-field-helper tsg-list" style="${margin}">
                <div class="tsg-multi-file">
                    <input name="attachment" class="file-input" type="file" tabindex="-1"'
                        ...
                        ${query9(this.el).prop("readOnly") || query9(this.el).prop("disabled") ? "disabled" : ""}
                        ${query9(this.el).attr("accept") ? ' accept="' + query9(this.el).attr("accept") + '"' : ""}>
                </div>
                <div class="tsg-multi-items">
    ...
    query9(this.el).before(html).css({
      "border-color": "transparent",
      "background-color": "transparent"
    });
    const div = query9(this.el.previousElementSibling);
    this.helpers.multi = div;
    query9(this.el).attr("tabindex", String(-1));
    div.on("mousedown", (event2) => {
      query9(event2.target).addClass("has-focus");
    }).on("mouseup", (event2) => {
      query9(event2.target).removeClass("has-focus");
    }).on("click", (event2) => {
      this.focus(event2);
      this.updateOverlay();
    });
...
var TsGrid = class extends TsBase {
  ...
  add(record, first) {
    ...
    if (!url) {
      ...
      if ((this.last.vscroll.recIndStart ?? 0) <= indEnd && (this.last.vscroll.recIndEnd ?? 0) >= indStart) {
        ...
        query10(this.box).find("#grid_" + this.name + "_footer .tsg-footer-right .tsg-total").html(TsUtils.formatNumber(this.total));
      }
    } else {
      this.refresh();
    }
  ...
  set(recid, record, noRefresh) {
    ...
    if (recid == null) {
      for (let i = 0; i < this.records.length; i++) {
        TsUtils.extend(this.records[i], record);
      }
      ...
      if (isSummary) {
        TsUtils.extend(this.summary[ind], record);
      } else {
        TsUtils.extend(this.records[ind], record);
      }
  ...
  processGroupBy() {
    ...
    this.records.forEach((rec) => {
      ...
      if (group != null) {
        ...
        rec[groupBy.field] = "";
        this.last.groupBy_links[group].TsUi.children.push(rec);
      }
    });
  ...
  localSort(silent, noResetRefresh) {
    ...
    if (silent !== true && this.show.statusSort) {
      setTimeout(() => {
        this.status(TsUtils.lang("Sorting took ${count} seconds", { count: time / 1e3 }));
      }, 10);
    }
    return time;
  ...
  localSearch(silent) {
    ...
    if (silent !== true && this.show.statusSearch) {
      setTimeout(() => {
        this.status(TsUtils.lang("Search took ${count} seconds", { count: time / 1e3 }));
      }, 10);
    }
    return time;
  ...
  removeRange(...names) {
    ...
    for (let a = 0; a < names.length; a++) {
      const name = names[a];
      query10(this.box).find("#grid_" + this.name + "_" + name).remove();
      query10(this.box).find("#grid_" + this.name + "_f" + name).remove();
  ...
  refreshRanges() {
    ...
    let range;
    const time = Date.now();
    const rec1 = query10(this.box).find(`#grid_${this.name}_frecords`);
    const rec2 = query10(this.box).find(`#grid_${this.name}_records`);
    for (let i = 0; i < this.ranges.length; i++) {
      ...
      let td1 = query10(this.box).find("#grid_" + this.name + "_rec_" + TsUtils.escapeId(first.recid) + ' td[col="' + first.column + '"]');
      let td2 = query10(this.box).find("#grid_" + this.name + "_rec_" + TsUtils.escapeId(last.recid) + ' td[col="' + last.column + '"]');
      let td1f = query10(this.box).find("#grid_" + this.name + "_frec_" + TsUtils.escapeId(first.recid) + ' td[col="' + first.column + '"]');
      let td2f = query10(this.box).find("#grid_" + this.name + "_frec_" + TsUtils.escapeId(last.recid) + ' td[col="' + last.column + '"]');
      let _lastColumn = last.column;
      if (first.column < this.last.vscroll.colIndStart && last.column > this.last.vscroll.colIndStart) {
        td1 = query10(this.box).find("#grid_" + this.name + "_rec_" + TsUtils.escapeId(first.recid) + ' td[col="start"]');
      }
      if (first.column < this.last.vscroll.colIndEnd && last.column > this.last.vscroll.colIndEnd) {
        td2 = query10(this.box).find("#grid_" + this.name + "_rec_" + TsUtils.escapeId(last.recid) + ' td[col="end"]');
        _lastColumn = "end";
      }
      const index_top = parseInt(query10(this.box).find("#grid_" + this.name + "_rec_top").next().attr("index"));
      const index_bottom = parseInt(query10(this.box).find("#grid_" + this.name + "_rec_bottom").prev().attr("index"));
      const index_ftop = parseInt(query10(this.box).find("#grid_" + this.name + "_frec_top").next().attr("index"));
      const index_fbottom = parseInt(query10(this.box).find("#grid_" + this.name + "_frec_bottom").prev().attr("index"));
      if (td1.length === 0 && first.index < index_top && last.index > index_top) {
        td1 = query10(this.box).find("#grid_" + this.name + "_rec_top").next().find('td[col="' + first.column + '"]');
      }
      if (td2.length === 0 && last.index > index_bottom && first.index < index_bottom) {
        td2 = query10(this.box).find("#grid_" + this.name + "_rec_bottom").prev().find('td[col="' + _lastColumn + '"]');
      }
      if (td1f.length === 0 && first.index < index_ftop && last.index > index_ftop) {
        td1f = query10(this.box).find("#grid_" + this.name + "_frec_top").next().find('td[col="' + first.column + '"]');
      }
      if (td2f.length === 0 && last.index > index_fbottom && first.index < index_fbottom) {
        td2f = query10(this.box).find("#grid_" + this.name + "_frec_bottom").prev().find('td[col="' + last.column + '"]');
      }
      const edit = query10(this.box).find("#grid_" + this.name + "_editable");
      ...
      range = query10(this.box).find("#grid_" + this.name + "_f" + rg.name);
      if (td1f.length > 0 || td2f.length > 0) {
        if (range.length === 0) {
          rec1.append('<div id="grid_' + this.name + "_f" + rg.name + '" class="tsg-selection" style="' + rg.style + '">' + (rg.name == "selection" && this.show.selectionResizer ? '<div id="grid_' + this.name + '_resizer" class="tsg-selection-resizer"></div>' : "") + "</div>");
          range = query10(this.box).find("#grid_" + this.name + "_f" + rg.name);
        } else {
        ...
        if (td2f.length === 0) {
          td2f = query10(this.box).find("#grid_" + this.name + "_frec_" + TsUtils.escapeId(last.recid) + " td:last-child");
          if (td2f.length === 0) td2f = query10(this.box).find("#grid_" + this.name + "_frec_bottom td:first-child");
      ...
      range = query10(this.box).find("#grid_" + this.name + "_" + rg.name);
      if (td1.length > 0 || td2.length > 0) {
        if (range.length === 0) {
          rec2.append(`
                        <div id="grid_${this.name}_${rg.name}" class="tsg-selection ${rg.class ?? ""}" style="${rg.style}">
                            ${rg.name == "selection" && this.show.selectionResizer ? `<div id="grid_${this.name}_resizer" class="tsg-selection-resizer"></div>` : ""}
                        </div>
                    `);
          range = query10(this.box).find("#grid_" + this.name + "_" + rg.name);
        } else {
        ...
        if (td1.length === 0) {
          td1 = query10(this.box).find("#grid_" + this.name + "_rec_" + TsUtils.escapeId(first.recid) + " td:first-child");
          if (td1.length === 0) td1 = query10(this.box).find("#grid_" + this.name + "_rec_top td:first-child");
        }
        if (td2f.length !== 0) {
    ...
    query10(this.box).find(".tsg-selection-resizer").off(".resizer").on("mousedown.resizer", mouseStart).on("dblclick.resizer", (event2) => {
    ...
    function mouseStart(event2) {
      ...
      query10("body").off(".tsg-" + self.name).on("mousemove.tsg-" + self.name, mouseMove).on("mouseup.tsg-" + self.name, mouseStop);
      event2.preventDefault();
    }
    function mouseMove(event2) {
      const mv = self.last.move;
      ...
      if (tmp.tagName.toUpperCase() != "TD") tmp = query10(tmp).closest("td")[0];
      if (query10(tmp).attr("col") != null) column = parseInt(query10(tmp).attr("col"));
      if (column == null) {
        return;
      }
      tmp = query10(tmp).closest("tr")[0];
      const index = parseInt(query10(tmp).attr("index"));
    ...
    function mouseStop(_event) {
      ...
      query10("body").off(".tsg-" + self.name);
  ...
  select(...selectArgs) {
    ...
    if (this.selectType == "row") {
      for (let a = 0; a < args.length; a++) {
        ...
        if (this.searchData.length !== 0 || index + 1 >= (this.last.vscroll.recIndStart ?? 0) && index + 1 <= (this.last.vscroll.recIndEnd ?? 0)) {
          recEl1 = query10(this.box).find("#grid_" + this.name + "_frec_" + TsUtils.escapeId(recid));
          recEl2 = query10(this.box).find("#grid_" + this.name + "_rec_" + TsUtils.escapeId(recid));
        }
        if (this.selectType == "row") {
      ...
      for (const recid in new_sel) {
        ...
        if (index + 1 >= (this.last.vscroll.recIndStart ?? 0) && index + 1 <= (this.last.vscroll.recIndEnd ?? 0)) {
          recEl1 = query10(this.box).find("#grid_" + this.name + "_rec_" + TsUtils.escapeId(recid));
          recEl2 = query10(this.box).find("#grid_" + this.name + "_frec_" + TsUtils.escapeId(recid));
        }
        ...
        for (let t = 0; t < new_sel_recid.length; t++) {
          ...
          if (recEl1) {
            recEl1.find("#grid_" + this.name + "_data_" + index + "_" + col).addClass("tsg-selected");
            recEl1.find(".tsg-col-number").addClass("tsg-row-selected");
            recEl1.find(".tsg-grid-select-check").prop("checked", true);
          }
          if (recEl2) {
            recEl2.find("#grid_" + this.name + "_data_" + index + "_" + col).addClass("tsg-selected");
      ...
      for (let c = 0; c < col_sel.length; c++) {
        query10(this.box).find("#grid_" + this.name + "_column_" + col_sel[c] + " .tsg-col-header").addClass("tsg-col-selected");
      }
    }
    ...
    if (areAllSelected || areAllSearchedSelected) {
      query10(this.box).find("#grid_" + this.name + "_check_all").prop("checked", true);
    } else {
      query10(this.box).find("#grid_" + this.name + "_check_all").prop("checked", false);
    }
    this.status();
  ...
  unselect(...unselectArgs) {
    ...
    for (let a = 0; a < args.length; a++) {
      ...
      const recEl1 = query10(this.box).find("#grid_" + this.name + "_frec_" + TsUtils.escapeId(recid));
      const recEl2 = query10(this.box).find("#grid_" + this.name + "_rec_" + TsUtils.escapeId(recid));
      if (this.selectType == "row") {
        if (sel.indexes.indexOf(index) == -1) continue;
        ...
        query10(this.box).find(`#grid_${this.name}_rec_${TsUtils.escapeId(recid)} > td[col="${col}"]`).removeClass("tsg-selected tsg-inactive");
        query10(this.box).find(`#grid_${this.name}_frec_${TsUtils.escapeId(recid)} > td[col="${col}"]`).removeClass("tsg-selected tsg-inactive");
        let isColSelected = false;
        let isRowSelected = false;
        const tmp2 = this.getSelectionCells();
        for (let i = 0; i < tmp2.length; i++) {
          if (tmp2[i].column == col) isColSelected = true;
          if (tmp2[i].recid == recid) isRowSelected = true;
        }
        if (!isColSelected) {
          query10(this.box).find(`.tsg-grid-columns td[col="${col}"] .tsg-col-header, .tsg-grid-fcolumns td[col="${col}"] .tsg-col-header`).removeClass("tsg-col-selected");
        }
        if (!isRowSelected) {
          query10(this.box).find("#grid_" + this.name + "_frec_" + TsUtils.escapeId(recid)).find(".tsg-col-number").removeClass("tsg-row-selected");
        }
        unselected++;
        if (s.length === 0) {
  ...
  compareSelection(newSel) {
    ...
    const unselect = [];
    if (this.selectType == "row") {
      const sel = this.getSelectionRows();
  ...
  selectAll() {
    ...
    const url = this.url?.get ?? this.url;
    let sel = TsUtils.clone(this.last.selection);
    const cols = [];
    ...
    if (this.selectType == "row") {
      query10(this.box).find(".tsg-grid-records tr:not(.tsg-empty-record)").addClass("tsg-selected").find(".tsg-col-number").addClass("tsg-row-selected");
      query10(this.box).find(".tsg-grid-frecords tr:not(.tsg-empty-record)").addClass("tsg-selected").find(".tsg-col-number").addClass("tsg-row-selected");
      query10(this.box).find("input.tsg-grid-select-check").prop("checked", true);
    } else {
      query10(this.box).find(".tsg-grid-columns td .tsg-col-header, .tsg-grid-fcolumns td .tsg-col-header").addClass("tsg-col-selected");
      query10(this.box).find(".tsg-grid-records tr .tsg-col-number").addClass("tsg-row-selected");
      query10(this.box).find(".tsg-grid-records tr:not(.tsg-empty-record)").find(".tsg-grid-data:not(.tsg-col-select)").addClass("tsg-selected");
      query10(this.box).find(".tsg-grid-frecords tr .tsg-col-number").addClass("tsg-row-selected");
      query10(this.box).find(".tsg-grid-frecords tr:not(.tsg-empty-record)").find(".tsg-grid-data:not(.tsg-col-select)").addClass("tsg-selected");
      query10(this.box).find("input.tsg-grid-select-check").prop("checked", true);
    }
    sel = this.getSelectionRows(true);
    this.addRange("selection");
    query10(this.box).find("#grid_" + this.name + "_check_all").prop("checked", true);
    this.status();
  ...
  selectNone(skipEvent) {
    ...
    const sel = this.last.selection;
    if (this.selectType == "row") {
      query10(this.box).find(".tsg-grid-records tr.tsg-selected").removeClass("tsg-selected tsg-inactive").find(".tsg-col-number").removeClass("tsg-row-selected");
      query10(this.box).find(".tsg-grid-frecords tr.tsg-selected").removeClass("tsg-selected tsg-inactive").find(".tsg-col-number").removeClass("tsg-row-selected");
      query10(this.box).find("input.tsg-grid-select-check").prop("checked", false);
    } else {
      query10(this.box).find(".tsg-grid-columns td .tsg-col-header, .tsg-grid-fcolumns td .tsg-col-header").removeClass("tsg-col-selected");
      query10(this.box).find(".tsg-grid-records tr .tsg-col-number").removeClass("tsg-row-selected");
      query10(this.box).find(".tsg-grid-frecords tr .tsg-col-number").removeClass("tsg-row-selected");
      query10(this.box).find(".tsg-grid-data.tsg-selected").removeClass("tsg-selected tsg-inactive");
      query10(this.box).find("input.tsg-grid-select-check").prop("checked", false);
    }
    sel.indexes = [];
    sel.columns = {};
  ...
  updateToolbar(sel, _areAllSelected) {
    ...
    if (this.show.toolbarSave) {
      if (this.getChanges().length > 0) {
        this.toolbar.enable("tsg-save");
      } else {
        this.toolbar.disable("tsg-save");
      }
    }
  ...
  getSelection(returnIndex) {
    return this.selectType === "row" ? this.getSelectionRows(returnIndex) : this.getSelectionCells();
  }
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  search(field, value) {
    const url = this.url?.get ?? this.url;
    ...
    const overlay = query10(`#w2overlay-${this.name}-search-overlay`);
    if (value === "") value = null;
    for (let i = 0; i < this.searches.length; i++) {
      ...
      if (value != null) {
        if (field.toLowerCase() == "all") {
          if (this.searches.length > 0) {
            for (let i = 0; i < this.searches.length; i++) {
              const search = this.searches[i];
              if (search.type == "text" || search.type == "alphanumeric" && TsUtils.isAlphaNumeric(value) || search.type == "int" && TsUtils.isInt(value) || search.type == "float" && TsUtils.isFloat(value) || search.type == "percent" && TsUtils.isFloat(value) || (search.type == "hex" || search.type == "color") && TsUtils.isHex(value) || search.type == "currency" && TsUtils.isMoney(value) || search.type == "money" && TsUtils.isMoney(value) || search.type == "date" && TsUtils.isDate(value) || search.type == "time" && TsUtils.isTime(value) || search.type == "datetime" && TsUtils.isDateTime(value) || search.type == "datetime" && TsUtils.isDate(value) || search.type == "enum" && TsUtils.isAlphaNumeric(value) || search.type == "list" && TsUtils.isAlphaNumeric(value)) {
  ...
  searchOpen(options = {}) {
    ...
    const $btn = query10(this.toolbar.box).find(".tsg-grid-search-input .tsg-search-drop");
    $btn.addClass("checked");
    TsTooltip3.show({
      name: this.name + "-search-overlay",
      anchor: query10(this.box).find("#grid_" + this.name + "_search_all").get(0),
      position: "bottom|top",
      ...
      overlay.data("gridName", this.name).off(".grid-search").on("click.grid-search", (event2) => {
        overlay.find("input, select").each((el) => {
          const names = query10(el).data("tooltipName");
          if (names) names.forEach((name) => {
            TsTooltip3.hide(name);
          });
        });
        console.log(event2.target);
        if (!query10(event2.target).hasClass("tsg-saved-searches")) {
          TsTooltip3.hide(this.name + "-search-suggest");
        }
      });
      TsUtils.bindEvents(overlay.find("select, input, button"), this);
      const sfields = query10(`#w2overlay-${this.name}-search-overlay *[rel=search]`);
  ...
  searchClose() {
    TsTooltip3.hide(this.name + "-search-overlay");
  }
  ...
  searchFieldTooltip(ind, sd_ind, el) {
    ...
    TsTooltip3.hide(this.name + "-search-props");
    TsTooltip3.show({
      ...
      query10(event2.detail.overlay.box).find("#remove").on("click", () => {
  ...
  searchSuggest(imediate, forceHide, anchor) {
    ...
    if (forceHide === true || anchor != null && query10(`#w2overlay-${this.name}-search-suggest`).length > 0) {
      TsTooltip3.hide(this.name + "-search-suggest");
      return;
    }
    if (query10(`#w2overlay-${this.name}-search-suggest`).length > 0) {
      return;
    }
    if (!imediate) {
    ...
    const el = anchor ?? query10(this.box).find(`#grid_${this.name}_search_all`).get(0);
    const searches = [
      ...this.defaultSearches ?? [],
    ...
    if (Array.isArray(searches) && searches.length > 0) {
      TsMenu3.show({
        ...
        this.searchData = TsUtils.clone(event2.detail.item.data);
        this["searchSelected"] = TsUtils.clone(event2.detail.item, { exclude: ["icon", "remove"] });
        this.reload();
        edata.finish();
      }).remove((event2) => {
        ...
        TsTooltip3.hide(this.name + "-search-overlay");
        this.confirm(TsUtils.lang('Do you want to delete search "${item}"?', { item: item.text })).yes((evt) => {
  ...
  searchSave() {
    ...
    this.message({
      ...
      query10(event2.detail.box).find("input, button").eq(0).val(value);
      await event2.complete;
      query10(event2.detail.box).find("#grid-search-cancel").on("click", () => {
        this.message();
      });
      query10(event2.detail.box).find("#grid-search-save").on("click", () => {
        const input = query10(event2.detail.box).find(".tsg-message .search-name");
        ...
        if (this["searchSelected"]) {
          ...
          query10(this.box).find(`#grid_${this.name}_search_name .name-text`).html(name);
        } else {
          this["searchSelected"] = {
            text: name,
            logic: this.last.logic,
          ...
          query10(event2.detail.box).find(`#grid_${this.name}_search_all`).val(" ").prop("readOnly", true);
          query10(event2.detail.box).find(`#grid_${this.name}_search_name`).show().find(".name-text").html(name);
        }
        edata.finish({ name });
      });
      await TsUtils.wait(100);
      query10(event2.detail.box).find("input, button").off(".message").on("keydown.message", (evt) => {
        const val = String(query10(event2.detail.box).find(".tsg-message-body input").val()).trim();
        if (evt.keyCode == 13 && val != "") {
          query10(event2.detail.box).find("#grid-search-save").trigger("click");
        }
        if (evt.keyCode == 27) {
          this.message();
        }
      }).eq(0).on("input.message", (_evt) => {
        const $save = query10(event2.detail.box).closest(".tsg-message").find("#grid-search-save");
        if (String(query10(event2.detail.box).val()).trim() === "") {
  ...
  searchReset(noReload) {
    ...
    if (edata.isCancelled === true) return;
    const input = query10(this.box).find("#grid_" + this.name + "_search_all");
    this.searchData = edata.detail["searchData"];
  ...
  searchShowFields(forceHide) {
    if (forceHide === true) {
      TsTooltip3.hide(this.name + "-search-fields");
      return;
    }
    const items = [];
    ...
    TsMenu3.show({
      ...
      anchor: query10(this.box).find("#grid_" + this.name + "_search_name").parent().find(".tsg-search-down").get(0),
      items,
      align: "none",
      hideOn: ["doc-click", "select"]
    }).select((event2) => {
  ...
  searchInitInput(field, _value) {
    let search;
    const el = query10(this.box).find("#grid_" + this.name + "_search_all");
    if (field == "all") {
    ...
    if (this["searchSelected"]) {
      query10(this.box).find(`#grid_${this.name}_search_all`).val(" ").prop("readOnly", true);
      query10(this.box).find(`#grid_${this.name}_search_name`).show().find(".name-text").html(this["searchSelected"].text);
    } else {
      query10(this.box).find(`#grid_${this.name}_search_all`).prop("readOnly", false);
      query10(this.box).find(`#grid_${this.name}_search_name`).hide().find(".name-text").html("");
    }
  }
  ...
  reset(noRefresh) {
    ...
    query10(this.box).find(`#grid_${this.name}_records`).prop("scrollTop", 0);
    if (!noRefresh) this.refresh();
  }
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  request(action, postData, url, callBack) {
    ...
    if (!TsUtils.isInt(this.offset)) this.offset = 0;
    if (!TsUtils.isInt(this.last.fetch.offset)) this.last.fetch.offset = 0;
    let edata;
    const params = {
      limit: this.limit,
    ...
    if (this.last.fetch.offset === 0) {
      this.lock(TsUtils.lang(this.msgRefresh), true);
    }
    ...
    if (!error) {
      ...
      if (action == "load") {
        ...
        if (!this.last.fetch.hasMore) {
          query10(this.box).find("#grid_" + this.name + "_rec_more, #grid_" + this.name + "_frec_more").hide();
        }
        if (this.last.fetch.offset === 0) {
          this.records = [];
          this.summary = [];
  ...
  editField(recid, column, value, event2) {
    ...
    if (this.last.inEditMode === true) {
      if (event2 && event2.keyCode == 13) {
        ...
        const input2 = query10(this.box).find("div.tsg-edit-box .tsg-input");
        if (input2.length > 0) {
    ...
    const tr = query10(this.box).find("#grid_" + this.name + prefix + "rec_" + TsUtils.escapeId(recid));
    let div = tr.find('[col="' + column + '"] > div');
    this.last._edit["tr"] = tr;
    this.last._edit["div"] = div;
    query10(this.box).find("div.tsg-edit-box").remove();
    if (this.selectType != "row") {
      query10(this.box).find("#grid_" + this.name + prefix + "selection").attr("id", "grid_" + this.name + "_editable").removeClass("tsg-selection").addClass("tsg-edit-box").prepend('<div style="position: absolute; top: 0px; bottom: 0px; left: 0px; right: 0px;"></div>').find(".tsg-selection-resizer").remove();
      div = query10(this.box).find("#grid_" + this.name + "_editable > div:first-child");
    }
    edit.attr = edit.attr ?? "";
    edit.text = edit.text ?? "";
    ...
    switch (edit.type) {
      case "div": {
        div.addClass("tsg-editable").html(TsUtils.stripSpaces(`<div id="grid_${this.name}_edit_${recid}_${column}" class="tsg-input tsg-focus"
      ...
      default: {
        div.addClass("tsg-editable").html(TsUtils.stripSpaces(`<input id="grid_${this.name}_edit_${recid}_${column}" class="tsg-input"
        ...
        const doHide = (event3) => {
          ...
          let selected = false;
          const name = query10(input).data("tooltipName");
    ...
    query10(input).off(".tsg-editable").on("blur.tsg-editable", (event3) => {
    ...
    function expand(input2) {
      try {
        ...
        const editBox = query10(self.box).find("#grid_" + self.name + "_editable").get(0);
        ...
        if (width + 20 > editBox.clientWidth) {
          query10(editBox).css("width", width + 20 + "px");
        }
      } catch (e) {
      }
    }
  ...
  editDone(index, column, event2) {
    ...
    const summary = index < 0;
    const cell = query10(this.last._edit?.["tr"]).find('[col="' + column + '"]');
    const rec = this.records[index];
    const col = this.columns[column];
    ...
    this.updateToolbar();
    setTimeout(() => {
      const input = query10(this.box).find(`#grid_${this.name}_focus`).get(0);
  ...
  click(recid, event2) {
    ...
    if (column == null && event2.target) {
      ...
      if (trg.tagName != "TD") trg = query10(trg).closest("td")[0];
      if (query10(trg).attr("col") != null) column = parseInt(query10(trg).attr("col"));
    }
    const index = this.get(recid, true);
    const rec = index != null ? this.records[index] : null;
    if (rec?.TsUi?.selectable === false && (rec?.TsUi?.children?.length ?? 0) > 0) {
      if (!query10(event2.target).hasClass("tsg-show-children")) {
        this.toggle(recid);
        return;
      }
    }
    ...
    if (event2.shiftKey && sel.length > 0 && this.multiSelect) {
      ...
      let fselect = false;
      if (query10(event2.target).closest("td").hasClass("tsg-col-select")) fselect = true;
  ...
  columnContextMenu(field, event2) {
    ...
    TsMenu3.show({
      ...
      query10("#w2overlay-context-menu .tsg-grid-skip").off(".tsg-grid").on("click.tsg-grid", (evt) => {
        evt.stopPropagation();
      }).on("keypress", (evt) => {
        if (evt.keyCode == 13) {
          this.skip(evt.target.value);
          this.toolbar.click("tsg-column-on-off");
        }
      });
    }).select((event3) => {
  ...
  columnAutoSize(colIndex) {
    ...
    const el = query10(`#grid_${this.name}_column_${colIndex} .tsg-col-header`)[0];
    ...
    query10(this.box).find(`.tsg-grid-records td[col="${colIndex}"] > div`, this.box).each((el2) => {
      const htmlEl = el2;
  ...
  focus(event2) {
    ...
    if (edata.isCancelled === true) return false;
    this.hasFocus = true;
    query10(this.box).removeClass("tsg-inactive").find(".tsg-inactive").removeClass("tsg-inactive");
    setTimeout(() => {
      const txt = query10(this.box).find(`#grid_${this.name}_focus`).get(0);
  ...
  blur(event2) {
    ...
    if (edata.isCancelled === true) return false;
    this.hasFocus = false;
    query10(this.box).addClass("tsg-inactive").find(".tsg-selected").addClass("tsg-inactive");
    query10(this.box).find(".tsg-selection").addClass("tsg-inactive");
    edata.finish();
  }
  // any: targeted-any per typing_policy; TsGrid record/cell shape is user-defined at runtime
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  keydown(event2) {
    ...
    if (edata.isCancelled === true) return;
    if (query10(this.box).find(".tsg-message").length > 0) {
      if (event2.keyCode == 27) this.message();
      return;
    }
    let empty = false;
    const records = query10(obj.box).find("#grid_" + obj.name + "_records");
    ...
    const recEL = query10(obj.box).find(`#grid_${obj.name}_rec_${ind >= 0 ? TsUtils.escapeId(obj.records[ind].recid) : "none"}`);
    ...
    switch (key) {
      ...
      case 91: {
        ...
        if (TsUtils.isSafari) {
          ...
          const focus = query10(obj.box).find("#grid_" + obj.name + "_focus");
  ...
  scrollIntoView(ind, column, instant, recTop) {
    ...
    const records = query10(this.box).find(`#grid_${this.name}_records`);
  ...
  dblClick(recid, event2) {
    ...
    if (column == null && event2.target) {
      let tmp = event2.target;
      if (tmp.tagName.toUpperCase() != "TD") tmp = query10(tmp).closest("td")[0];
      column = parseInt(query10(tmp).attr("col"));
    }
    const index = this.get(recid, true);
    const rec = index != null ? this.records[index] : null;
  ...
  showContextMenu(event2, options) {
    ...
    if (event2 == null) {
      event2 = { offsetX: 0, offsetY: 0, target: query10(this.box).find(`#grid_${this.name}_rec_${recid}`)[0] };
    }
  ...
  expand(recid, noRefresh) {
    ...
    if (Array.isArray(children)) {
      ...
      if (query10(this.box).find("#grid_" + this.name + "_rec_" + id + "_expanded_row").length > 0 || this.show.expandColumn !== true) return false;
      if (rec.TsUi.expanded == "none") return false;
      query10(this.box).find("#grid_" + this.name + "_rec_" + id).after(
        `<tr id="grid_${this.name}_rec_${recid}_expanded_row" class="tsg-expanded-row">
      ...
      query10(this.box).find("#grid_" + this.name + "_frec_" + id).after(
        `<tr id="grid_${this.name}_frec_${recid}_expanded_row" class="tsg-expanded-row">
                    ${this.show.lineNumbers ? '<td class="tsg-col-number"></td>' : ""}
      ...
      if (edata.isCancelled === true) {
        query10(this.box).find("#grid_" + this.name + "_rec_" + id + "_expanded_row").remove();
        query10(this.box).find("#grid_" + this.name + "_frec_" + id + "_expanded_row").remove();
        return false;
      }
      const row1 = query10(this.box).find("#grid_" + this.name + "_rec_" + recid + "_expanded");
      const row2 = query10(this.box).find("#grid_" + this.name + "_frec_" + recid + "_expanded");
      ...
      query10(this.box).find("#grid_" + this.name + "_rec_" + id).attr("expanded", "yes").addClass("tsg-expanded");
      query10(this.box).find("#grid_" + this.name + "_frec_" + id).attr("expanded", "yes").addClass("tsg-expanded");
      query10(this.box).find("#grid_" + this.name + "_cell_" + this.get(recid, true) + "_expand div").html("-");
      rec.TsUi.expanded = true;
      edata.finish();
      this.resizeRecords();
    }
  ...
  collapse(recid, noRefresh) {
    ...
    if (Array.isArray(children)) {
      ...
      clearExpanded(rec);
      const stops = [];
      for (let r = rec; r != null; r = r.TsUi?.parent_recid != null ? this.get(r.TsUi.parent_recid) : null)
        stops.push(r.TsUi?.parent_recid);
      const start = ind + 1;
      let end = start;
      while (true) {
        if (this.records.length <= end + 1 || this.records[end + 1].TsUi == null || stops.indexOf(this.records[end + 1].TsUi.parent_recid) >= 0) {
          break;
        }
        end++;
      }
      ...
      if (edata.isCancelled === true) return false;
      query10(this.box).find("#grid_" + this.name + "_rec_" + id).removeAttr("expanded").removeClass("tsg-expanded");
      query10(this.box).find("#grid_" + this.name + "_frec_" + id).removeAttr("expanded").removeClass("tsg-expanded");
      query10(this.box).find("#grid_" + this.name + "_cell_" + this.get(recid, true) + "_expand div").html("+");
      query10(this.box).find("#grid_" + this.name + "_rec_" + id + "_expanded").css("height", "0px");
      query10(this.box).find("#grid_" + this.name + "_frec_" + id + "_expanded").css("height", "0px");
      setTimeout(() => {
        query10(this.box).find("#grid_" + this.name + "_rec_" + id + "_expanded_row").remove();
        query10(this.box).find("#grid_" + this.name + "_frec_" + id + "_expanded_row").remove();
  ...
  getCellCopy(ind, col_ind) {
    return TsUtils.stripTags(this.getCellHTML(ind, col_ind));
  }
  // any: targeted-any per typing_policy; TsGrid record/cell shape is user-defined at runtime
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  paste(text, event2) {
  ...
  resize() {
    const time = Date.now();
    if (!this.box || query10(this.box).attr("name") != this.name) return;
    const edata = this.trigger("resize", { target: this.name });
    if (edata.isCancelled === true) return;
    if (this.box != null) {
      this.resizeBoxes();
  ...
  update({ cells, fullCellRefresh, ignoreColumns } = {}) {
    ...
    function _update(rec, row1, row2, index, column) {
      ...
      if (cell == null) return;
      if (fullCellRefresh) {
        query10(cell).replace(self.getCellHTML(index, column, false));
        cell = self.box.querySelector(`#grid_${self.name}_data_${index}_${column}`);
        rec.TsUi["_update"].cells[column] = cell;
      } else {
      ...
      if (rec.TsUi.class != null) {
        if (typeof rec.TsUi.class == "string") {
          ...
          const remove = [];
          const add = rec["TsUi"]["class"].split(" ").filter((cl) => !!cl);
          if (row1 && row2) {
        ...
        if (TsUtils.isPlainObject(rec.TsUi.class) && typeof rec.TsUi.class[pcol?.field ?? ""] == "string") {
          const ignore = ["tsg-grid-data"];
          const remove = [];
          const add = rec["TsUi"]["class"][pcol.field].split(" ").filter((cl) => !!cl);
          cell.classList.forEach((cl) => {
            if (!ignore.includes(cl)) remove.push(cl);
          });
          cell.classList.remove(...remove);
  ...
  refreshCell(recid, field) {
    ...
    const cell = query10(this.box).find(`${isSummary ? ".tsg-grid-summary " : ""}#grid_${this.name}_data_${index}_${col_ind}`);
  ...
  refreshRow(recid, ind = null) {
    let tr1 = query10(this.box).find("#grid_" + this.name + "_frec_" + TsUtils.escapeId(recid));
    let tr2 = query10(this.box).find("#grid_" + this.name + "_rec_" + TsUtils.escapeId(recid));
    if (tr1.length > 0) {
      ...
      if (typeof st == "string") {
        tr1 = query10(this.box).find("#grid_" + this.name + "_frec_" + TsUtils.escapeId(recid));
        tr2 = query10(this.box).find("#grid_" + this.name + "_rec_" + TsUtils.escapeId(recid));
  ...
  refresh() {
    ...
    if (edata.isCancelled === true) return;
    if (this.show.header) {
      query10(this.box).find(`#grid_${this.name}_header`).html(TsUtils.lang(this.header) + "&#160;").show();
    } else {
      query10(this.box).find(`#grid_${this.name}_header`).hide();
    }
    if (this.show.toolbar) {
      query10(this.box).find("#grid_" + this.name + "_toolbar").show();
    } else {
      query10(this.box).find("#grid_" + this.name + "_toolbar").hide();
    }
    this.searchClose();
    ...
    const sInput = query10(this.box).find("#grid_" + this.name + "_search_all");
    ...
    if (this.last.multi) {
      ...
      sInput.attr("placeholder", TsUtils.lang("Search") + " " + TsUtils.lang(this.last.label, true));
    }
    ...
    this.refreshBody();
    if (this.show.footer) {
      query10(this.box).find(`#grid_${this.name}_footer`).html(this.getFooterHTML()).show();
    } else {
      query10(this.box).find(`#grid_${this.name}_footer`).hide();
    }
    ...
    for (let r = 0; r < rows.length; r++) {
      const tmp = this.records[rows[r]].TsUi;
    ...
    if (this.markSearch) {
      setTimeout(() => {
        ...
        if (search.length > 0) {
          search.forEach((item) => {
            const el = query10(this.box).find('td[col="' + item.col + '"]:not(.tsg-head)');
  ...
  refreshSearch() {
    if (this.multiSearch && this.searchData.length > 0) {
      if (query10(this.box).find(".tsg-grid-searches").length == 0) {
        query10(this.box).find(".tsg-grid-toolbar").css("height", this.last.toolbar_height + 35 + "px").append(`<div id="grid_${this.name}_searches" class="tsg-grid-searches"></div>`);
      }
      let searches = `
                <span id="grid_${this.name}_search_logic" class="tsg-grid-search-logic"></span>
      ...
      searches += `
            ...
            `;
      query10(this.box).find(`#grid_${this.name}_searches`).html(searches);
      query10(this.box).find(`#grid_${this.name}_search_logic`).html(TsUtils.lang(this.last.logic == "AND" ? "All" : "Any"));
    } else {
      query10(this.box).find(".tsg-grid-toolbar").css("height", this.last.toolbar_height + "px").find(".tsg-grid-searches").remove();
    }
    if (this["searchSelected"]) {
      query10(this.box).find(`#grid_${this.name}_search_all`).val(" ").prop("readOnly", true);
      query10(this.box).find(`#grid_${this.name}_search_name`).show().find(".name-text").html(this["searchSelected"].text);
    ...
    TsUtils.bindEvents(query10(this.box).find(`#grid_${this.name}_searches .tsg-action, #grid_${this.name}_searches button`), this);
  }
  refreshBody() {
    this.updateExpanded();
    this.scroll();
    ...
    const bodyHTML = '<div id="grid_' + this.name + '_frecords" class="tsg-grid-frecords" style="margin-bottom: ' + (TsUtils.scrollBarSize() - 1) + 'px;">' + recHTML[0] + '</div><div id="grid_' + this.name + '_records" class="tsg-grid-records">' + recHTML[1] + '</div><div id="grid_' + this.name + '_scroll1" class="tsg-grid-scroll1" style="height: ' + TsUtils.scrollBarSize() + 'px"></div><div id="grid_' + this.name + '_fcolumns" class="tsg-grid-fcolumns">    <table><tbody>' + colHTML[0] + '</tbody></table></div><div id="grid_' + this.name + '_columns" class="tsg-grid-columns">    <table><tbody>' + colHTML[1] + `</tbody></table></div><div class="tsg-intersection-marker" style="display: none; height: ${this.recordHeight - 5}px">
               <div class="top-marker"></div>
               <div class="bottom-marker"></div>
            </div>`;
    const gridBody = query10(this.box).find(`#grid_${this.name}_body`, this.box).html(bodyHTML);
    const records = query10(this.box).find(`#grid_${this.name}_records`, this.box);
    const frecords = query10(this.box).find(`#grid_${this.name}_frecords`, this.box);
    if (this.selectType == "row") {
      records.on("mouseover mouseout", { delegate: "tr" }, (event2) => {
        const ind = query10(event2.delegate).attr("index");
        const recid = this.records[ind]?.recid;
        query10(this.box).find(`#grid_${this.name}_frec_${TsUtils.escapeId(recid)}`).toggleClass("tsg-record-hover", event2.type == "mouseover");
      });
      frecords.on("mouseover mouseout", { delegate: "tr" }, (event2) => {
        const ind = query10(event2.delegate).attr("index");
        const recid = this.records[ind]?.recid;
        query10(this.box).find(`#grid_${this.name}_rec_${TsUtils.escapeId(recid)}`).toggleClass("tsg-record-hover", event2.type == "mouseover");
      });
    }
    if (TsUtils.isMobile) {
      records.append(frecords).on("click", { delegate: "tr" }, (event2) => {
        const index = query10(event2.delegate).attr("index");
        ...
        const index = parseInt(query10(event2.delegate).attr("index"));
        const recid = this.records[index]?.recid;
        const td = query10(event2.target).closest("td");
    ...
    query10(this.box).find(".tsg-grid-body").off(".body-global").on("click.body-global dblclick.body-global contextmenu.body-global", { delegate: "td.tsg-head" }, (event2) => {
      const col_ind = parseInt(query10(event2.delegate).attr("col"));
      ...
      const col = query10(event2.delegate).parent().attr("col");
      this.columnTooltipShow(col, event2);
      query10(event2.delegate).off(".tooltip").on("mouseleave.tooltip", () => {
      ...
      if (["mouseenter", "mouseover"].includes(col?.["info"]?.showOn?.toLowerCase()) && event2.type == "mouseover") {
        this.showBubble(parseInt(tr.attr("index")), parseInt(td.attr("col")), isSummary).then(() => {
          query10(event2.delegate).off(".tooltip").on("mouseleave.tooltip", () => {
            TsTooltip3.hide(this.name + "-bubble");
          });
        });
      ...
      const td = query10(event2.delegate).parent();
      const tr = td.parent();
      const col = this.columns[td.attr("col")];
      const isSummary = tr.parents(".tsg-grid-body").hasClass("tsg-grid-summary");
      TsTooltip3.show({
        ...
        query10(event2.delegate).off(".tooltip");
      });
      query10(event2.delegate).off(".tooltip").on("mouseleave.tooltip", (_evt) => {
        TsTooltip3.hide(this.name + "-bubble");
      }).on("click.tooltip", (evt) => {
        evt.stopPropagation();
        TsTooltip3.update(this.name + "-bubble", TsUtils.lang("Copied"));
      ...
      const dt = query10(event2.delegate).data();
      this.editChange.call(this, event2.delegate, dt.changeind, dt.colind, event2);
      this.updateToolbar();
    });
    if (this.records.length === 0 && this.msgEmpty) {
      query10(this.box).find(`#grid_${this.name}_body`).append(`<div id="grid_${this.name}_empty_msg" class="tsg-grid-empty-msg"><div>${TsUtils.lang(this.msgEmpty)}</div></div>`);
    } else if (query10(this.box).find(`#grid_${this.name}_empty_msg`).length > 0) {
      query10(this.box).find(`#grid_${this.name}_empty_msg`).remove();
    }
    if (this.summary.length > 0) {
      const sumHTML = this.getSummaryHTML();
      query10(this.box).find(`#grid_${this.name}_fsummary`).html(sumHTML?.[0] ?? "").show();
      query10(this.box).find(`#grid_${this.name}_summary`).html(sumHTML?.[1] ?? "").show();
    } else {
      query10(this.box).find(`#grid_${this.name}_fsummary`).hide();
      query10(this.box).find(`#grid_${this.name}_summary`).hide();
    }
  }
  render(box) {
    const time = Date.now();
    ...
    query10(this.box).find(`#grid_${this.name}_footer`).html(this.getFooterHTML());
    ...
    query10(this.box).find(`#grid_${this.name}_focus`).on("focus", (_event) => {
      clearTimeout(this.last.kbd_timer ?? void 0);
    ...
    let edataCol;
    query10(this.box).off("mousedown.mouseStart").on("mousedown.mouseStart", mouseStart);
    this.updateToolbar();
    edata.finish();
    this.last["observeResize"] = new ResizeObserver(() => {
      this.resize();
    ...
    function mouseStart(event2) {
      ...
      if (obj.last.userSelect == "text") {
        obj.last.userSelect = "";
        query10(obj.box).find(".tsg-grid-body").css("user-select", "none");
      }
      if (obj.selectType == "row" && (query10(event2.target).parents().hasClass("tsg-head") || query10(event2.target).hasClass("tsg-head"))) return;
      if (obj.last.move && obj.last.move.type == "expand") return;
      if (event2.altKey) {
        query10(obj.box).find(".tsg-grid-body").css("user-select", "text");
        obj.selectNone();
        ...
        const index = query10(event2.target).parents("tr").attr("index");
        const recid = obj.records[index]?.recid;
        if (obj.selectType == "cell" && !event2.shiftKey) {
          let column1 = parseInt(query10(event2.target).closest("td").attr("col"));
          let column2 = column1;
          if (isNaN(column1)) {
            column1 = 0;
        ...
        obj.last.move = {
          ...
          focusY: pos.y,
          recid,
          column: parseInt(event2.target.tagName.toUpperCase() == "TD" ? query10(event2.target).attr("col") : query10(event2.target).parents("td").attr("col")),
          type: "select",
          ghost: false,
          start: true
        };
        if (obj.last.move.recid == null && obj.records.length > 0) {
          ...
          const column = parseInt(query10(event2.target).closest("td").attr("col"));
        ...
        const target = event2.target;
        const $input = query10(obj.box).find("#grid_" + obj.name + "_focus");
        if (obj.last.move) {
          let sLeft = obj.last.move.focusX;
          let sTop = obj.last.move.focusY;
          const $owner = query10(target).parents("table").parent();
          if ($owner.hasClass("tsg-grid-records") || $owner.hasClass("tsg-grid-frecords") || $owner.hasClass("tsg-grid-columns") || $owner.hasClass("tsg-grid-fcolumns") || $owner.hasClass("tsg-grid-summary")) {
            sLeft = obj.last.move.focusX - query10(obj.box).find("#grid_" + obj.name + "_records").prop("scrollLeft");
            sTop = obj.last.move.focusY - query10(obj.box).find("#grid_" + obj.name + "_records").prop("scrollTop");
          }
          if (query10(target).hasClass("tsg-grid-footer") || query10(target).parents("div.tsg-grid-footer").length > 0) {
            sTop = query10(obj.box).find("#grid_" + obj.name + "_footer").get(0).offsetTop;
          }
      ...
      if (obj.reorderRows == true) {
        ...
        if (el.tagName.toUpperCase() != "TD") el = query10(el).parents("td")[0];
        if (query10(el).hasClass("tsg-col-number") || query10(el).hasClass("tsg-col-order")) {
          ...
          const eColor = query10(obj.box).find(".tsg-even.tsg-empty-record").css("background-color");
          const oColor = query10(obj.box).find(".tsg-odd.tsg-empty-record").css("background-color");
          query10(obj.box).find(".tsg-even td").filter(":not(.tsg-col-number)").css("background-color", eColor);
          query10(obj.box).find(".tsg-odd td").filter(":not(.tsg-col-number)").css("background-color", oColor);
          const mv = obj.last.move;
          const recs = query10(obj.box).find(".tsg-grid-records");
          if (!mv["ghost"]) {
            const rows = sel.map((r) => query10(obj.box).find(`#grid_${obj.name}_rec_${r}`));
            ...
            mv["ghost"] = query10(rows.map((row) => row.get(0).cloneNode(true)));
            ...
            query10(obj.box).find("#grid_" + obj.name + "_ghost").append(tmp).append(mv["ghost"]);
          }
          const ghost = query10(obj.box).find("#grid_" + obj.name + "_ghost");
          ghost.css({
            top: mv.pos.top + "px",
            left: mv.pos.left + "px"
          });
      ...
      query10(document).on("mousemove.tsg-" + obj.name, mouseMove).on("mouseup.tsg-" + obj.name, mouseStop);
      event2.stopPropagation();
    }
    function mouseMove(event2) {
      ...
      if (obj.reorderRows == true && obj.last.move.reorder) {
        const tmp = query10(event2.target).parents("tr");
        ...
        if (mv.from.indexOf(recid2) == -1) {
          const row2 = query10(obj.box).find("#grid_" + obj.name + "_rec_" + recid2);
          query10(obj.box).find(".insert-before");
          row2.addClass("insert-before");
          mv.lastY = event2.screenY;
          mv.to = recid2;
          const pos = { top: row2.get(0)?.offsetTop, left: row2.get(0)?.offsetLeft };
          const ghost_line = query10(obj.box).find("#grid_" + obj.name + "_ghost_line");
          if (pos) {
            ghost_line.css({
              top: pos.top + "px",
              left: mv.pos.left + "px",
      ...
      const newSel = [];
      const ind = event2.target.tagName.toUpperCase() == "TR" ? query10(event2.target).attr("index") : query10(event2.target).parents("tr").attr("index");
      const recid = obj.records[ind]?.recid;
      if (recid == null) {
        if (obj.selectType == "row") return;
        if (obj.last.move && obj.last.move.type == "select") return;
        const col = parseInt(query10(event2.target).parents("td").attr("col"));
        if (isNaN(col)) {
          obj.removeRange("column-selection");
          query10(obj.box).find(".tsg-grid-columns .tsg-col-header, .tsg-grid-fcolumns .tsg-col-header").removeClass("tsg-col-selected");
          query10(obj.box).find(".tsg-col-number").removeClass("tsg-row-selected");
          delete mv.colRange;
        } else {
        ...
        let col2 = parseInt(event2.target.tagName.toUpperCase() == "TD" ? query10(event2.target).attr("col") : query10(event2.target).parents("td").attr("col"));
    ...
    function mouseStop(event2) {
      ...
      if (query10(event2.target).parents().hasClass(".tsg-head") || query10(event2.target).hasClass(".tsg-head")) return;
      obj.removeRange("selection-preview");
      if (mv && ["select", "select-column"].includes(mv.type)) {
        ...
        if (obj.reorderRows == true && obj.last.move.reorder) {
          if (mv.to != null) {
            ...
            obj.sortData = [];
            query10(obj.box).find(`#grid_${obj.name}_columns .tsg-col-header`).removeClass("tsg-col-sorted");
            resetRowReorder();
            obj.selectNone(true);
            obj.select(mv.from);
            edata2.finish();
      ...
      delete obj.last.move;
      query10(document).off(".tsg-" + obj.name);
    }
    function resetRowReorder() {
      query10(obj.box).find(`#grid_${obj.name}_ghost`).remove();
      query10(obj.box).find(`#grid_${obj.name}_ghost_line`).remove();
      obj.refresh();
      delete obj.last.move;
    }
  }
  ...
  destroy() {
    ...
    if (query10(this.box).find(`#grid_${this.name}_body`).length > 0) {
      this.unmount();
    }
    delete TsUi[this.name];
    edata.finish();
  ...
  initColumnOnOff() {
    ...
    if (this.show.skipRecords) {
      const skip = TsUtils.lang("Skip") + `<input id="${this.name}_skip" type="text" class="tsg-input tsg-grid-skip" value="${this.offset}">` + TsUtils.lang("records");
    ...
    this.toolbar.set("tsg-column-on-off", { selected, items });
    return items;
  }
  // any: callback parameter — caller signature varies; TsGrid record/cell shape is user-defined at runtime
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  initColumnDrag(_box) {
    ...
    const hasInvalidClass = (target, lastColumn) => {
      ...
      for (let i = 0; i < iClass.length; i++) {
        if (query10(target).closest(".tsg-head").hasClass(iClass[i])) {
          return true;
        }
      }
      return false;
    };
    query10(self.box).off(".colDrag").on("mousedown.colDrag", dragColStart);
    function dragColStart(event2) {
      ...
      if (!query10(event2.target).parents().hasClass("tsg-head") || hasInvalidClass(event2.target)) return;
      dragData.pressed = true;
      dragData["initialX"] = event2.pageX;
      dragData["initialY"] = event2.pageY;
      dragData["numberPreColumnsPresent"] = query10(self.box).find(preColHeadersSelector).length;
      const origColumn = dragData.columnHead = query10(event2.target).closest(".tsg-head");
      ...
      if (edata.isCancelled === true) return false;
      const columns = dragData["columns"] = query10(self.box).find(".tsg-head:not(.tsg-head-last)");
      query10(document).on("mouseup.colDrag", dragColEnd);
      query10(document).on("mousemove.colDrag", dragColOver);
      const col = self.columns[dragData["originalPos"]];
      const colText = TsUtils.lang(typeof col.text == "function" ? col.text(col) : col.text);
      dragData["ghost"] = query2.html(`<span col="${dragData["originalPos"]}">${colText}</span>`)[0];
      query10(document.body).append(dragData["ghost"]);
      query10(dragData["ghost"]).css({
        display: "none",
        left: event2.pageX,
        top: event2.pageY,
        opacity: 1,
    ...
    function dragColEnd(event2) {
      ...
      let target;
      const finish = () => {
        const ghosts = query10(self.box).find(".tsg-grid-ghost");
        query10(self.box).find(".tsg-intersection-marker").hide();
        query10(dragData["ghost"]).remove();
        ghosts.remove();
        query10(document).off(".colDrag");
        dragData = {};
      };
    ...
    function markIntersection(event2) {
      if (query10(event2.target).closest("td").length == 0) {
        return;
      }
      const td = query10(event2.target).closest("td");
      const newPos = td.hasClass("tsg-head-last") ? self.columns.length : parseInt(td.attr("col"));
      if (dragData.targetPos != newPos) {
        const rect1 = query10(self.box).find(".tsg-grid-body").get(0).getBoundingClientRect();
        const rect2 = query10(event2.target).closest("td").get(0).getBoundingClientRect();
        query10(self.box).find(".tsg-intersection-marker").show().css({
  ...
  initToolbar() {
    ...
    if (this.show.toolbarReload) {
      this.toolbar.items.push(TsUtils.extend({}, this.buttons["reload"]));
    }
    if (this.show.toolbarColumns) {
      this.toolbar.items.push(TsUtils.extend({}, this.buttons["columns"]));
    }
    if (this.show.toolbarSearch) {
      const html = `
                <div class="tsg-grid-search-input">
                    ${this.buttons["search"].html}
                    <div id="grid_${this.name}_search_name" class="tsg-grid-search-name">
                        <span class="name-icon tsg-icon-search"></span>
                        <span class="name-text"></span>
                        <span class="name-cross tsg-action" data-click="searchReset">x</span>
                    </div>
                    <input type="text" id="grid_${this.name}_search_all" class="tsg-search-all" tabindex="-1"
                        autocapitalize="off" autocomplete="off" autocorrect="off" spellcheck="false"
      ...
      this.toolbar.items.push({
        ...
        onRefresh: async (event2) => {
          await event2.complete;
          const input = query10(this.box).find(`#grid_${this.name}_search_all`);
          TsUtils.bindEvents(query10(this.box).find(`#grid_${this.name}_search_all, .tsg-action`), this);
          ...
          input.on("blur", () => {
            ...
            switch (event3.keyCode) {
              ...
              case 13: {
                TsMenu3.hide(this.name + "-search-suggest");
    ...
    if (Array.isArray(tb_items)) {
      ...
      if (this.show.toolbarAdd && !ids.includes(this.buttons["add"].id)) {
        this.toolbar.items.push(TsUtils.extend({}, this.buttons["add"]));
      }
      if (this.show.toolbarEdit && !ids.includes(this.buttons["edit"].id)) {
        this.toolbar.items.push(TsUtils.extend({}, this.buttons["edit"]));
      }
      if (this.show.toolbarDelete && !ids.includes(this.buttons["delete"].id)) {
        this.toolbar.items.push(TsUtils.extend({}, this.buttons["delete"]));
      }
      if (this.show.toolbarSave && !ids.includes(this.buttons["save"].id)) {
        if (this.show.toolbarAdd || this.show.toolbarDelete || this.show.toolbarEdit) {
          this.toolbar.items.push({ type: "break", id: "tsg-break2" });
        }
        this.toolbar.items.push(TsUtils.extend({}, this.buttons["save"]));
      }
      tb_items = tb_items.map((item) => this.buttons[item.name] ? TsUtils.extend({}, this.buttons[item.name], item) : item);
    }
      ...
      switch (event2.detail.item.id) {
        ...
        case "tsg-column-on-off":
          if (event2.detail.subItem) {
            ...
            setTimeout(() => {
              query10(`#w2overlay-${this.name}_toolbar-drop .tsg-grid-skip`).off(".tsg-grid").on("click.tsg-grid", (evt) => {
                evt.stopPropagation();
              }).on("keypress", (evt) => {
                if (evt.keyCode == 13) {
                  this.skip(evt.target.value);
  ...
  initResize() {
    const obj = this;
    query10(this.box).find(".tsg-resizer").off(".grid-col-resize").on("click.grid-col-resize", function(event2) {
      event2.stopPropagation();
      event2.preventDefault();
      ...
      obj.last.tmp = {
        ...
        gx: mev.screenX,
        gy: mev.screenY,
        col: parseInt(query10(this).attr("name"))
        // 'this' is the DOM element
      };
      obj.last.tmp.tds = query10(obj.box).find("#grid_" + obj.name + '_body table tr:first-child td[col="' + obj.last.tmp.col + '"]');
      mev.stopPropagation();
      mev.preventDefault();
      ...
      const mouseUp = function(event3) {
        query10(document).off(".grid-col-resize");
        obj.resizeRecords();
        obj.scroll();
      ...
      query10(document).off(".grid-col-resize").on("mousemove.grid-col-resize", mouseMove).on("mouseup.grid-col-resize", mouseUp);
    }).on("dblclick.grid-col-resize", function(event2) {
      const ind = parseInt(query10(this).attr("name"));
      obj.columnAutoSize(ind);
      event2.stopPropagation();
      event2.preventDefault();
    }).each((el) => {
      const td = query10(el).get(0).parentNode;
      query10(el).css({
  ...
  resizeBoxes() {
    const header = query10(this.box).find(`#grid_${this.name}_header`);
    const toolbar = query10(this.box).find(`#grid_${this.name}_toolbar`);
    const fsummary = query10(this.box).find(`#grid_${this.name}_fsummary`);
    const summary = query10(this.box).find(`#grid_${this.name}_summary`);
    const footer = query10(this.box).find(`#grid_${this.name}_footer`);
    const body = query10(this.box).find(`#grid_${this.name}_body`);
    if (this.show.header) {
      header.css({
        top: "0px",
        left: "0px",
    ...
    if (this.show.toolbar) {
      toolbar.css({
        top: 0 + (this.show.header ? TsUtils.getSize(header, "height") : 0) + "px",
        left: "0px",
        right: "0px"
      });
    }
    if (this.summary.length > 0) {
      fsummary.css({
        bottom: 0 + (this.show.footer ? TsUtils.getSize(footer, "height") : 0) + "px"
      });
      summary.css({
        bottom: 0 + (this.show.footer ? TsUtils.getSize(footer, "height") : 0) + "px",
        right: "0px"
    ...
    body.css({
      top: 0 + (this.show.header ? TsUtils.getSize(header, "height") : 0) + (this.show.toolbar ? TsUtils.getSize(toolbar, "height") : 0) + "px",
      bottom: 0 + (this.show.footer ? TsUtils.getSize(footer, "height") : 0) + (this.summary.length > 0 ? TsUtils.getSize(summary, "height") : 0) + "px",
      left: "0px",
      right: "0px"
    });
  }
  resizeRecords() {
    const obj = this;
    query10(this.box).find(".tsg-empty-record").remove();
    const box = query10(this.box);
    const grid = query10(this.box).find(":scope > div.tsg-grid-box");
    const header = query10(this.box).find(`#grid_${this.name}_header`);
    const toolbar = query10(this.box).find(`#grid_${this.name}_toolbar`);
    const summary = query10(this.box).find(`#grid_${this.name}_summary`);
    const fsummary = query10(this.box).find(`#grid_${this.name}_fsummary`);
    const footer = query10(this.box).find(`#grid_${this.name}_footer`);
    const body = query10(this.box).find(`#grid_${this.name}_body`);
    const columns = query10(this.box).find(`#grid_${this.name}_columns`);
    const fcolumns = query10(this.box).find(`#grid_${this.name}_fcolumns`);
    const records = query10(this.box).find(`#grid_${this.name}_records`);
    const frecords = query10(this.box).find(`#grid_${this.name}_frecords`);
    const scroll1 = query10(this.box).find(`#grid_${this.name}_scroll1`);
    ...
    if (body[0]?.clientHeight - (columns[0]?.clientHeight ?? 0) < (query10(records).find(":scope > table")[0]?.clientHeight ?? 0) + (bodyOverflowX ? TsUtils.scrollBarSize() : 0)) {
      bodyOverflowY = true;
    }
    if (!this.fixedBody) {
      const bodyHeight = TsUtils.getSize(columns, "height") + TsUtils.getSize(query10(this.box).find("#grid_" + this.name + "_records table"), "height") + (bodyOverflowX ? TsUtils.scrollBarSize() : 0);
      const calculatedHeight = bodyHeight + (this.show.header ? TsUtils.getSize(header, "height") : 0) + (this.show.toolbar ? TsUtils.getSize(toolbar, "height") : 0) + (summary.css("display") != "none" ? TsUtils.getSize(summary, "height") : 0) + (this.show.footer ? TsUtils.getSize(footer, "height") : 0);
      grid.css("height", calculatedHeight + "px");
      body.css("height", bodyHeight + "px");
      box.css("height", TsUtils.getSize(grid, "height") + "px");
    } else {
      const calculatedHeight = grid[0]?.clientHeight - (this.show.header ? TsUtils.getSize(header, "height") : 0) - (this.show.toolbar ? TsUtils.getSize(toolbar, "height") : 0) - (summary.css("display") != "none" ? TsUtils.getSize(summary, "height") : 0) - (this.show.footer ? TsUtils.getSize(footer, "height") : 0);
    ...
    if (bodyOverflowX || bodyOverflowY) {
      ...
      records.css({
        top: (this.columnGroups.length > 0 && this.show.columns ? 1 : 0) + TsUtils.getSize(columns, "height") + "px",
    ...
    function addEmptyRow(row, height, grid2) {
      ...
      query10(grid2.box).find("#grid_" + grid2.name + "_frecords > table").append(html1);
      query10(grid2.box).find("#grid_" + grid2.name + "_records > table").append(html2);
    }
    let width_box, percent;
    if (body.length > 0) {
      let width_max = parseInt(body[0].clientWidth) - (bodyOverflowY ? TsUtils.scrollBarSize() : 0) - (this.show.lineNumbers ? lineNumberWidth : 0) - (this.reorderRows ? 26 : 0) - (this.show.selectColumn ? 26 : 0) - (this.show.expandColumn ? 26 : 0) - 1;
      width_box = width_max;
      percent = 0;
      let restart = false;
      for (let i = 0; i < this.columns.length; i++) {
    ...
    columns.find(":scope > table > tbody > tr:nth-child(1) td").add(fcolumns.find(":scope > table > tbody > tr:nth-child(1) td")).each((el) => {
      if (query10(el).hasClass("tsg-col-number")) {
        query10(el).css("width", lineNumberWidth + "px");
      }
      const ind = query10(el).attr("col");
      if (ind != null) {
        if (ind == "start") {
          let width = 0;
          ...
          query10(el).css("width", width + "px");
        }
        if (obj.columns[ind]) query10(el).css("width", obj.columns[ind].sizeCalculated ?? "");
      }
      if (query10(el).hasClass("tsg-head-last")) {
        if ((obj.last.vscroll.colIndEnd ?? 0) + 1 < obj.columns.length) {
          ...
          query10(el).css("width", TsUtils.scrollBarSize() + (width_diff > 0 && percent === 0 ? width_diff : 0) + "px");
        }
      }
    });
    ...
    records.find(":scope > table > tbody > tr:nth-child(1) td").add(frecords.find(":scope > table > tbody > tr:nth-child(1) td")).each((el) => {
      ...
      if (query10(el).hasClass("tsg-grid-data-last") && query10(el).parents(".tsg-grid-frecords").length === 0) {
        if ((obj.last.vscroll.colIndEnd ?? 0) + 1 < obj.columns.length) {
          let width = 0;
          for (let i = (obj.last.vscroll.colIndEnd ?? 0) + 1; i < obj.columns.length; i++) {
            if (!obj.columns[i] || obj.columns[i].frozen || obj.columns[i].hidden) continue;
          ...
          query10(el).css("width", (width_diff > 0 && percent === 0 ? width_diff : 0) + "px");
        }
      }
    });
  ...
  getSearchesHTML() {
    let html = `
            <div class="search-title">
                ...
                ${this.savedSearches?.length > 0 ? `<button class="tsg-btn tsg-saved-searches" data-click="searchSuggest|true|false|this">Saved Searches (${this.savedSearches?.length ?? 0})</button>` : ""}
...
`;
  ...
  initOperator(ind) {
    ...
    switch (search.type) {
      ...
      case "enum":
        ...
        if (!$fld1[0]._w2field) {
          const fld2 = new TsField(search.type, {
            el: $fld1[0],
            ...options,
            // any: callback parameter — caller signature varies; TsGrid record/cell shape is user-defined at runtime
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    fields.forEach((field) => {
      ...
      if (search != null && search.options?.["parentList"] != null) {
        ...
        search["_w2field"]?.options?.items?.forEach?.((item) => {
          const parent2 = TsUtils.getNested(item, search?.options?.["parentField"] ?? "parentId");
          if (parent2 == null) {
            return;
          }
  ...
  getColumnCellHTML(i) {
    const col = this.columns[i];
    if (col == null) return "";
    const reorderCols = this.reorderColumns && (!this.columnGroups || !this.columnGroups.length) ? " tsg-col-reorderable " : "";
    let sortStyle = "";
    for (let si = 0; si < this.sortData.length; si++) {
      if (this.sortData[si].field == col.field) {
        if ((this.sortData[si].direction || "").toLowerCase() === "asc") sortStyle = "tsg-sort-up";
        if ((this.sortData[si].direction || "").toLowerCase() === "desc") sortStyle = "tsg-sort-down";
      }
    }
    ...
    const text = TsUtils.lang(typeof col.text == "function" ? col.text(col) : col.text);
    const html = '<td id="grid_' + this.name + "_column_" + i + '" col="' + i + '" class="tsg-head ' + sortStyle + reorderCols + '">' + (col.resizable !== false ? '<div class="tsg-resizer" name="' + i + '"></div>' : "") + '    <div class="tsg-col-header ' + (sortStyle ? "tsg-col-sorted" : "") + " " + (selected ? "tsg-col-selected" : "") + '">        <div class="' + sortStyle + '"></div>' + (!text ? "&#160;" : text) + "    </div></td>";
    return html;
  }
  // any: callback parameter — caller signature varies; TsGrid record/cell shape is user-defined at runtime
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  columnTooltipShow(ind, _event) {
    const $el = query10(this.box).find("#grid_" + this.name + "_column_" + ind);
  ...
  columnTooltipHide(_ind, _event) {
    TsTooltip3.hide(this.name + "-column-tooltip");
  }
  getRecordsHTML() {
    let buffered = this.records.length;
    const url = this.url?.get ?? this.url;
    ...
    html1 += '<tr id="grid_' + this.name + '_frec_bottom" rec="bottom" line="bottom" style="height: ' + h2 + 'px; vertical-align: top">    <td colspan="2000" style="border: 0"></td></tr><tr id="grid_' + this.name + '_frec_more" style="display: none; ">    <td colspan="2000" class="tsg-load-more"></td></tr></tbody></table>';
    html2 += '<tr id="grid_' + this.name + '_rec_bottom" rec="bottom" line="bottom" style="height: ' + h2 + 'px; vertical-align: top">    <td colspan="2000" style="border: 0"></td></tr><tr id="grid_' + this.name + '_rec_more" style="display: none">    <td colspan="2000" class="tsg-load-more"></td></tr></tbody></table>';
  ...
  scroll(event2) {
    ...
    if (event2) {
      ...
      const cols = query10(this.box).find(`#grid_${this.name}_columns`)[0];
      const summary = query10(this.box).find(`#grid_${this.name}_summary`)[0];
    ...
    if (colStart != null) {
      ...
      if (colStart != this.last.vscroll.colIndStart || colEnd != this.last.vscroll.colIndEnd) {
        ...
        if (deltaStart < 5 && deltaEnd < 5) {
          const $cfirst = $box.find(`.tsg-grid-columns #grid_${this.name}_column_start`);
          const $clast = $box.find(".tsg-grid-columns .tsg-head-last");
          const $rfirst = $box.find(`#grid_${this.name}_records .tsg-grid-data-spacer`);
          const $rlast = $box.find(`#grid_${this.name}_records .tsg-grid-data-last`);
          const $sfirst = $box.find(`#grid_${this.name}_summary .tsg-grid-data-spacer`);
          const $slast = $box.find(`#grid_${this.name}_summary .tsg-grid-data-last`);
          ...
          if (colStart < this.last.vscroll.colIndStart) {
            for (let i = (this.last.vscroll.colIndStart ?? 0) - 1; i >= colStart; i--) {
              ...
              $rfirst.each((el) => {
                const index = query10(el).parent().attr("index");
          ...
          if (colEnd > this.last.vscroll.colIndEnd) {
            for (let i = (this.last.vscroll.colIndEnd ?? 0) + 1; i <= colEnd; i++) {
              ...
              $rlast.each((el) => {
                ...
                let td = '<td class="tsg-grid-data" col="' + i + '" style="height: 0px"></td>';
                if (index != null) td = this.getCellHTML(parseInt(index), i, false);
                query10(el).before(td);
              });
              $slast.each((el) => {
                const index = query10(el).parent().attr("index") || -1;
    ...
    query10(this.box).find("#grid_" + this.name + "_footer .tsg-footer-right").html(
      (this.show.statusRange ? TsUtils.formatNumber(this.offset + t1) + "-" + TsUtils.formatNumber(this.offset + t2) + (this.total != -1 ? " " + TsUtils.lang("of") + ' <span class="tsg-total">' + TsUtils.formatNumber(this.total) + "</span>" : "") : "") + (url && this.show.statusBuffered ? " (" + TsUtils.lang("buffered") + ' <span class="tsg-buffered">' + TsUtils.formatNumber(buffered) + "</span>" + (this.offset > 0 ? ', skip <span class="tsg-skip">' + TsUtils.formatNumber(this.offset) : "") + "</span>)" : "")
    );
    ...
    if (e + 10 > buffered && this.last.vscroll.pull_more !== true && (buffered < this.total - this.offset || this.total == -1 && this.last.fetch.hasMore)) {
      ...
      const more = query10(this.box).find("#grid_" + this.name + "_rec_more, #grid_" + this.name + "_frec_more");
      more.show().eq(1).off(".load-more").on("click.load-more", function() {
        query10(this).find("td").html('<div><div style="width: 20px; height: 20px;" class="tsg-spinner"></div></div>');
    ...
    function markSearch() {
      ...
      obj.last.marker_timer = setTimeout(() => {
        ...
        if (search.length > 0) {
          search.forEach((item) => {
            const el = query10(obj.box).find('td[col="' + item.col + '"]:not(.tsg-head)');
            TsUtils.marker(el, item.search);
          });
        }
      }, 50);
  ...
  getRecordHTML(ind, lineNum, summary) {
    ...
    if (ind == -1) {
      rec_html1 += '<tr line="0">';
      rec_html2 += '<tr line="0">';
      if (this.show.lineNumbers) rec_html1 += '<td class="tsg-col-number" style="height: 0px"></td>';
      if (this.show.selectColumn) rec_html1 += '<td class="tsg-col-select" style="height: 0px"></td>';
      if (this.show.expandColumn) rec_html1 += '<td class="tsg-col-expand" style="height: 0px"></td>';
      rec_html2 += '<td class="tsg-grid-data tsg-grid-data-spacer" col="start" style="height: 0px; width: 0px"></td>';
      if (this.reorderRows) rec_html2 += '<td class="tsg-col-order" style="height: 0px"></td>';
      for (let i = 0; i < this.columns.length; i++) {
        const col = this.columns[i];
    ...
    rec_html1 += '<tr id="grid_' + this.name + "_frec_" + record.recid + '" recid="' + record.recid + '" line="' + lineNum + '" index="' + ind + '"  class="' + (lineNum % 2 === 0 ? "tsg-even" : "tsg-odd") + " tsg-record " + rec_class + (isRowSelected && this.selectType == "row" ? " tsg-selected" : "") + (record.TsUi && record.TsUi["editable"] === false ? " tsg-no-edit" : "") + (record.TsUi && record.TsUi.expanded === true ? " tsg-expanded" : "") + '"  style="height: ' + this.recordHeight + "px; " + (!isRowSelected && rec_style != "" ? rec_style : rec_style.replace("background-color", "none")) + '" ' + (rec_style != "" ? 'custom_style="' + rec_style + '"' : "") + ">";
    rec_html2 += '<tr id="grid_' + this.name + "_rec_" + record.recid + '" recid="' + record.recid + '" line="' + lineNum + '" index="' + ind + '"  class="' + (lineNum % 2 === 0 ? "tsg-even" : "tsg-odd") + " tsg-record " + rec_class + (isRowSelected && this.selectType == "row" ? " tsg-selected" : "") + (record.TsUi && record.TsUi["editable"] === false ? " tsg-no-edit" : "") + (record.TsUi && record.TsUi.expanded === true ? " tsg-expanded" : "") + '"  style="height: ' + this.recordHeight + "px; " + (!isRowSelected && rec_style != "" ? rec_style : rec_style.replace("background-color", "none")) + '" ' + (rec_style != "" ? 'custom_style="' + rec_style + '"' : "") + ">";
    if (this.show.lineNumbers) {
      rec_html1 += '<td id="grid_' + this.name + "_cell_" + ind + "_number" + (summary ? "_s" : "") + '"    class="tsg-col-number ' + (isRowSelected ? " tsg-row-selected" : "") + '"' + (this.reorderRows ? ' style="cursor: move"' : "") + ">" + (summary !== true ? this.getLineHTML(lineNum) : "") + "</td>";
    }
    if (this.show.selectColumn) {
      rec_html1 += '<td id="grid_' + this.name + "_cell_" + ind + "_select" + (summary ? "_s" : "") + '" class="tsg-grid-data tsg-col-select">' + (summary !== true && !(record.TsUi && record.TsUi["hideCheckBox"] === true) ? '    <div>        <input class="tsg-grid-select-check" type="checkbox" tabindex="-1" ' + (isRowSelected ? 'checked="checked"' : "") + ' style="pointer-events: none"/>    </div>' : "") + "</td>";
    }
    if (this.show.expandColumn) {
      ...
      rec_html1 += '<td id="grid_' + this.name + "_cell_" + ind + "_expand" + (summary ? "_s" : "") + '" class="tsg-grid-data tsg-col-expand">' + (summary !== true ? `<div>${tmp_img}</div>` : "") + "</td>";
    }
    rec_html2 += '<td class="tsg-grid-data-spacer" col="start" style="border-right: 0"></td>';
    if (this.reorderRows) {
      rec_html2 += '<td id="grid_' + this.name + "_cell_" + ind + "_order" + (summary ? "_s" : "") + '" class="tsg-grid-data tsg-col-order" col="order">' + (summary !== true ? '<div title="Drag to reorder">&nbsp;</div>' : "") + "</td>";
    }
    let col_ind = 0;
    let col_skip = 0;
    while (true) {
      ...
      if (col_skip > 0) {
        ...
        record.TsUi["colspan"][this.columns[col_ind - 1].field] = 0;
        col_skip--;
        continue;
      } else if (record.TsUi) {
  ...
  getCellHTML(ind, col_ind, summary, col_span) {
    ...
    data = '<td class="tsg-grid-data' + (isCellSelected ? " tsg-selected" : "") + " " + className + (isChanged ? " tsg-changed" : "") + '"    id="grid_' + this.name + "_data_" + ind + "_" + col_ind + '" col="' + col_ind + '"    style="' + style + (col.style != null ? col.style : "") + '" ' + (col.attr != null ? col.attr : "") + attr + ((col_span ?? 0) > 1 ? 'colspan="' + col_span + '"' : "") + ">" + data + (clipboardIcon && TsUtils.stripTags(data) ? clipboardIcon : "") + "</td>";
  ...
  clipboardCopy(ind, col_ind, summary) {
    ...
    query10(this.box).find("#grid_" + this.name + "_focus").text(txt).get(0).select();
  ...
  showBubble(ind, col_ind, summary) {
    ...
    let html = "";
    const rec = this.records[ind];
    const el = query10(this.box).find(`${summary ? ".tsg-grid-summary" : ""} #grid_${this.name}_data_${ind}_${col_ind} .tsg-info`);
    if (this.last.bubbleEl) {
      TsTooltip3.hide(this.name + "-bubble");
    }
      ...
      for (let i = 0; i < fields.length; i++) {
        ...
        if (tmp.length > 1) {
          if (TsUtils.formatters[tmp[1] ?? ""]) {
            ...
            val = TsUtils.formatters[tmp[1]].call(this, rec, extra);
          } else {
    ...
    return TsTooltip3.show(TsUtils.extend({
      name: this.name + "-bubble",
      html,
      anchor: el.get(0),
      position: "top|bottom",
  ...
  status(msg) {
    if (msg != null) {
      query10(this.box).find(`#grid_${this.name}_footer`).find(".tsg-footer-left").html(msg);
    } else {
      let msgLeft = "";
      const sel = this.getSelection();
      if (sel.length > 0) {
      ...
      query10(this.box).find("#grid_" + this.name + "_footer .tsg-footer-left").html(msgLeft);
    }
  }
  lock(msg, showSpinner) {
  ...
  unlock(speed) {
    setTimeout(() => {
      if (query10(this.box).find(".tsg-message").hasClass("tsg-closing")) return;
      TsUtils.unlock(this.box, speed);
    }, 25);
  }
  // any: callback parameter — caller signature varies; TsGrid record/cell shape is user-defined at runtime
...
var TsForm = class extends TsBase {
  ...
  getFieldValue(name) {
    ...
    if (["radio"].includes(field.type)) {
      const selected2 = query11(el).closest(".tsg-field-group").find("input:checked").get(0);
      if (selected2) {
        const item = field.options.items[query11(selected2).data("index")];
        current = item.id;
      } else {
        current = null;
      }
    ...
    if (["check", "checks"].includes(field.type)) {
      current = [];
      const selected2 = query11(el).closest(".tsg-field-group").find("input:checked");
      if (selected2.length > 0) {
        selected2.each((node) => {
          const el2 = node;
          const item = field.options.items[query11(el2).data("index")];
    ...
    if (["map", "array"].includes(field.type)) {
      ...
      field.$el.parent().find(".tsg-map-field").each((div, _ind) => {
        const key = query11(div).find(".tsg-map.key").val();
        const value = query11(div).find(".tsg-map.value").val();
        if (typeof field.html?.render == "function") {
          current[_ind] ??= {};
          query11(div).find("input, textarea").each((node) => {
            const inp = node;
  ...
  setFieldValue(name, value) {
    ...
    switch (field.type) {
      ...
      case "radio": {
        ...
        const inputs = query11(el).closest(".tsg-field-group").find("input");
      ...
      case "checks": {
        ...
        const inputs = query11(el).closest("div.tsg-field-group").find("input");
        const items = field.options.items;
        items.forEach((it, ind) => {
          const input = inputs.filter(`[data-index="${ind}"]`);
      ...
      case "array": {
        ...
        const container = query11(field.el).parent().find(".tsg-map-container");
        field.el.mapRefresh(value, container);
        break;
      }
      case "div":
      case "custom": {
        query11(el).html(value);
        break;
      }
      case "color": {
        el.value = value ?? "";
  ...
  updateEmptyGroups() {
    query11(this.box).find(".tsg-group").each((node) => {
      const group = node;
      if (isHidden(query11(group).find(".tsg-field"))) {
        query11(group).hide();
      } else {
        query11(group).show();
      }
  ...
  toggleGroup(groupName, show) {
    const el = query11(this.box).find('.tsg-group-title[data-group="' + TsUtils.base64encode(groupName) + '"]');
    if (el.length === 0) return;
    const el_next = query11(el.prop("nextElementSibling"));
  ...
  showErrors() {
    ...
    query11(errors[0].field.$el).parents(".tsg-field").get(0).scrollIntoView({ block: "nearest", inline: "nearest" });
    errors.forEach((error) => {
      ...
      if (error.field.type === "radio") {
        anchor = query11(error.field.el).closest("div").get(0);
      } else if (["enum", "file"].includes(error.field.type)) {
      }
      TsTooltip4.show(TsUtils.extend({
        anchor,
    ...
    query11(errors[0].field.$el).parents(".tsg-page").off(".hideErrors").on("scroll.hideErrors", (_evt) => {
  ...
  hideErrors() {
    ...
    this.fields.forEach((field) => {
      TsTooltip4.hide(`${this.name}-${field.field}-error`);
    });
  }
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  getChanges() {
  ...
  lockPage(page, msg, spinner) {
    const $page = query11(this.box).find(".page-" + page);
    if ($page.length) {
  ...
  goto(page) {
    ...
    if (query11(this.box).data("autoSize") === true) {
      query11(this.box).get(0).clientHeight = 0;
    }
    this.refresh();
  }
  generateHTML() {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  getAction(action) {
    const ret = query11(this.box).find('.tsg-buttons button[name="' + action + '"]');
    if (ret.length === 0) {
  ...
  resize() {
    ...
    if (this.box != null) {
      ...
      const header = query11(this.box).find(":scope > div .tsg-form-header");
      const toolbar = query11(this.box).find(":scope > div .tsg-form-toolbar");
      const tabs = query11(this.box).find(":scope > div .tsg-form-tabs");
      const page = query11(this.box).find(":scope > div .tsg-page");
      const dpage = query11(this.box).find(":scope > div .tsg-page.page-" + this.page + " > div");
      const buttons = query11(this.box).find(":scope > div .tsg-buttons");
      const { headerHeight, tbHeight, tabsHeight } = resizeElements2();
      if (this.autosize) {
        const cHeight = query11(this.box).get(0).clientHeight;
        if (cHeight === 0 || query11(this.box).data("autosize") == "yes") {
          query11(this.box).css({
          ...
          query11(this.box).data("autosize", "yes");
        }
        resizeElements2();
      }
      this.tabs?.resize?.();
  ...
  refresh(...args) {
    ...
    if (!this.isGenerated || !query11(this.box).html()) return 0;
    ...
    if (args.length > 0) {
      ...
      query11(this.box).find("input, textarea, select").each((node) => {
        const el = node;
        const name = query11(el).attr("name") != null ? query11(el).attr("name") : query11(el).attr("id");
        const field = this.get(name);
        if (field) {
          const div = query11(el).closest(".tsg-page");
          if (div.length > 0) {
            for (let i = 0; i < 100; i++) {
      ...
      query11(this.box).find(".tsg-page").hide();
      query11(this.box).find(".tsg-page.page-" + this.page).show();
      query11(this.box).find(".tsg-form-header").html(TsUtils.lang(this.header));
      if (typeof this.tabs === "object" && Array.isArray(this.tabs.tabs) && this.tabs.tabs.length > 0) {
        query11(this.box).find("#form_" + this.name + "_tabs").show();
        this.tabs.active = this.tabs.tabs[this.page].id;
        this.tabs.refresh();
      } else {
        query11(this.box).find("#form_" + this.name + "_tabs").hide();
      }
      if (typeof this.toolbar === "object" && Array.isArray(this.toolbar.items) && this.toolbar.items.length > 0) {
        query11(this.box).find("#form_" + this.name + "_toolbar").show();
        this.toolbar.refresh();
      } else {
        query11(this.box).find("#form_" + this.name + "_toolbar").hide();
      }
    }
    for (let f = 0; f < fields.length; f++) {
      ...
      field.$el = query11(this.box).find(`[name='${String(field.name).replace(/\\/g, "\\\\")}']`);
      ...
      field.$el.off(".TsForm").on("change.TsForm", function(event2) {
        ...
        if (["enum", "file"].includes(field.type)) {
          ...
          query11(helper).removeClass("tsg-error");
        }
      ...
      let tmp = field.el;
      if (!tmp) tmp = query11(this.box).find("#" + field.field);
      if (field.hidden) {
        query11(tmp).closest(".tsg-field").hide();
      } else {
        query11(tmp).closest(".tsg-field").show();
      }
    }
    query11(this.box).find("button, input[type=button]").each((node) => {
      const el = node;
      query11(el).off("click").on("click", function(event2) {
    ...
    for (let f = 0; f < fields.length; f++) {
      ...
      if (field.type == "switch") {
        ...
        field.$el.off(".form-input").on("focus.form-input", (event2) => {
          ...
          query11(event2.target).prop("_index", ind);
          query11(field.toolbar.box).addClass("tsg-tb-focus");
        }).on("blur.form-input", (event2) => {
          query11(event2.target).removeProp("_index");
          query11(`#${field.name}-tb .tsg-tb-button`).removeClass("over");
          query11(field.toolbar.box).removeClass("tsg-tb-focus");
        }).on("keydown.form-input", (event2) => {
          let ind = query11(event2.target).prop("_index");
          switch (event2.key) {
            case "ArrowLeft": {
              if (ind > 0) ind--;
              query11(`#${field.name}-tb .tsg-tb-button`).removeClass("over").eq(ind).addClass("over");
              query11(event2.target).prop("_index", ind);
              break;
            }
            case "ArrowRight": {
      ...
      if (["map", "array"].includes(field.type)) {
        (function(obj, field2) {
          field2.el.mapAdd = function(field3, div, cnt, empty) {
            ...
            if (typeof field3.html.render == "function") {
              ...
              if (!field3.el._errorDisplayed) {
                query2.html(html).filter("input, textarea").each((node) => {
                  const inp = node;
            ...
            if (typeof field3.html.render == "function") {
              ...
              box.find("input, textarea").each((el) => {
                if (query11(el).attr("tabindex") == null) {
                  query11(el).attr("tabindex", field3.html.tabindex);
                }
              });
          ...
          field2.el.mapRefresh = function(map, div) {
            ...
            let lastKey = null;
            const container = query11(field2.el).get(0)?.nextSibling;
            query11(container).off(".mapChange").on("mouseup.mapChange", { delegate: "input, textarea" }, function(event2) {
              if (document.activeElement != event2.target) {
                event2.target.focus();
              }
            }).on("keyup.mapChange", { delegate: "input, textarea" }, function(event2) {
              const kbdEvent = event2;
              const $div = query11(kbdEvent.target).closest(".tsg-map-field");
              const next = $div.get(0).nextElementSibling;
              const prev = $div.get(0).previousElementSibling;
              const className = query11(kbdEvent.target).hasClass("key") ? "key" : "value";
              if (kbdEvent.keyCode == 38 && prev) {
                query11(prev).find(`input.${className}, textarea.${className}, input[name="${kbdEvent.target["name"]}"] textarea[name="${kbdEvent.target["name"]}"]`).get(0)?.select();
              ...
              if (kbdEvent.keyCode == 40 && next) {
                ...
                query11(next2).find(`input.${className}, textarea.${className}, input[name="${kbdEvent.target["name"]}"] textarea[name="${kbdEvent.target["name"]}"]`).get(0)?.select();
                kbdEvent.preventDefault();
              }
              ...
              const $cnt = query11(event2.target).closest(".tsg-map-container");
              if (typeof field2.html?.render == "function") {
              ...
              let index;
              let className = "";
              const cnt2 = query11(event2.target).closest(".tsg-map-container");
              if (field2.type == "array" || lastKey == "tab") {
                cnt2.find("input, textarea").each((node, ind) => {
                  if (node == event2.target) index = ind;
                });
              } else {
                className = query11(event2.target).hasClass("key") ? ".key" : ".value";
  ...
  render(box) {
    ...
    query11(this.box).attr("name", this.name).addClass("tsg-reset tsg-form").html(html);
    if (query11(this.box).length > 0) query11(this.box).get(0).style.cssText += this.style;
    ...
    if (typeof this.toolbar === "object" && typeof this.toolbar.render === "function") {
      this.toolbar.render(query11(this.box).find("#form_" + this.name + "_toolbar").get(0));
    }
    ...
    if (typeof this.tabs === "object" && typeof this.tabs.render === "function") {
      this.tabs.render(query11(this.box).find("#form_" + this.name + "_tabs").get(0));
    ...
    if (this.focus != -1) {
      let setCount = 0;
      const setFocus = () => {
        if (query11(self.box).find("input, select, textarea").length > 0) {
          self.setFocus();
        } else {
          setCount++;
  ...
  destroy() {
    ...
    this.tabs?.destroy?.();
    this.toolbar?.destroy?.();
    if (query11(this.box).find("#form_" + this.name + "_tabs").length > 0) {
      this.unmount();
    }
    this.last.observeResize?.disconnect();
    delete TsUi[this.name];
  ...
  setFocus(focus) {
    ...
    if (TsUtils.isInt(focus)) {
      ...
      const inputs = query11(this.box).find("div:not(.tsg-field-helper) > input, select, textarea, div > label:nth-child(1) > [type=radio]").filter(":not(.file-input)");
      while (inputs.get(focus)?.offsetParent == null && inputs.length > focus) {
        focus++;
      }
      if (inputs.get(focus)) {
        $input = query11(inputs.get(focus));
      }
    } else if (typeof focus === "string") {
      $input = query11(this.box).find(`[name='${focus}']`);
    }
    if ($input?.length > 0) {
      $input.get(0).focus();
    }
