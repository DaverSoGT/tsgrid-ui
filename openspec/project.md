...
- **Spec language "MUST remain unmodified" — strict-stronger refinements are not drift**: when a spec requirement says a test file "MUST remain unmodified" but the underlying non-determinism source the test asserts on gets eliminated mid-cycle, the test MUST be refined to assert the new stronger guarantee (e.g. an "outputs MUST differ" sanity check flipped to "outputs MUST be byte-equal" after the wall-clock source is removed). This is NOT a spec violation — it is a strict-stronger semantic refinement that the spec language failed to anticipate. Spec authors SHOULD write **"MUST preserve or strengthen its guarantee"** instead of **"MUST remain unmodified"** when the test sits on a determinism boundary that the cycle might affect. Template reference: v2.14.0 R-FE-19 / commit `0b67fbea` (`test/unit/build-determinism.test.ts` sanity-check flipped from "headers differ" to "headers equal" after the gulp banner timestamp was removed — verify report #1117 W-1).
- **Unit-test version anchors — intentional manual-review triggers**: a unit test that hardcodes the current package version (e.g. `expect(pkg.version).toBe('2.14.0')`) MAY be left as a literal string when the test's PURPOSE is to force a manual review of associated baselines on every version bump (byte-floor ranges, fixture content, baseline JSON files). Such anchors are NOT refactor candidates — replacing the literal with `pkg.version` makes the test compare the same value it reads, turning the assertion into a tautology and defeating the manual-review trigger. The convention: (a) the `it(...)` block name MUST include "(release anchor)" or equivalent intent marker, (b) the release runbook MUST list the anchor file(s) as manual-update items, (c) a verify report MAY flag the anchor as a tracked test surface, but MUST NOT recommend the `pkg.version` refactor. Template reference: `test/unit/cjs-subpath-bytes.test.ts:17-19` (byte-floor baselines release anchor, originally v2.13.0, bumped to v2.14.0 in font-externalization cycle commit `72a8eb22` — verify report #1117 W-2).
- **Design-authorized dts widening**: when a re-export from a sub-module to a public subpath (e.g. `tsgrid-ui/utils`) requires that a source declaration be visible in the rolled-up `.d.ts`, the design phase MAY authorize removal of `@internal` JSDoc tags from the canonical declaration site as a documented Known Limitation. Spec/design need not pre-authorize this widening explicitly; apply may invoke this exception with a verify-report carry-forward classification. The convention: (a) the apply-phase discovery MUST be classified as a D-A (apply-time discovery) entry, (b) the CHANGELOG MUST document the widening in a Known Limitations section describing what the consumer sees (the type is now public), (c) the verify report MUST carry it forward as a WARNING and the archive report MUST record it as a documented spec drift. Future design phases should note explicitly when sub-module re-export will require @internal removal rather than leaving it as an implicit consequence. Precedent: barrel-deprecation v2.15.0 (Brand removed `@internal` per D-A-2 to enable R-TG-2 rollup-dts re-export; CHANGELOG Known Limitation #3 documents it — verify report #1130 W-3).
