...
test.describe('Sidebar', () => {
    test.beforeEach(async ({ page }) => {
        ...
        await page.waitForSelector('.TsUi-sidebar', { state: 'attached' })
    })

    test('renders sidebar container', async ({ page }) => {
        await expect(page.locator('.TsUi-sidebar')).toBeAttached()
    })

    test('renders tree nodes', async ({ page }) => {
        // TsUi-node class for each tree node rendered
        const nodes = page.locator('.TsUi-node')
