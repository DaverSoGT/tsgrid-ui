...
test.describe('Sidebar', () => {
    test.beforeEach(async ({ page }) => {
        ...
        await page.waitForSelector('.w2ui-sidebar', { state: 'attached' })
    })

    test('renders sidebar container', async ({ page }) => {
        await expect(page.locator('.w2ui-sidebar')).toBeAttached()
    })

    test('renders tree nodes', async ({ page }) => {
        // w2ui-node class for each tree node rendered
        const nodes = page.locator('.w2ui-node')
